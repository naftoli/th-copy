...
class JewishDatepicker {
        ...
        let html = '<div class="jewish_datepicker_inner">';
                ...
                html += '<section class="jewish_datepicker_section">';
                    html += '<div class="jewish_datepicker_section_week">';
                        html += '<span class="jewish_datepicker_week_day jewish_dp_day_1">S</span>';
                        html += '<span class="jewish_datepicker_week_day jewish_dp_day_2">M</span>';
                        html += '<span class="jewish_datepicker_week_day jewish_dp_day_3">T</span>';
                        html += '<span class="jewish_datepicker_week_day jewish_dp_day_4">W</span>';
                        html += '<span class="jewish_datepicker_week_day jewish_dp_day_5">T</span>';
                        html += '<span class="jewish_datepicker_week_day jewish_dp_day_6">F</span>';
                        html += '<span class="jewish_datepicker_week_day jewish_dp_day_7">S</span>';
                    html += '</div>';
                ...
                //     html += '<button class="jewish_datepicker_footer_today">Today</button>';
                //     html += '<button class="jewish_datepicker_footer_cancel">Cancel</button>';
                //     html += '<button class="jewish_datepicker_footer_apply">Apply</button>';
                // html += '</footer>';
    ...
    initialize(){
        ...
        const handleClickOutside = (event) => {
            if (!wrapper.contains(event.target) && event.target !== element) {
        ...
        // Store the cleanup function to remove the event listener later
    ...
    select_day( dayElement ) {
        let selectedDay = dayElement;
        let days = dayElement.closest('.jewish_datepicker_section_month').querySelectorAll('.jewish_datepicker_month_day');
        let jewish_datepicker_inner = dayElement.closest('.jewish_datepicker_inner');

        days.forEach( element => { element.classList.remove('select'); });

        let attr = {
            title       : selectedDay.getAttribute('title'),
            full        : selectedDay.getAttribute('data-full'),
            h_full      : selectedDay.getAttribute('data-h-full'),
            day_of_week : selectedDay.getAttribute('data-day_of_week'),
            h_day       : selectedDay.getAttribute('data-h_day'),
            h_month     : selectedDay.getAttribute('data-h_month'),
            h_year      : selectedDay.getAttribute('data-h_year'),
            day         : selectedDay.getAttribute('data-day'),
            month       : selectedDay.getAttribute('data-month'),
            year        : selectedDay.getAttribute('data-year'),
        }
        ...
        selectedDay.classList.add("select");
        if (!this.setfirstTiem)
            return;

        // Get the date directly from the attributes and format correctly
        // The format in attr.full is DD-MM-YYYY, we need to convert to YYYY-MM-DD
        const dateParts = attr.full.split('-');
        ...
        // Remove the first Hebrew word (usually 'יום') from the display
        const hebrewDateParts = attr.h_full.split(' ');
        const displayValue = hebrewDateParts.length > 1 ? hebrewDateParts.slice(1).join(' ') : attr.h_full;
        
        // Set both the display value (Hebrew date without first word) and the actual input value (Gregorian date)
        this.element.value = displayValue;
        this.element.setAttribute('data-gregorian-date', formattedDate);
        this.element.setAttribute('data-full-hebrew-date', attr.h_full);
        
        // Automatically close the datepicker after selection
    ...
    get_calendar_html(days_arr, currentDayObj, CurrentHeMonth){
        ...
        days_arr.forEach((dateobj, index) => {
            ...
            if( dateobj.category === "hebdate") {
                ...
                // day of week - fix the calculation
                // JavaScript getDay() returns 0 for Sunday, 1 for Monday, etc.
                // We need to adjust to match the Hebrew calendar where Sunday is 1
                ...
                obj_to_push.day_of_week = jsDate.getDay() + 1; // Sunday is 1, Monday is 2, etc.
                obj_to_push.events = [];
