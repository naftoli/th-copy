# Mashpia.com Source Code Repository
...
* Database Schema ( Do not have proper database seperation yet )
...
1. CentOS 7
2. PHP (5.4)
    1. For full extension list please see current production database
3. MySQL
...
1. Clone the master branch to the desired deployment location.
2. Edit `httpd.conf` ( or relevent apache configuration ) to point to the `public_html` directory in this repository.
3. Configure the DNS to be `mashpia.com` ( or a subdirectory of it ).
    1. If you do not do this please make sure that the database is accessible at `mashpia.com:3306`
...
2. Setup Virtualbox:
    ...
    3. Select the new image and press `Settings` and verify the following options:
        ...
        4. `Shared Folders` Please edit the existing shared folder ( or create a new one with the following properties ):
            1. Folder name **must** be `mashpia.com`.
...
3. Start the Virtual Machine and wait for it to prompt you with a login screen for the user `mashpia`.

4. Login with the password `tzivos5778` ( this is also your sudo and root password )
...
6. To ease development you may want to direct this IP address to `mashpia.local` in your hosts file. For instructions on how to edit your Operating Systems Hosts file please see this page: https://www.howtogeek.com/howto/27350/beginner-geek-how-to-edit-your-hosts-file/
