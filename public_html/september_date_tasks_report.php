...
if ($school_id > 0) {
	...
	$sql = $sql . "JOIN users AS u ON (u.school_id=" . $school_id . " AND u.class_id=c.class_id) ";
	...
	$sql = "SELECT DISTINCT s.* FROM schools JOIN subjects AS s USING (inst_id) JOIN school_subjects USING (school_id, subject_id) WHERE school_id=" . $school_id . " and (subject_id = 40 or subject_id = 1) ORDER BY s.subject_ord, s.subject_name"; 
	$query = mysql_query($sql);
	while ($row = mysql_fetch_assoc($query)) {
...
<html>
	...
	<body>
		...
		<script type="text/javascript">
			...
			var school_type_id = <?=$school_type_id;?>;
			...
			function update_weekly_shabbos_date_task(span, date_task_id, mark_date) {
				...
				if (check_all == false) {
					var parameters = [user_id, school_type_id, date_task_id, mark_date];
...
<? if ($report_school_id < 1) : ?>
			<form name="date_tasks_report" id="date_tasks_report" action="september_date_tasks_report.php" method="post" accept-charset="UTF-8">
		...
		<div class="marking module">
			...
			<div class="row tasks bonus">
					...
					<div class="days">	
						...
						<? else :?>
						<div class="cell text_input">
							<input type="text" onblur="update_mark(this, <?=$date_task_mark->date_task_id;?>, <?=$mark_date;?>)" onkeypress="return number_validation(event);" maxlength="3">
						</div>
						<? endif; ?>
					</div>
