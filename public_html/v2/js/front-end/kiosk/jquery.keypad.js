...
function Keypad() {
	...
	this.regional[''] = { // Default regional settings
		...
		closeStatus: '', // Status text for close link "Close the keypad"
		clearText: 'Clear', // Display text for clear link
		clearStatus: '', // Status text for clear link "Erase all the text"
		backText: 'Back', // Display text for back link
		backStatus: '', // Status text for back link "Erase the previous character"
	...
	this._defaults = { // Global defaults for all the keypad instances
		...
		layout: ['123','456','789',this.CLOSE + '0' + this.CLEAR],
		showAnim: 'fadeIn',
		keypadOnly: false,
		showOn: 'both',
		buttonImage: 'images/keypad_btn.jpg',
...
$.extend(Keypad.prototype, {
	...
	_showKeypad: function(field) {
		...
		if (inst._input[0].type != 'hidden') {
			inst._input[0].focus();
			inst._input[0].value = inst._input[0].value;
		}
