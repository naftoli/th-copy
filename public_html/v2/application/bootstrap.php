...
$link = mysql_connect('localhost', $global_db_user, $global_db_pass);
...
/*
 * Provide a date with any delimiter in month day year format
 * Retunrs UNIX_TIMESTAMP or NULL if the date is unparsable
 */
function parse_date_mdy($strDate) {
	if (!preg_match('/^ *([0-9a-z]+)[^0-9a-z]+([0-9a-z]+)[^0-9a-z]+([0-9a-z]+) *$/i', $strDate, $arrMatched)) {
		return NULL;
	}
	list ($strFull, $strMonth, $strDay, $strYear) = $arrMatched;
	if (!is_numeric($strMonth)) {
		if (is_string($strMonth)) {
			$arrMonths = array(
				"jan" => 1,
				"feb" => 2,
				"mar" => 3,
			...
			$strMonth = $arrMonths[strtolower(substr($strMonth, 0, 3))];
			if (!isset($strMonth))
				return NULL;
		}
		else
	...
	if (!is_numeric($strDay) || !is_numeric($strYear)) {
		return NULL;
	}
	if ($strYear < 100)
		$strYear += 2000;
	return mktime(0, 0, 0, $strMonth, $strDay, $strYear);
}
...
function datetime_to_timestamp($strDateTime)
{
	if (!preg_match('/^([0-9]{4})\-([0-9]{2})\-([0-9]{2}) ([0-9]{2}):([0-9]{2}):([0-9]{2})$/', $strDateTime, $arrMatched))
	{
		return FALSE;
	}
	list($strWhole, $strYear, $strMonth, $strDay, $strHour, $strMin, $strSec) = $arrMatched;
	$intTime = mktime($strHour, $strMin, $strSec, $strMonth, $strDay, $strYear);
	return $intTime;
}
function array_to_table($arrData, $strCss = "") {
	if (empty($arrData)) {
		print "<div>Empty table</div>";
		exit;
	}
	$strOut = "<table border='1'>";
	$strOut .= "<tr>";
	foreach (first($arrData) as $key => $row) {
		
		$strOut .= "<td style='white-space:nowrap;overflow:hidden;max-width:160px;$strCss'>" . $key . "</td>";
	}
	$strOut .= "</tr>";
	foreach ($arrData as $row) {
		...
		{
			$strOut .= "<td style='" . (empty($value) ? "background-color:#fff;color:#fff;" : "background-color:#eee;") . "$strCss'>" . (empty($value) ? "NULL" : $value) . "</td>";
		}
		$strOut .= "</tr>";
	}
	$strOut .= "</table>";
