...
if (isset($_POST['letter'])) {
    $where .= " and last like '" . mysql_real_escape_string($_POST['letter']) . "%'";
}

$sql = "select user_id, first, last, user_registered, user_serial, user_code, u.school_id, school_name 
        from users u 
...
<? } else { ?>
    <form action="soldiers.php" method="post">
        ...
        <p><!--(This is only needed when choosing "not registered".)<br />-->
        Please choose the letter that the last name starts with:
        <select name="letter">
            <?php
            $letters = range('A', 'Z');  
