...
if (preg_match('/MSIE/', $agent)) {
	echo "You cannot use our Kiosk in Internet Explorer.";
	exit;
}
...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<head>
		...
		<script src="kiosk/scripts/shadowbox/shadowbox.js" type="text/javascript"></script>
	...
	<body class="green" onload="check_browser();document.documentElement.requestFullscreen();">
...
<?php
	$strUrl = 'http://mashpia2.icorpa.com/legacy/userpointsextract2/bar_code/3' . $user_row['user_code'];
	ob_start();
	$objCurl = curl_init();
	...
	curl_close($objCurl);
	ob_end_clean();
	if (!preg_match("/Sorry, there was an error/", $strResult))
	{
		$arrResultPoints = preg_split("/\t/", $strResult);
		$cur_points += $arrResultPoints[0];
		$user_row['total_miles'] += intval(@$arrResultPoints[1]);
	}
	//var_dump($arrResultPoints);
...
-->					<? } ?>
					...
					<?php /*
						...
						{ */
					?>					
						<li><a href="http://mashpia2.icorpa.com/kiosk/auto-login/uc/3<?=$user_row['user_code']?>" class="icon_shop"><?=T_('Store')?></a></li>
					<?php 
						//}
					?>
