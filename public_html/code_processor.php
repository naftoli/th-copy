				...
				case '7': //tanya
					...
					switch($scan_code[1]) {
						...
						case '3': //tanya pledge
							...
							$s = "select value from maos_chitim_cards where number = " . $scan_code;
							$r = mysql_query($s);
							$row = mysql_fetch_assoc($r);
							$value = $row['value'];
							
							$s = "select sum(amount) as total from maos_chitim_student_pledges where user_id = " . $user['user_id'];
							$r = mysql_query($s);
							if ($pledge = mysql_fetch_assoc($r)) {
								return array('old_pledges' => $pledge['total'], 'new_pledge' => $value);
							} else {
								return array('old_pledges' => 0, 'new_pledge' => $value);
							} 							 
							break;
