...
$bp = BalPehCampaign::getInstance( $campaign_id );
$bps = new BpSummary( $campaign_id, 'school' );
...
foreach ($schoolsInfo as $id => $school) {
	...
	if ($registered) {
		...
		$schoolsInfo[$id]['pledged'] = $bp->getTotalPledged( 'school', $id );		
	} else {
...
<HTML>
            ...
            $( function() {
                ...
                $("#school").change( function() {
	                ...
	                /*
	                	...
	                	$("#grade").next("div.newListSelected").remove();
	                	$("#grade").empty();
	                	$("#grade").append("<option value='0'>Choose grade</option><option value='-1'>All</option>");
	                	$("#grade").sSelect();
	                }
	                */
                });
    ...
    <BODY>
        ...
        //if (isset($_POST['submit'])) {
			...
			//after 21 schools show new page
			$i = 0;
			...
			echo "<tr><th>School Name</th><th>Address</th>
				...
				<th><a href='new_tanya_report.php?sort=pledged'>Total Lines Commited</a><br /><span class='no-print'>(click to sort by)</span></th>
				<th><a href='new_tanya_report.php?sort=learned'>Total Lines Learned</a><br /><span class='no-print'>(click to sort by)</span></th>
			...
			foreach ($users as $id => $users) {
				...
				foreach ($users as $grade => $students) {
					...
					$cTotal = $rcRow['total'];
					echo $cTotal . "</td><td>";
					$cLines = $tanya->getTotalLearned('class', $gradeInfo[0]);
				...
				foreach ($users as $grade => $students) {
					...
					foreach ($students as $userID => $student) {
						...
						$numLines = $tanya->getTotalLearned('user', $userID);
		...
		//} else {
			?>
			<!--
			<form action="new_tanya_report.php" method="post">
				<fieldset>
					<legend>Options</legend>
					<? if (count($schools) > 1) { ?>
					<select name="school" id="school" class="sSelect">
					...
					<select name="grade" id="grade" class="sSelect">
						<option value="0">Choose class</option>
						<option value="-1">All</option>
					</select><br /><br />
					
					<select name="user" id="user" class="sSelect">
