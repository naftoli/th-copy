    ...
    <BODY>
        ...
        foreach ( $schoolsUsers as $school => $users ) {
            ...
            foreach ( $users as $user ) {
            	$address = $user['user_address1'] . "<br />";
				if (!empty($user['user_address2'])) $address .= $user['user_address2'];
				$address .= $user['user_city'] . ', ' . $user['user_state'] . ' ' . $user['user_postal'];
                echo "<tr><td>" . $user['class_grade'] . ( empty( $user['class_sub']) ? '' : "-" . $user['class_sub'] ) . 
                    ...
                    $user['dob'] . "</td><td>" . $user['dob_he'] . "</td><td>" . $address . "</td></tr>";
            }
            echo "</table><br />";
        }
        ?>
