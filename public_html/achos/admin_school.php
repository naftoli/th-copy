...
if (!empty($action)) switch($action) {
  ...
  case 'edit':
    $result = mq("SELECT school_id, school_name, school_name_he, school_makeup_id, inst_id, school_settings, school_gender, school_logo_id, school_logo_kiosk_id, school_no_logo, school_file_id, school_address1, school_address2, school_city, school_state, school_postal, school_country, school_phone, cc_number, cc_exp, cc_cvv, kiosk_print, shipping_method, shipping_first, shipping_last, shipping_phone, shipping_address1, shipping_address2, shipping_city, shipping_state, shipping_postal, shipping_country, school_store, notes FROM schools WHERE school_id = $school_id");
    $edit_row = mysql_fetch_assoc($result);
    break;
						...
						<P><?=mysql_result(mq("SELECT message_text FROM messages WHERE message_type = 'base_mission'"), 0);?></P>
							...
							<FORM action="admin_school.php" method="post" accept-charset="UTF-8" enctype="multipart/form-data">
								<P class="rows">
									...
									<LABEL><?=T_('Hebrew Name')?><BR><INPUT type="text" name="name_he" value="<?=es($edit_row['school_name_he'])?>"> (<?=T_('This is how it will appear on school banner')?>)</LABEL><BR>
