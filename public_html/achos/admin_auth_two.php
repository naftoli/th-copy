...
//echo "<input type='hidden' name='COOKIES' value='" . var_dump($_COOKIE) . "'>\n";
...
if (!function_exists('check_auth_admin')) 
{
	...
	echo "<input type='hidden' name='check_auth_admin exists' value='check_auth_admin exists'>\n";

	function check_auth_admin($admin_id, $auth) 
	{
		echo "<input type='hidden' name='ADMIN ID' value='" . $admin_id . "'>\n";
		...
		{
			$sql = "SELECT username, password, auth, first, last, lang, camp_id, is_parent FROM admins WHERE admin_id=" . $admin_id;
			...
			if ($row = mysql_fetch_assoc($result)) {
				...
				if ($auth == hash_hmac('ripemd128', strtolower($row['username']).$row['password'], '53fdc95857aac68970159dd07e7c3782')) {
					...
					foreach($admin_auth as $type) {
						...
						if (count($admin_user['auths'][$type])) {
							$allow = true;
...
if (!function_exists('check_login_admin')) 
{
	...
	function check_login_admin() 
	{
		...
		if (isset($_POST['new_login']) && isset($_POST['login_username']) && isset($_POST['login_password'])) 
		{
			...
			{
				echo "<input type='hidden' name='3) function_exists' value='3) function_exists'>\n";
		...
		{
			echo "<input type='hidden' name='4) function_exists' value='4) function_exists'>\n";
...
if (!isset($no_login)) 
{
	...
	{  
		echo "<input type='hidden' name='LOGGING OUT' value='LOGGING OUT'>\n";
		include('login.php');
		exit;
	}
}
