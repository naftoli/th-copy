...
class MissionsDone {
    ...
    public function setMissionsDone( $missions = array(), $children = array() ) {
    	if ( empty( $children ) ) { 
	        $sql = "select c.class_grade, c.class_sub, u.user_id, u.first, u.last, su.subject_id, su.subject_name, count( mm.date_tasks_mission_id ) as total  
	        ...
	        while ( $row = mysql_fetch_assoc( $result ) ) {
	            ...
	            $this->missionsDone[$class][$userName][$row['subject_name']] = $row['total'];
				$this->userIDs[$userID][] = $userName . ":" . $row['subject_id'] . ":" . $row['total'];
	        }
	    } else {
	    	$sql = "select u.user_id, u.first, u.last, su.subject_id, su.subject_name, count( mm.date_tasks_mission_id ) as total  
	                from date_tasks_mission_marks mm 
	                join users u using (user_id) 
	                join subjects su using (subject_id) 
	        ...
	        while ( $row = mysql_fetch_assoc( $result ) ) {
	        	$userID = $row['user_id']; 
	            $userName = $row['first'] . " " . $row['last'];
	            $this->missionsDone[$userName][$row['subject_name']] = $row['total'];
				$this->userIDs[$userID][] = $userName . ":" . $row['subject_id'] . ":" . $row['total'];
	        }
	    }
    }
