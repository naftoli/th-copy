...
class upm_subject_user {
	...
	function get_incomplete_missions($subject_id, $start_date, $end_date)
	{
		...
		//get all missions for full year of ayin beis in order to project onto ayin gimmel
		//dates for ayin beis
		$start = 2456158;
		$end = 	2456512;
		*/
		...
		$sql = $sql . "WHERE ut.user_id=" . $this->user_id . " ";
		$sql = $sql . "AND ut.subject_id=" . $subject_id . " ";
		$sql = $sql . "AND dtm.start_date >" . $start_date . " ";
		$sql = $sql . "AND dtm.end_date <" . $end_date . " ";
		$sql = $sql . "GROUP BY ut.user_id, ut.subject_id  ";
		//echo "Second: " . $sql . "<br />";
		$query = mysql_query($sql);
		$row = mysql_fetch_assoc($query);	
		...
		if ($row['incomplete_missions'] > 0)
			...
			$this->incomplete_missions = 0;
		//echo "Incomplete missions: " . $this->incomplete_missions . "<br />";		
	}
