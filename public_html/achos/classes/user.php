...
class user {
	...
	function get_rank() {
		$sql = "SELECT r.rank_name, r.rank_image_id, date_promoted FROM rank_marks JOIN ranks AS r USING(rank_ord) WHERE user_id=" . $this->user_id . " ORDER BY rank_ord DESC LIMIT 1";
		$query = mysql_query($sql);
		$row = mysql_fetch_assoc($query);
		if ($row) {
	...
	function get_user_tracks($subject_id, $start_date, $end_date) {				
		...
		for ($utno = 0; $utno < count($this->user_tracks); $utno++) 
		{
			...
			//////////echo "<input type='hidden' name='# OF DAILY TASKS' value='" .  count($user_track->daily_tasks) . "'>\n";	
