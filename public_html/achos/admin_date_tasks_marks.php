...
if (!$end_date || ($end_date > unixtojd() && !(isset($admin_user) && $admin_user['auth'] == 'super'))) 
	$end_date = unixtojd();
...
$start_date_gregorian = jdtogregorian($start_date);
$end_date_gregorian = jdtogregorian($end_date);
echo "<input type='hidden' name='start_date' value='" . $start_date_gregorian . "'>\n";
echo "<input type='hidden' name='end_date' value='" . $end_date_gregorian . "'>";

$user_code = gri('user_code', 1);
$class_id = gri('class_id', -1);
$user_id = gri('user_id', -1);
$subject_id = gri('subject_id', -1); //-2 means no (including all) subject selected, so don't show tasks
...
if (!gr('no_save')) {
	...
	if (($missions = gra('missions')) && $save_user_id != -1) {
		foreach($missions as $date_tasks_mission_id => $data) {
			...
			$mission_done = mysql_fetch_assoc(mq("SELECT date_tasks_mission_id, NOT EXISTS (SELECT * FROM date_tasks LEFT JOIN date_tasks_marks ON (date_tasks.date_task_id = date_tasks_marks.date_task_id AND date_tasks_marks.user_id = $save_user_id) WHERE date_tasks.date_tasks_mission_id = date_tasks_missions.date_tasks_mission_id AND (IFNULL(done_qty, 0) < mandatory_qty or (quantity is not null and ifnull(mark_quantity, 0) < quantity*mandatory_qty))) done FROM date_tasks_missions WHERE  date_tasks_mission_id = $date_tasks_mission_id"));
...
<html dir="<?=$dir?>">
	<head>
		<title><?=T_('Marking Missions'), ' - ', T_('Tzivos Hashem Management System')?></title>
	...
	*/
	...
	<body>
		...
		<script type="text/javascript" src="scripts/jquery.styleselect.js">
		</script>
		...
		<script type="text/javascript">
			$(function(){
				...
				$('.marking_list .check_all a').toggle(function(){
					$('.cell.checkbox').addClass('checked').find(':checkbox').attr('checked','checked');
					$(this).text('<?=T_('Uncheck All')?>');
				},function(){
					$('.cell.checkbox').removeClass('checked').find(':checkbox').removeAttr('checked');
					$(this).text('<?=T_('Check All')?>');
				});
...
<? if ($admin_user['auth'] == 'super' || $auth_mode == 'school' && count($admin_user['auths']['school']) != 1) : ?>
	<? $school_result = mq('SELECT school_id, school_name, inst_name FROM schools JOIN institutions USING (inst_id)' . ($admin_user['auth'] != 'super' ? ' WHERE school_id IN (' . implode(',', $admin_user['auths']['school']) . ')' : '') . ' ORDER BY inst_name, school_name'); ?>
			...
			<form action="admin_date_tasks_marks.php" method="get" accept-charset="UTF-8">
...
<? else:?>
	...
	<? if($admin_user['auth'] == 'super' || $auth_mode == 'school'):?>
					<div class="class_list select_box">
						...
						<a class="next button">
							<span class="icon"></span><span class="label"><?=T_('Next Platoon')?></span>
						</a>
	...
	<? if ($admin_user['auth'] == 'super' || $auth_mode == 'school') : ?>
		...
		<? $sql = "SELECT users.user_id, first, last, username, class_grade, class_sub FROM users LEFT JOIN classes USING (class_id) WHERE users.school_id = $school_id" . ($class_id >= 0 ? " AND users.class_id = $class_id" : '') . ' AND user_registered IS NOT NULL ORDER BY class_grade, class_sub, last, first, username'; ?>
		...
		<? $users = mq("SELECT users.user_id, first, last, username, class_grade, class_sub FROM users LEFT JOIN classes USING (class_id) WHERE users.school_id = $school_id" . ($class_id >= 0 ? " AND users.class_id = $class_id" : '') . ' AND user_registered IS NOT NULL ORDER BY class_grade, class_sub, last, first, username'); ?>
					<div class="user_list select_box">
						...
						<select name="user_id" class="sSelect">
							...
							<option name="user_id" value="<?=$row['user_id']?>" <?=$row['user_id'] == $user_id ? 'selected="selected"' : ''?>>
								<?=$row['class_grade'] != '' ? es($row['class_grade'] . '-' . $row['class_sub']) . ': ' : ''?><?=ucwords(strtolower(es($row['last'])))?>, <?=ucwords(strtolower(es($row['first'])))?>
							</option>
							<? endwhile; ?>
						</select>
	...
	<? endif; ?>
					...
					<div class="date_list select_box">
						...
						<a class="prev button">
							<span class="icon"></span><span class="label"><?=T_('Previous Week')?></span>
						</a>
						
						<select name="date_list" class="sSelect">
							<? $result = mq("SELECT start_date, end_date, report_name FROM (SELECT start_date, end_date, report_name FROM reports WHERE report_type = 'mission_cover_sheet' AND visibility != 'none' AND start_date < $start_date order by start_date desc) s order by start_date"); ?>
							<? while($row = mysql_fetch_assoc($result)): ?>
							<option value="admin_date_tasks_marks.php?school_id=<?=$school_id?>&amp;class_id=<?=$class_id?>&amp;user_id=<?=$user_id?>&amp;start_date=<?=$row['start_date']?>&amp;end_date=<?=$row['end_date']?>"><?=es($row['report_name'])?> - <?=es(dateToHebrew($row['start_date']))?></option>
							<? endwhile; ?>								
							...
							<option value="0" selected="selected"><?=es($row['report_name'])?> - <?=es(dateToHebrew($start_date))?></option>							
							<? $result = mq("SELECT start_date, end_date, report_name FROM reports WHERE report_type = 'mission_cover_sheet' AND visibility != 'none' AND start_date > $start_date ORDER BY start_date"); ?>
							<? while($row = mysql_fetch_assoc($result)): ?>
							<option value="admin_date_tasks_marks.php?school_id=<?=$school_id?>&amp;class_id=<?=$class_id?>&amp;user_id=<?=$user_id?>&amp;start_date=<?=$row['start_date']?>&amp;end_date=<?=$row['end_date']?>"><?=es($row['report_name'])?> - <?=es(dateToHebrew($row['start_date']))?></option>
							<? endwhile; ?>
						</select>
					...
					<div class="campaign_list select_box">
						...
						<select name="subject_id" id="subject_id">
							...
							<option value="<?=$row['subject_id']?>" <?=$subject_id == $row['subject_id'] ? 'selected="selected"' : '' ?>><?=es($row['subject_name'])?></option>
							<? endwhile; ?>
						</select>
					...
					<input class="submit" type="submit" id="display_submit" value="<?=T_('Go')?>">
						
					<div class="select_box check_all clearfix">
						<a class="button"><span class="icon"></span><?=T_('Check All')?></a>
					</div>
	...
	<? if ($subject_id != -2) : ?>
			...
			<form action="admin_date_tasks_marks.php" method="post" accept-charset="UTF-8" name="edit">
			
				<h2><!--<?=T_('Report For')?>: (<?=es($user_row['username'])?>)--> <?=ucwords(strtolower(es($user_row['first'])))?> <?=ucwords(strtolower(es($user_row['last'])))?><!-- - <?=es($user_row['first_he'])?> <?=es($user_row['last_he'])?>--><a class="edit" href="admin_user_track.php?action=edit&amp;user_id=<?=$user_row['user_id']?>&amp;school_id=<?=$school_id?>&amp;class_id=<?=$class_id?>&amp;back=<?=urlencode("admin_date_tasks_marks.php?subject_id=$subject_id&start_date=$start_date&edit_date=$end_date&")?>&amp;back_text=<?=urlencode(T_('Back to Marking Missions'))?>">Edit</a></h2>
				
				<!--<a href="admin_user_track.php?action=edit&amp;user_id=<?=$user_row['user_id']?>&amp;school_id=<?=$school_id?>&amp;class_id=<?=$class_id?>&amp;back=<?=urlencode("admin_date_tasks_marks.php?subject_id=$subject_id&start_date=$start_date&edit_date=$end_date&")?>&amp;back_text=<?=urlencode(T_('Back to Marking Missions'))?>"><?=T_("Edit Soldier's ladders/years/campaigns")?></a>-->
		...
		<? if ($user_row['class_grade'] != '') : ?>
			<?=es($user_row['class_grade'])?>-<?=es($user_row['class_sub'])?>
		<?endif;?><br>
		...
		<? while($row = mysql_fetch_assoc($result)): ?>		
				<a href="admin_date_tasks_marks.php?school_id=<?=$school_id?>&amp;class_id=<?=$class_id?>&amp;user_id=<?=$user_id?>&amp;start_date=<?=$row['start_date']?>&amp;end_date=<?=$row['end_date']?>" style="white-space: nowrap;" dir="RTL"><?=es($row['report_name'])?> <?=es(dateToHebrew($row['start_date']))?></a><br>
		<? endwhile; ?>
...
<?
$SQL_1 = "SELECT subject_name, subject_image_id, inst_name, date_tasks_missions.mission_name, mission_number, subject_id, start_date, end_date, date_tasks_mission_id, track_id, track_name, level, school_type_id, school_type_name, date_tasks_mission_marks.date_tasks_mission_id done, mark_override,
...
?>
				...
				<input type="hidden" name="SQL_1" value="<?=$SQL_1;?>">
...
<? $row = (($alt_mission = mysql_fetch_assoc($alt_missions)) ? $alt_mission : $mission_row); ?>
<? $alt_mission = false; $alt_missions = mq('select 1 from dual where 0=1'); ?>				
...
<? 
$dt_sql = "SELECT date_tasks.date_task_id, name, mandatory_qty, optional_qty, quantity, points, done_qty, mark_quantity, label_name, label_image_id ";
$dt_sql = $dt_sql . "FROM date_tasks ";
$dt_sql = $dt_sql. "LEFT JOIN labels USING (label_id) ";
$dt_sql = $dt_sql . "LEFT JOIN date_tasks_marks ON (date_tasks.date_task_id=date_tasks_marks.date_task_id AND user_id=" . $user_row['user_id'] . ") ";
$dt_sql = $dt_sql . "WHERE date_tasks_mission_id=" . $row['date_tasks_mission_id'] . " AND mandatory_qty > 0 ORDER BY ord"; 
?>
<!--<input type="hidden" name="SQL_2" value="<?//=$dt_sql;?>">-->
...
<? $task_ids = array(); ?>
		...
		<? while($task = mysql_fetch_assoc($tasks)) : ?>
			...
			<? if (is_null($task['quantity'])):?>
				...
				<? for ($i = 0; $i < $task['mandatory_qty']; $i++) : ?>
					...
					<div class="row tasks mission<? if (mysql_num_rows($alt_missions)) : ?> alt_mission<? endif; ?>">
					
						<div class="days">
										
							<div class="cell checkbox<?=$task['done_qty'] > $i ? ' checked' : ''?>">
								<span>
									<input type="checkbox" name="missions[<?=$row['date_tasks_mission_id']?>][task_ids][<?=$task['date_task_id']?>][done][]" id="missions_<?=$row['date_tasks_mission_id']?>_<?=$task['date_task_id']?>_<?=$i?>" value="1" <?=$task['done_qty'] > $i ? 'checked="checked"' : ''?>> <?=sprintf(($task['points'] == 1 ? T_('%s mile') : T_('%s miles')),  floatval($task['points']))?>
								</span>							
							</div>
						...
						<div class="campaign_logo">
							<?=!is_null($row['subject_image_id']) ? linkImgFile($row['subject_image_id']) : ''?>
						</div>

						<!--<div class="task_image"><?=!is_null($task['label_image_id']) ? linkImgFile($task['label_image_id']) : ''?></div>-->
					...
					<div class="row tasks mission<? if(mysql_num_rows($alt_missions)):?> alt_mission<? endif;?>">
						...
						<div class="days">
							...
							<div class="cell text_input">
								<span><input type="text" name="missions[<?=$row['date_tasks_mission_id']?>][task_ids][<?=$task['date_task_id']?>][quantity]" id="missions_<?=$row['date_tasks_mission_id']?>_<?=$task['date_task_id']?>" size="5" maxlength="5" value="<?=$task['mark_quantity']?>" onChange="this.value = Math.max(0, Math.min(parseInt('0'+this.value, 10), 65535));"><span class="label"> (<?=T_('Mission Req')?>: <?=$task['quantity']*$task['mandatory_qty']?>)<?=sprintf(($task['points'] == 1 ? T_('Quota: %s mile') : T_('Quota: %s miles')),  floatval($task['points']))?>, <? $each = @round($task['points']/($task['quantity']*$task['mandatory_qty']), 2); ?> <?=sprintf(($each == 1 ? T_('%s mile each') : T_('%s miles each')), $each)?></span></span>
							</div>
						...
						<div class="campaign_logo"><?=!is_null($row['subject_image_id']) ? linkImgFile($row['subject_image_id']) : ''?></div>
						<!--<div class="task_image"><?=!is_null($task['label_image_id']) ? linkImgFile($task['label_image_id']) : ''?></div>-->
						
						<div class="cell">
							<?=es($task['name'])?>
							<div class="small_info">
								<?=T_('Mission Requirement')?>: <?=$task['quantity']*$task['mandatory_qty']?>
							</div>
						</div>
		...
		<? if (mysql_num_rows($bonus_tasks)) : ?>
			...
			<? while($task = mysql_fetch_assoc($bonus_tasks)):?>
				...
				<? if (is_null($task['quantity'])) : ?>
					...
					<div class="row tasks bonus<? if(mysql_num_rows($alt_missions)):?> alt_mission<? endif;?>">
					
						<div class="days">
						
							<div class="cell checkbox<?=$task['done_qty'] > $i ? ' checked' : ''?>">
								<span><input type="checkbox" name="missions[<?=$row['date_tasks_mission_id']?>][task_ids][<?=$task['date_task_id']?>][done][]" id="missions_b_<?=$row['date_tasks_mission_id']?>_<?=$task['date_task_id']?>_<?=$i?>" value="1" <?=$task['done_qty'] > $i ? 'checked="checked"' : ''?>> <?=sprintf(($task['points'] == 1 ? T_('%s mile') : T_('%s miles')),  floatval($task['points']))?></span>
							</div>
				...
				<? else:?>
					...
					<div class="row tasks mission<? if(mysql_num_rows($alt_missions)):?> alt_mission<? endif;?>">
						...
						<div class="days">
							...
							<div class="cell text_input">
								<span><input type="text" name="missions[<?=$row['date_tasks_mission_id']?>][task_ids][<?=$task['date_task_id']?>][quantity]" id="missions_b_<?=$row['date_tasks_mission_id']?>_<?=$task['date_task_id']?>" maxlength="5" value="<?=$task['mark_quantity']?>" onChange="this.value = Math.max(0, Math.min(parseInt('0'+this.value, 10), 65535));"><span class="label"> <?=sprintf(($task['points'] == 1 ? T_('Quota: %s mile') : T_('Quota: %s miles')),  floatval($task['points']))?>, <? $each = @round($task['points']/($task['quantity']*$task['mandatory_qty']), 2); ?> <?=sprintf(($each == 1 ? T_('%s mile each') : T_('%s miles each')), $each)?></span></span>
							</div>
		...
		<!-- if (mysql_num_rows($bonus_tasks)) : -->
				...
				<p>
				<?=sprintf(T_('There are no missions for this soldier, this is because of the following reasons: The soldiers has not enrolled to this campaign, or they are not on a ladder, or they are not in a year, or they have not yet registered. %sClick Here%s to modify their ladder.'),"<a href='admin_user_track.php?action=edit&amp;user_id={$user_row['user_id']}&amp;school_id=$school_id&amp;class_id=$class_id'>", '</a>')?><br>
				<br>
				<input type="submit" value="<?=T_('Edit next soldier')?>">
				</p>
