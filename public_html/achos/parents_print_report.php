...
if (isset($_POST['action']) || count($admin->children) == 1) {
	...
	if ($action == "produce_report" || count($admin->children) == 1) {
		...
		if (isset($_POST['date_list'])) {		
			...
			$sql = "SELECT * FROM reports WHERE report_type='mission_cover_sheet' AND visibility != 'none' AND start_date >= " . ($start + 7) . " ORDER BY start_date limit 1";	
...
<? if (count($admin->children == 1)) { ?>
	<p align='center' class='no-print'>Please Note: You may have to actively click on the weekly drop-down and select a Parsha 
				...
				<div class="infobox2 marking_list clearfix">
					...
					<div class="child_list select_box">
						...
						<SELECT name="child_id" id="child_id">
							...
							<? //if ($admin->children[$cno]->user_registered != '') : ?>
