...
class medal_updater {
	...
	/*
	function update_medal($user_id) {
		$updated = false;
		
		// ***** Get the number of finished missions ***** //
		$sql1 = "SELECT *, s.subject_name, count(*) AS finished_missions ";
		$sql1 = $sql1 . "FROM users AS u ";
		$sql1 = $sql1 . "JOIN user_tracks AS ut USING (user_id) ";
		$sql1 = $sql1 . "JOIN date_tasks_mission_marks AS dtmm ON (u.user_id=dtmm.user_id AND ut.subject_id=dtmm.subject_id)  ";
		$sql1 = $sql1 . "JOIN subjects AS s ON (ut.subject_id=s.subject_id) ";
		$sql1 = $sql1 . "JOIN date_tasks_missions AS dtm ON ( dtm.date_tasks_mission_id = dtmm.date_tasks_mission_id
						AND dtm.level = ut.level
						AND dtm.track_id = ut.track_id ) ";
		$sql1 = $sql1 . "WHERE u.school_id > 0 ";
		$sql1 = $sql1 . "AND u.school_type_id > 0 ";
		$sql1 = $sql1 . "AND ut.enrolled=1 ";
		if ($user_id > 0)
			$sql1 = $sql1 . "AND u.user_id=" . $user_id . " ";
		$sql1 = $sql1 . "GROUP BY u.user_id, ut.subject_id ";
		//echo $sql1;
			...
			while ($row2 = mysql_fetch_assoc($query2)) {
				...
				if ($finished_missions >= $missions_required) {
					...
					if ($row3['number_of_medals'] == 0) {
						...
						$insert_sql = "INSERT INTO medal_marks SET medal_ord=" . $row2['medal_ord'] . ", subject_id=" . $row1['subject_id'] . ", user_id=" . $row1['user_id'] . ", date_awarded=" . $today_julian;
	...
	*/
	...
	function update_medal_two($user_id) {
		...
		if ($user_id > 0)
			$sql1 = $sql1 . "AND u.user_id=" . $user_id . " ";
		$sql1 = $sql1 . "GROUP BY ut.subject_id ";
		//if ($user_id == 51629) echo $sql1 . "<br />";
			...
			while ($row2 = mysql_fetch_assoc($query2)) {
				...
				if ($finished_missions >= $missions_required) {
					...
					if ($row3['number_of_medals'] == 0) {
						...
						$insert_sql = $insert_sql . "subject_id=" . $row1['subject_id'] . ", ";
						$insert_sql = $insert_sql . "user_id=" . $row1['user_id'] . ", ";
