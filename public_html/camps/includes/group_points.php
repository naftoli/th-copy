	...
	<script>
		...
		$(function() {
			$('form a.submit').click(function(e){
				e.preventDefault();
				var selected_index = $("#group_type_id option:selected").text;
				$("#group_type_name").value = selected_index;
				//var selected_index = document.getElementById("group_type_id").selectedIndex;
				//var text = document.getElementById("group_type_id").options[selected_index].text;
				//document.getElementById("group_type_name").value = text;
				var url = 'content.php?output=group_marking&' + $('#points_form').serialize();
				$(this).attr('href',url);
				slideForward(this);
			});
	...
	<div class="slider">
		...
		<div class="col_content">
			...
			<form action="content.php?output=marking" name="cms_presentation" id="points_form">
				...
				<input type="hidden" name="mission_no" id="mission_no" value="0">
