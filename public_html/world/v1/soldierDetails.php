...
$users = array();
$sql = "select user_id, first, last from users where class_id = " . $class_id . " and (user_registered > 0 or yan = 1)";
$result = mysql_query( $sql );
while ($row = mysql_fetch_assoc( $result )) {
...
foreach ($campaigns as $id => $campaign) {
	...
	foreach ($users as $user_id => $user) {
		...
		$results[$campaign]['learned'][$user_id] = $learned;
		//$grandTotal[$campaign]['pledged'] += $results[$campaign]['pledged'][$school_id];
		$grandTotal[$campaign]['learned'] += $results[$campaign]['learned'][$user_id];
	}
}
?>
<!DOCTYPE html>
	       		...
	       		<div class="table-responsive">       
			        <table class="table table-striped table-bordered table-hover sortable">
				    	...
				    	foreach ($users as $user_id => $user) {
							...
							$data[$user]['tanyaL']  = isset($results['tanya']['learned'][$user_id]) ? $results['tanya']['learned'][$user_id] : 0;
							//$data[$school]['mishnaP'] = isset($results['mishna']['pledged'][$id]) ? $results['mishna']['pledged'][$id] : 0;
							$data[$user]['mishnaL'] = isset($results['mishna']['learned'][$user_id]) ? $results['mishna']['learned'][$user_id] : 0;
				    	}
