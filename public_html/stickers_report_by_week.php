        ...
        if ( isset( $_POST['submit'] ) ) {
			...
			if ($_POST['options'] == 'school') {		
	            foreach ( $userMissions as $school => $users ) { 
	                ...
	                echo "<p>This report is from the beginning of " . $start_report . " to the end of " . $end_report . ".</p>";
	                echo "<table>";
	                    ...
	                    foreach ( $user as $name => $mission ) {
	                        ...
	                        foreach ( $missionsPosted as $m ) {
	                            if ( !isset( $mission[$missions[$m]] ) ) {
									...
									//find user id
									$userID = null;
									foreach( $userIDs[$school] as $id => $info ) {
										foreach ( $info as $userInfo ) {
											if ( $userInfo == $name . ":" . $m . ":" . $missionsDone ) {
												$userID = $id;
												break;
											}
										}
									}
	                                echo  "<td>" . $sticker[$k] . "</td>"; 
	                            }
	                        }
	                        echo "</tr>";
	                    }
	        ...
	        } else if ($_POST['options'] == 'class') {
	        	foreach ( $userMissions as $school => $users ) { 
	                foreach ( $users as $class => $user ) {
		                echo "<h2>" . $schools[$school] . " Grade: " . $class . "</h2>";
		                echo "<p>This report is from the beginning of " . $start_report . " to the end of " . $end_report . ".</p>";
		                echo "<table>";
            ...
            ?>
            <form action="stickers_report_by_week.php" method="post">
	        	...
	        	<fieldset>
	        		...
	        		<input type='radio' name='options' value='school' checked="checked" /> Show report by school<br />
	        		<input type='radio' name='options' value='class' /> Show report by class
	        	</fieldset>
