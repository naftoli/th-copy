...
if (!empty($action)) switch($action) {
  ...
  case 'edit':
    $result = mq("SELECT school_id, school_name, school_name_he, school_makeup_id, inst_id, school_settings, school_gender, school_logo_id, school_logo_kiosk_id, school_no_logo, school_file_id, school_address1, school_address2, school_city, school_state, school_postal, school_country, school_phone, cc_number, cc_exp, cc_cvv, kiosk_print, shipping_method, shipping_first, shipping_last, shipping_phone, shipping_address1, shipping_address2, shipping_city, shipping_state, shipping_postal, shipping_country, school_store, notes, shipping_requests FROM schools WHERE school_id = $school_id");
    $edit_row = mysql_fetch_assoc($result);
    break;
				...
				<DIV class="ui_body">
					...
					<DIV class="content">
						<P><?=mysql_result(mq("SELECT message_text FROM messages WHERE message_type = 'base_mission'"), 0);?></P>
							...
							<?if($edit_row):?>
							<?if($admin_user['auth'] == 'super' || count($admin_user['auths']['school']) != 1):?><A HREF="admin_school.php"><?=T_('Cancel')?></A><?endif;?>
							
							<FORM action="admin_school.php" method="post" accept-charset="UTF-8" enctype="multipart/form-data">
								<P class="rows">
									...
									<LABEL>
											...
											<?
												$sql = "select child_type_id from school_child_types where school_id = $school_id";
												$result = mq($sql);
											...
											(Changing your mission type will give you access to more or less campaigns.)
									</LABEL><BR>
									<? } ?>
									<div class="input_group input_half">
                                    ...
                                    <? if ( !$h_school && false ) { ?>
									<h2><?=T_('File Upload')?></h2>
									<LABEL><?=T_('File')?> - <?=T_('Use this to upload a database of your students for us to import, or to send us other files.')?><BR>
									...
									if ($admin_user['auth'] == 'super') 
										...
										$type = 'password';
									?>
									<LABEL><?=T_('CC Number')?><BR><INPUT TYPE="<?=$type?>" NAME="cc_number" VALUE="<?=es($edit_row['cc_number'])?>" MAXLENGTH="19"></LABEL><BR>
									<LABEL><?=T_('Expires MM/YY')?><BR><INPUT TYPE="<?=$type?>" NAME="cc_exp" VALUE="<?=es($edit_row['cc_exp'])?>" MAXLENGTH="5"></LABEL><BR>
									<LABEL><?=T_('CVV')?><BR><INPUT TYPE="<?=$type?>" NAME="cc_cvv" VALUE="<?=es($edit_row['cc_cvv'])?>" MAXLENGTH="4"></LABEL><BR>
							...
							<form action="admin_school.php" method="get">
