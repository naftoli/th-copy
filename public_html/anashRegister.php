...
require 'db.php';
$grades = array();
$sql = "select * from classes where class_era = 0 and school_id = 269";
...
<html>
	...
	<div id="info">
		...
		<form method="post" enctype="multipart/form-data" action="https://www.mashpia.com/anashRegisterPost.php" accept-charset="UTF-8" name="form" id='form' onsubmit="return checkChildren();"> 								
			...
			<fieldset>
				...
				<div id="children"></div>
				Number of children <span id="regText">registering</span>: 
			...
			<fieldset>
				...
				<input type="radio" name="shipping" class="shipping" value="1" /> Option 1: shipping of magazines, medals and rank books<br />
				<input type="radio" name="shipping" class="shipping" value="2" /> Option 2: shipping of medals and rank books, pick up magazines from our office<br />
				<small>When selecting an option the price shown on the shipping timetable will be added to your total amount to be charged.</small>
				<p>
					Destination<br />
					<input type="radio" name="shipDest" class="shipDest" value="1" /> USA<br />
					<input type="radio" name="shipDest" class="shipDest" value="2" /> Canada<br />
					<input type="radio" name="shipDest" class="shipDest" value="3" /> International<br />
					<small>Please note: the charge for shipping will be taken on a later date.</small>
				</p>
				<p><a href="http://www.myshliach.com/templates/articlecco_cdo/aid/2922463/jewish/Shipping-Timetable.htm">View Shipping Rates</a></p>
			</fieldset>
	...
	<script>
		...
		$(document).on('change', '#numChildren', function() {
			...
			for (var i = 1; i <= val; i++) {
				...
				str += "<tr><td>First Name in HEBREW LETTERS</td><td><input type='text' name='fnameh[]' class='fnameh' /></td></tr>";
				str += "<tr><td>Last Name in HEBREW LETTERS</td><td><input type='text' name='lnameh[]' class='lnameh' /></td></tr>";
				...
				if (success == 0) {
					...
					$.post('ajax/getChildren.php', {admin : id, year : year}, function(success) {
						if (success) {
							...
							str += "<input type='hidden' id='admin_id' name='admin_id' value='" + id + "' /><br />";
							$("#children").append(str);
							$("#regText").text('adding');
						}
					});
		...
		$(document).on('change', '.addChildren', function() {
			...
			var num = $(this).val();
			if (num > 0) {
				var str = "<div id='childrenAdded'><br />";
				for (var i = 1; i <= num; i++) {
					str += "<fieldset class='child'><legend>Child #" + i + "</legend>";
		...
		$(document).on('click', '.shipDest', function() {
			calculateTotal();
		});
		
		$(document).on('click', '#children input', function() {
			calculateTotal();
		});
		...
		function calculateTotal() {
			...
			//find out shipping charge and add to total
			var selection = 0;
			var dest = 0;
			...
			$(".shipDest").each( function() {
				if ($(this).is(":checked")) {
					dest = parseInt($(this).val());
				}
			});
			var shipping = {
				1 : {
			...
			if (selection && dest) {
				var num;
				if ($(".addChildren").length)
					num = parseInt($(".addChildren").val());
				else {
					num = parseInt($("#numChildren").val());
					if ($(".regPay").length) {
						$(".regPay").each( function() {
							if ($(this).is(":checked")) {
								num++;
				...
				//alert(num);				
				total += shipping[selection][dest][num];
			}
		...
		function checkChildren() {
			...
			if (good) {
				...
				if (!shipDestSel) {
					alert('You must choose a shipping destination.');
					return false;
				}
			} 
			return good;
