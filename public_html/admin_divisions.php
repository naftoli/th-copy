...
if ($action != '') {
	...
	switch($action) {
		case 'add':
			$sql = "INSERT INTO divisions SET group_type_id=" . $group_type_id .  ", division_name=" . ms(gr('division_name')) . ", groups=" . $groups;
			mq($sql);			
			
			if ($groups == 0) {
				$division_id = mysql_insert_id();
				
				$sql = "INSERT INTO groups SET division_id=" . $division_id . ", group_name=''";
				mq($sql);
			}
		...
		case 'save':
			$sql = "UPDATE divisions SET division_name=" . ms(gr('division_name')) . ", groups=" . $groups . " WHERE group_type_id=" . $group_type_id;
			mq($sql);
			$action = "";
		break;
		...
		case 'delete':
			...
			$sql = "DELETE FROM groups WHERE division_id=" . $division_id;
			mq($sql);
...
<? if ($action == "") : ?>	
	<body onload="get_groups('123', 'divisions_form');">
<? else : ?>
	<body>
<? endif; ?>		
...
<? if ($action == 'add_new' || $action == "edit") : ?>
	...
	<form name="divisions_form" id="divisions_form" action="admin_divisions.php" method="post" accept-charset="UTF-8">
		...
		<? $row = mysql_fetch_assoc(mq("SELECT group_type_name FROM group_types WHERE group_type_id=" . $group_type_id)); ?>
		...
		<table>
			<tr><td><?=T_('Division Name');?>:<input type="text" size="100" maxlength="255" name="division_name" id="division_name" value="<?=$division_name;?>"></td></tr>
			<tr><td><?=T_('Groups');?>:<input type="checkbox" <? if ($action == 'add_new') echo " checked ";?> <? if ($action == 'edit') echo $checked; ?> name="groups" id="groups"></td></tr>			
		</table>
...
<? if ($action == "") : ?>
	...
	<form name="divisions_form" id="divisions_form" action="admin_divisions.php" method="post" accept-charset="UTF-8">
		...
		<a href="#" onclick="document.getElementById('action').value='add_new'; document.forms['divisions_form'].submit();"><?=T_('Add New Division')?></a>
