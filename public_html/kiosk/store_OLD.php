...
if ($checkout != "") {
	$sql = "UPDATE cart_purchases SET prize_shipped=1 WHERE user_id=" . $user_id . " AND prize_shipped=0";
...
// ***** TOTAL PRICE ***** //
$total_price = gri('total_price', 0);
// ***** TOTAL PRICE ***** //

// ********** INSERT or UPDATE CART ITEMS ********** //
if ($prize_id > 0) {
	$sql = "SELECT cart_purchase_id  FROM cart_purchases WHERE user_id=" . $user_id . " AND prize_id=" . $prize_id . " AND prize_shipped=0";
	$query = mysql_query($sql);
	$row = mysql_fetch_assoc($query);
	
	if ($row['cart_purchase_id'] > 0) {	
	
		if ($quantity > 0) {
			$sql = "UPDATE cart_purchases SET prize_quantity=" . $quantity . " WHERE cart_purchase_id=" . $row['cart_purchase_id'];
			mq($sql);
		}
		else {
			$sql = "DELETE FROM cart_purchases WHERE cart_purchase_id=" . $row['cart_purchase_id'];
			mq($sql);		
		}
	} 
	else {
		$sql = "INSERT INTO cart_purchases (user_id, prize_id, prize_shipped, prize_points, prize_date, prize_quantity) VALUES(";
		$sql = $sql . $user['user_id'] . ", " . $prize_id . ", 0, " . $total_price . ", CURDATE()," . $quantity . ")";
		mq($sql);
	}
...
// ********** CART ITEMS ********** //
$cart_items = array();
$sql = "SELECT cp.prize_id, ps.prize_name, ps.prize_image_id, cp.prize_quantity FROM cart_purchases AS cp JOIN prizes_store AS ps USING (prize_id) WHERE user_id=" . $user_id . " AND prize_shipped=0";
$query = mysql_query($sql);
...
function get_store_prize_points($school_id) {
	...
	$sql = "SELECT prize_points FROM prizes_store WHERE school_id=" . $school_id . " GROUP BY prize_points ORDER BY prize_points";	
	$query = mysql_query($sql);	
...
function get_prizes_store($school_id, $prize_points) {	
	...
	$sql = "SELECT ps.prize_id, ps.prize_points, ps.prize_name, ps.prize_image_id, sp.prize_quantity FROM prizes_store AS ps LEFT JOIN cart_purchases AS sp ON user_id=" . $user_id ." AND ps.prize_id=sp.prize_id AND sp.prize_shipped=0 WHERE school_id=" . $school_id . " ORDER BY prize_points";	
	$query = mysql_query($sql);	
...
<HTML DIR="<?=$dir?>">
	<HEAD>
		...
		<SCRIPT type="text/javascript">
			...
			function update_points(quantity, price, hidden_field, id) {
				...
				var left =  document.getElementById(hidden_field).value - total;
				
				$('.prize_price_total', quantity.parentNode.parentNode).find('span').text(total).end().fadeIn('normal');
				$('.prize_price_left', quantity.parentNode.parentNode).find('span').text(left).end().fadeIn('normal');
				
				document.getElementById(quantity_div).value = quantity.value;
				document.forms[form_name].elements["total_price"].value = total;
			}
...
<?php
$row_num = 0;
$sql = "SELECT ps.*, cp.prize_quantity AS quantity FROM prizes_store AS ps LEFT JOIN cart_purchases AS cp ON user_id=" . $user_id . " AND ps.prize_id=cp.prize_id AND cp.prize_shipped=0";
$query = mysql_query($sql);
					...
					<div class="prize_overlay" id="prize_view_<?=$prizes['prize_id']?>" style="display: none;">
						...
						<div class="prize_bottom">
							...
							<div class="prize_price">
								...
								<div class="prize_price_left">Miles Left: <span><?=$miles_left;?></span></div>
								<input type="hidden" name="<?=$hidden_name;?>" id="<?=$hidden_name;?>" value="<?=$miles_left;?>">
							</div>
							
							<div class="input">
								<input type="text" name="quantity" class="prize_qty keypad" onChange="update_points(this, <?=$prizes['prize_points'];?>, '<?=$hidden_name;?>', <?=$prizes['prize_id'];?>);" value="<?=$prizes['quantity'];?>">
...
<?	}	
	echo "<div class='clear'></div>\n<div class='button_small'>\n<div>\n<a class='cart_buynow' href='#' onClick='buy_now();'>Buy Now</a>\n</div>\n</div>\n</div>\n</form>";
} ?>	
<!-- ****************************** CHECKOUT OVERLAY ****************************** -->
