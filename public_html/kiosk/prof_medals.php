...
function get_tlses($subject_id) {
	...
	*/
	...
	$sql = $sql . " ORDER BY start_date ";
	echo "<input type='hidden' name='tlses' value='$sql'>";
...
function get_missions($track_level_school_type) {
    ...
    echo "<input type='hidden' name='mission_count' value='" . $sql . "' />";
    
	$rows = mysql_query($sql);
	/* commented out to fix issue of not showing medals in future
	while ($row = mysql_fetch_assoc($rows)) 
	{
...
function get_medals($subject_id, $missions_count) {
	...
	while ($row = mysql_fetch_assoc($rows)) {
		...
		if ($row_num == 0) {
			 $first_medal = $row['medal_ord'];
			 //echo 'FIRST:' . $first_medal . '<br />';
		}
		$row_num++;
		if ($row_num == $num_rows) {
			$last_medal = $row['medal_ord'];
			//echo 'LAST:' . $last_medal . '<br />';
			//exit();
		}
...
function get_all_medals($subject_id) {
	...
	echo "<input type='hidden' name='tanya/seferHamitzvosMedals' value='$sql' />";
	$rows = mysql_query($sql);
...
function list_items($subject_id, $medals) {
	...
	for ($cntr = 0; $cntr < count($medals); $cntr++) {	
		...
		if ($medals[$cntr]->completed == true) 
		{
			...
			$sql = "SELECT * FROM medal_marks WHERE user_id=" . $user_id . " AND subject_id=" . $subject_id . " AND medal_ord=" . $medals[$cntr]->medal_ord;	
			$rows = mysql_query($sql);
			$num_rows = mysql_num_rows($rows);
			
			if ($num_rows == 0) {
				$today_julian = gregoriantojd(date("m"), date("d"), date("Y"));
				$insert_sql = "INSERT INTO medal_marks SET medal_ord=" . $medals[$cntr]->medal_ord . ", subject_id=" . $subject_id . ", user_id=" . $user_id . ", date_awarded=" . $today_julian . ", medals_updated=0, prof_medals_updater=1";
		...
		if ($medals[$cntr]->clickable == true) 
			$return_string = $return_string . "\t\t\t<a href='#' onclick='submitForm(" . $subject_id . ", " . $medals[$cntr]->medal_ord . ", " . $first_medal . ", " . $last_medal . ");'>\n";
...
$sql = $sql . " WHERE subject_type NOT IN ('school_points', 'home_points') AND subjects.subject_id not in (91)";
$sql = $sql . " ORDER BY subject_id ";
echo "<input type='hidden' name='subjects' value='$sql'>";
$rows = mysql_query($sql);
while ($row = mysql_fetch_assoc($rows)) {
...
<script type="text/javascript">
	...
	{
		document.getElementById("first_medal").value = first_name;
		document.getElementById("last_medal").value = last_name;
							...
							<ul>
								...
								<?php
									...
									for ($cntr = 0; $cntr < count($subjects); $cntr++) {
										...
										//echo "<pre>";
                                        //print_r( $track_level_school_types );
                                        //echo "</pre>";
										...
										for ($cntr1 = 0; $cntr1 < count($track_level_school_types); $cntr1++) {
											$missions_count += get_missions($track_level_school_types[$cntr1]);
										}

										if ($subjects[$cntr]->subject_id != 27 && $missions_count != 0) {
