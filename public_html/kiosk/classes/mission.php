...
class week_two {
	...
	public function get_campaigns($todays_date) {	
		global $no_of_tasks;
		global $mission_tasks_completed;
		global $mission_points;
		global $mission_points_completed;
		...
		while ($row = mysql_fetch_assoc($query)) {
			...
			$mission_points = 0;
			$mission_points_completed = 0;
			$no_of_tasks = 0;
			$mission_tasks_completed = 0;
			$tasks = $this->get_tasks($camp_mission_id, $todays_date);
			$element = compact('camp_mission_id', 'mission_name', 'no_of_tasks', 'mission_tasks_completed', 'mission_points', 'mission_points_completed', 'tasks');
		...
		while ($row1 = mysql_fetch_assoc($query1)) {
			...
			for ($dno = 0; $dno < ($difference); $dno++) {
				...
				if ($num_rows > 0) {
					...
					$no_of_tasks++;
					$mission_points = $mission_points + $task_points;
					...
					if ($task_date > $todays_date) {
						...
						if ($row2['completed'] == 0) {
							...
							$completed = "1";
							$completed_points = $completed_points + $task_points; 
							$mission_tasks_completed++;
							$mission_points_completed = $mission_points_completed + $task_points;
						}
					}					
			...
			if ($found) {
				$element = compact('camp_task_id', 'task_name', 'task_points', 'total_points', 'completed_points', 'task_dates');
