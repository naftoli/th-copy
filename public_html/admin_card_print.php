...
if ($admin_user['auth'] == 'super') {
	...
	while ($rowP = mysql_fetch_row($resultP)) {
		$sqlP2 = "insert into rank_marks (rank_ord, user_id, date_promoted) values(1, $rowP[0], " . unixtojd() . ")";
		//echo $sqlP2 . "<br />";
		mysql_query($sqlP2);
	}
}
...
if (!empty($admin_user)) {
	...
	if (isset($_GET['action']) && !isset($_GET['withoutPic'])) $withoutPic = 0;
} 
else {
...
$sql1 = "SELECT DISTINCT user_id, first, last, first_he, last_he, username, user_code, 
		...
		class_grade, class_sub, user_photo_id, dob, dob_he_offset, user_start_date, 
	...
	<BODY>
		...
		<FORM action="admin_card_print.php" method="get" accept-charset="UTF-8">
			...
			<P>								
				...
				<input type="checkbox" name="withoutPic" value="1" <?=$withoutPic ? 'checked' : ''?> > Include soldier(s) without pictures
				<br />
				<br />
				...
				<span class='dates'>
	                ...
	                <INPUT type="text" name="end_date_disp" READONLY value="<?=es(dateToHebrew(unixtojd()))?>" onClick="getDate(this.form, 'end_date', true);"/>
				</span>
				<br />
		...
		<FORM action="admin_card_print.php" method="post" accept-charset="UTF-8">
			...
			<? if ($action != "") : ?>
				...
				<? if (!isset($_GET['withoutPic']) && (!($row['user_photo_id'] > 0) && !$row['mobile_pic'])) continue; ?>
	...
	?>
					...
					<TD class="card">
						...
						<DIV class="sig" style="border-color: <?=es($row['rank_color'])?>;">
							...
							<H1><EM style="color: <?=es($row['rank_color'])?>;"><?=es($row['rank_name'])?></EM> <?=es(!empty($row['first_he']) ? $row['first_he'] : $row['first']), ' ', es(!empty($row['last_he']) ? $row['last_he'] : $row['last'])?></H1>
						...
						<DIV class="panel">
								...
								<TR>
									...
									<TD class="base">
										<P>
											...
											Platoon: <?=es($row['class_grade']) . (empty($row['class_sub']) ? '' : '-' . es($row['class_sub']))?>
										</P>
									</TD>
							...
							<TABLE>
								<TR>
									...
									<TD class="stats">
										<P>
											<?=T_('Member since')?><BR><SPAN dir="rtl"><?=!empty($row['user_start_date']) ? dateToHebrewShortYear($row['user_start_date']) : ''?></SPAN>
										</P>
										<P>
											<?
