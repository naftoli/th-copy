...
$qryTimes[$qryCounter]['start'] = time();
$sqlStorePoints = "select sum(points) as points
					FROM pointsDB.user_points
					where user_id = " . $user['user_id'];
$resStorePoints = mysql_query($sqlStorePoints);
$rowStorePoints = mysql_fetch_assoc($resStorePoints);
$intUserStorePoints = $rowStorePoints['points'];
$qryTimes[$qryCounter++]['end'] = time();
$qryTimes[$qryCounter]['start'] = time();
$sqlPoints = "select sum(points) as points
				FROM pointsDB.user_points
				where user_id = " . $user['user_id'] . "
				and resource_name in ('generic achievement card','specific achievement card', 'direct_transfer')";
$resPoints = mysql_query($sqlPoints);
$rowPoints = mysql_fetch_assoc($resPoints);
$arrPointsEarned = $rowPoints['points'];
$qryTimes[$qryCounter++]['end'] = time();
...
<!--                    	<li class="icon_home"><a href="../statement.php">Home</a></li>-->
							<li class="icon_logout"><a href="logout.php?n=statement2.php">Logout</a></li>
						</ul>
					</div>
...
<?php
	...
	$user_row['total_miles'] += $arrPointsEarned["all_points"];
...
?>
	...
	function loadShadow(cardnum) {
		...
		if (cardnum.value.length == 6) {
			...
			Shadowbox.open({
				content:    '/cardpop2.php?card_id=' + cardnum.value,
				player:     "iframe",
				width:      770,
				height:     600// 430
			});
	     ...
	     <? if($user['registered'] && (!$camp_season) ): ?>
            <div class="one_column">
				...
				<!--<li><a href="deposit.php" class="icon_deposit"><?=T_('Deposit')?>   </a><? if($inbox['num']){ ?><span class="badge"><?=$inbox['num']?></span><?}?></li>-->
					...
					?>					
						<li><a href="http://mashpia.com/v2/kiosk/auto-login/uc/3<?=$user_row['user_code']?>" class="icon_shop"><?=T_('Store')?></a></li>
					<?php 
						//}
						//}
						//}
					...
					<li style="vertical-align:middle;">
                        <?=linkImgFile(1407977020,NULL,NULL," style='float:left;margin:20px 0 0 15px;'");?>
                        <a href="http://v2.mashpia.com/kiosk-main/view-campaign/campaign_id/1/bar_code/3<?php print $user_row['user_code']; ?>/tanya_portal/true/rank_title/<?php print strtolower($user_row['rank_name']); ?>/base_num/<?php print $user_row['school_number']; ?>" style="padding-left:20px;">תניא בעל פה</a></li>
                    <?php
						}
                    ?>
