...
$sql = "select end from parshos where year = " . $year . " order by end desc limit 1";
$result = mysql_query($sql);
$row = mysql_fetch_assoc($result);
$end = $row['end'];

$parshos = array();
$sql1 = "select * from parshos where start >= " . (unixtojd() - 7) . " and end <= " . $end;
            ...
            $(function() { 
                ...
                $("#submit").click( function() {
					...
					if ($("#shortName").val() == '') {
						if (errors != '')
							errors += '\n';
						errors += "You have not entered a title for the task.";	
					}
                    if ($("#name").val() == '') {
                        if (errors != '') 
                            errors += '\n';
        ...
        ?>
		        ...
		        <form action='createNewParentTask.php' method='post'>
					...
					<p>
						...
						<input name="shortName" size="30" id="shortName" type="text">
					</p>
