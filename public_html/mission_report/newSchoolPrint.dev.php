...
// split back into an array if multiple classes where passed in
...
<html>
	...
	<body>
		...
		?>
		<? if ($class_id && $startReport == $endReport-6) {?>
		<div id="marking">
			<div id="outer">
				<div id="grade">
		...
		<? } // only show this if there was one grade provided ?>
		<div style="clear: both"></div>	
		<div id="instructions" class="noPrint" style="<?= !($class_id && $startReport == $endReport-6) ? "margin-top: 15px; margin-bottom: 15px;" : ""; ?>">
        	<input type="button" value="Print Instructions" 
        		onclick="window.open('instructions/', '_blank', 'width=790, height=700, menubar=no, scrollbars=yes, status=no, toolbar=no, titlebar=no')" />
        ...
        // function get_missions($startReport, $endReport, $user_id) => creates a new mission for each week and adds it to the array....
        function get_missions($startReport, $endReport, $user_id, &$missions) {
            if($startReport + 6 < $endReport){
                $start = $startReport; $end = $startReport + 6;
                while($start < $endReport){
		...
		if ( $user_id != -1 ) {
			get_missions( $startReport, $endReport, $user_id, $missions);
		} else {
			if ( $class_id && $class_id != -1 ) {
				...
				foreach ($users as $user) {
                    get_missions( $startReport, $endReport, $user, $missions);
				}
			} else if($class_ids) {
				foreach($class_ids as $class_id){
					if($class_id == -1) continue; // skip "All schools"
					$users = array();
					$sql = "select user_id from users where class_id = " . $class_id  . " and user_registered > 0 order by last, first";
					$result = mysql_query($sql);
					while ( $row = mysql_fetch_assoc( $result ) ) {
