...
if ( isset($_GET['go']) && $_GET['go'] == 'back' ) {
    $previous = true; 
    $m->setPreviousDates();
	$rr->setPreviousDates(); // move the rank report up to remove a date's are off bug....
}

// allow for custom date range.... (TODO: REMOVE)
if ( isset($_GET['start']) && isset($_GET['end'])) {
    $previous = true;
	// get the gregorian dates
	$greg_start = explode("-", $_GET['start']); $greg_end = explode("-", $_GET['end']); // split the dates for the converison
    $greg_start = gregoriantojd($greg_start[1], $greg_start[2], $greg_start[0]);
    $greg_end = gregoriantojd($greg_end[1], $greg_end[2], $greg_end[0]);
	// override the start and end dates
    $m->overrideDates($greg_start, $greg_end); 
	$rr->overrideDates($greg_start, $greg_end);
}
...
$url = "https://mashpia.com/medal_rank_ceremony.dev.php";
    ...
    <BODY>
        ...
        <div class='no-print'>
            ...
            <?if ( $previous ) { // if we are in previous mode. add a link to current mode?>
                Click <a href='<?=$url?>'>here</a> to show next report dates.<br /><br />
            <?} else { // if we are in current mode. add a link to previous mode?>
                Click <a href='<?=$url?>?go=back'>here</a> to show previous report dates.<br /><br />
            <?}?>
			<div style="margin: 5px 0px;">
				Custom Date Range: From <input type="date" placeholder="YYYY-MM-DD" id="start_date"/> To <input type="date" placeholder="YYYY-MM-DD" id="end_date"/>
				<a style="display: inline-block" id="custom_date_range" class="button">Go</a>
			</div>
			<script>
