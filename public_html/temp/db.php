...
$arrDevRemotes = array(
	...
	"173.176.52.44" => 1,  //
	"206.108.167.68" => 1, // second cup downtown
	"209.112.40.162" => 1, // starbucks downtown
	'71.19.171.38' => 1,
	'173.176.48.60' => 1,
...
function totalMarks($extra='', $group='', $boolExtract=true) { //$extra can be used for joins or WHERE
	...
	#union all 
	#select sum(points) mark_points from bp_points " . $bp . " 
	) marks $group_by
	";
...
function createInviteTwo($email, $id, $auth, $role_id, $name, $admin_id) {
	global $admin_user;

	if (is_null($role_id))
		$role_id = 'NULL';

	if (mysql_result(mq("SELECT GET_LOCK('invitations', 30)"),0) != 1)
		trigger_error_server('could not get lock', E_USER_ERROR);

	$count = 0;
	do {
		if($count++ > 100000) trigger_error_server('could not get ID', E_USER_ERROR);
		$invitation_id = mysql_result(mq('SELECT FLOOR(RAND() * 9223372036854775807)'),0);
	} while(mysql_result(mq("SELECT COUNT(*) FROM invitations WHERE invitation_id = $invitation_id"),0) != 0);

	mq("INSERT INTO invitations (invitation_id, id, auth, role_id, admin_id, email) VALUES ($invitation_id, $id, " . ms($auth) . ", $role_id, " .  $admin_id . ", " . ms($email) . ')');
	mq("SELECT RELEASE_LOCK('invitations')");

	$to = preg_replace('[^\x20-\x7e]', '', $email);
	mail($to, 'Chayolei Tzivos Hashem Invitation', "
You have been designated as the program director for Tzivos Hashem in 
{$name}.

Please follow this link to create an account, or to add this authorization to your existing account:

http" . (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] == 'on' ? 's' : '') . "://{$_SERVER['HTTP_HOST']}" . rtrim(dirname($_SERVER['SCRIPT_NAME']), '/') . "/register_admin.php?invitation_id=$invitation_id

Tzivos Hashem
718 907 8884
CTH@tzivoshashem.org

", "From: Chayolei Tzivos Hashem <cth@tzivoshashem.org>\nMIME-Version: 1.0\nContent-Type: text/plain; charset=UTF-8");
  return $id;
}

function createInvite($email, $id, $auth, $role_id, $name) {
	global $admin_user;
	...
	mq("INSERT INTO invitations (invitation_id, id, auth, role_id, admin_id, email) VALUES ($invitation_id, $id, " . ms($auth) . ", $role_id, {$admin_user['admin_id']}, " . ms($email) . ')');
	mq("SELECT RELEASE_LOCK('invitations')");

	$to = preg_replace('[^\x20-\x7e]', '', $email);
	mail($to, 'Chayolei Tzivos Hashem Invitation', "
You have been invited to monitor the progress of the $auth
{$name}.
...
To either create an account, or to add this authorization to your existing account.

", "From: Chayolei Tzivos Hashem <cth@tzivoshashem.org>\nMIME-Version: 1.0\nContent-Type: text/plain; charset=UTF-8");
  return $id;
}

if ( !function_exists('header_v2_missions') ) {
...
if ( !function_exists('header_store_points') ) {
	function header_store_points($arrParams) {
		...
		$strUrl = 'http://v2dev1.mashpia.com/legacy/userstorepoints';
		$arrPost = array(
...
if ( !function_exists('header_icorpa_points')) {
	function header_icorpa_points($arrParams)
	{
		...
		$strUrl = 'http://v2dev1.mashpia.com/legacy/userpointsextract2';
...
function getStoreBalance($user) {
	...
	$mashpiaPoints = floatval(mysql_result(mq(totalMarks("WHERE user_id = " . $user)), 0));
	
	$points = array();
	$sql = "select sum(points) as total from pointsDB.user_points
			where resource_name not in ('store', 'transaction_manager_store')
			and user_id = " . $user . "
			and institution_id = " . $institution_id;
	$result = mysql_query($sql);
	$row = mysql_fetch_assoc($result);
	$points['earned'] = floatval($row['total']);
	$points['earned'] += $mashpiaPoints;
	
	$sql = "select sum(points) as total from pointsDB.user_points where user_id = " . $user;
	$result = mysql_query($sql);
	$row = mysql_fetch_assoc($result);
	$points['available'] = $mashpiaPoints + floatval($row['total']);
