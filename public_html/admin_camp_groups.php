...
if ($action != '') {
	...
	switch($action) {
		case 'add':
			$sql = "INSERT INTO group_types SET camp_id=" . $camp_id .  ", group_type_name=" . ms(gr('group_type_name'));
			mq($sql);
			$action = "";
			$group_id = -1;
		break;
		...
		case 'save':
			$sql = "UPDATE group_types SET group_type_name=" . ms(gr('group_type_name')) . " WHERE group_type_id=" . $group_type_id;
			mq($sql);
			$action = "";
			$group_id = -1;
		break;
...
<? if ($action == "") : ?>	
	<body onload="get_levels('12', 'groups_form');">
<? else : ?>
	<body>
<? endif; ?>		
