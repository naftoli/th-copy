...
$admin_auth = array('school','class','team','user','camp'); 
...
if ($admin_user['auths']['school']) {
    ...
    //$schl_id = $admin_user['auths']['school'][0];
    ...
    $sql = "SELECT school_id FROM admin_auths AS aa JOIN schools AS s ON (aa.id=s.school_id AND s.school_era > 0) WHERE aa.admin_id=" . $admin_user["admin_id"] . " AND auth='school' AND (aa.role_id=16 OR aa.role_id=18 or aa.role_id=34)";
    $query = mysql_query($sql);
    $row = mysql_fetch_assoc($query);
    ...
    //$sql = "SELECT school_era AS registered FROM schools WHERE school_id=" . $schl_id;
    //$query = mysql_query($sql);
    //$row = mysql_fetch_assoc($query);
    //$registered = $row['registered'];
    //if (!is_null($registered)) {
    //  $sql = "SELECT role_id FROM admin_auths WHERE admin_id=" . $admin_user['admin_id'];
    //  $query = mysql_query($sql);
    //  $row = mysql_fetch_assoc($query);
    //  $role_id = $row["role_id"];
        
    //  if ($role_id == 18 || $role_id == 16)
    //      $school_registered = "false";
    //}
    // ***** CHECK FOR ALL SCHOOLS ***** //
    ...
    <HEAD>
        ...
        <script>
            ...
            $(document).ready( function() {
                ...
                $("#brochure").blur( function() { 
                	var val = $("#brochure").val();
                	var school_id = <?=$admin->school_id?>;
                	var year = 5774;
                	$.post('ajax/update_brochure.php', {school_id : school_id, year : year, val : val}, function( data ) {
...
if ($admin_school_id > 0) 
    $row = mysql_fetch_assoc(mq("SELECT school_name, inst_name, school_address1, school_address2, school_city, school_state, school_country, school_postal, school_phone, school_logo_id, school_era FROM schools LEFT JOIN institutions USING (inst_id) WHERE school_id=" . $admin_school_id)); 
else 
    $row = mysql_fetch_assoc(mq("SELECT school_name, inst_name, school_address1, school_address2, school_city, school_state, school_country, school_postal, school_phone, school_logo_id, school_era FROM schools LEFT JOIN institutions USING (inst_id) WHERE school_id=" . $school_id . " LIMIT 1")); 
?>
...
<?=$first ? '<H1>' . T_('Home page') . '</H1>' . (!empty($message) ? '<H2>' . $message . '</H2>' : '') : '<HR><HR>'?>
                ...
                <br />
               	...
               	<div>
               		<span class="red"><b>Sivan Resources</b></span><br />
               		Click <a href="https://go.madmimi.com/redirects/1367528654-82f1690899a4986903ff56a9522baaff-33ce7e6?pa=421598565126435451">here</a> for Sivan Resources!<br />
               	</div>
               	
               	<br />
               	<div>
               		<span class="red"><b>Promotion Picture Report</b></span><br />
               		Please make sure to update the pictures by clicking <a href="http://mashpia.com/promotion_report.php">here</a>.<br />
               	</div>
               	
               	<?				
				$sql = "select brochures from registration_brochures 
						where year = '5774' 
						and school_id = " . $admin->school_id;
				$res = mysql_query( $sql );
				$b = mysql_fetch_assoc( $res );
				$brochures = $b['brochures'];
               	?>
               	<br />
               	<div class="brochure">
               		<span class="red">Registration Brochures</span><br />
               		We will be sending you <input id="brochure" type="text" size="3" value="<?=$brochures?>"> registration brochures 
               		for 5774. This number is based on the amount of Chayolim registered in your school this year 
               		plus one third extra. You can modify this number until Sunday, Gimmel Sivan, May 12th 12:00pm EST.<br />
               	</div>
               	<!--
                <?
                    //print_r($admin);
                ...
                <div>
                	<span class="red"><b>Promotion Picture Report</b></span><br />
					Promotion pictures must be in by Thursday Morning, Gimmel Nissan<br />
					Click <a href="promotion_report.php">here</a> for report
                </div>
                <br />
                
                <?
                //find out how many days left until yud alef nissan
                $target = 2456374; //March 22, 2013
                $today = unixtojd();
                $daysLeft = $target - $today;
                ?>
                
                <div>
                    <span class="red"><b>Nissan</b></span><br />
                    Click <a href="https://www.dropbox.com/sh/hrxeu1ooi8hgmqv/MczhSoy8qx?m">here</a> for the Nissan Resources<br />
                    Click <a href="https://www.dropbox.com/s/xi7eyt4jajy40yd/Tzivos%20Hashem%20Haggadah.pdf">here</a> for the Haggada PDF<br />
                </div>
                <br />
                <!--
                <div>   
...
<? if ($admin->is_parent && $child_id == 0) : ?>
    <br />
    <H2><?=T_('Welcome')?>, <?=$admin->first?> <?=$admin->last?></H2>   
    
                            <form method="post" action="admin.php" accept-charset="UTF-8" name="info"  onsubmit="return validation();">                                 
...
if ($admin_user["is_parent"] == 1 && $admin_user["no_of_children"] == 0) : ?>
    <CENTER>
        <br />
        <h1 style="color:red;">You have no children registered.</h1>
        <ul>
        <li>
