  ...
  case 'save':
    foreach(gra('tanya') as $user_id => $data) {
      ...
      //if($length_days) {
        ...
        //mq("INSERT INTO tanya_users (user_id, track, year, tanya_start_date, length_days, length_days_offset, lines_done, lines_offset, pledges, collected, tanya_lines, mishna_lines) SELECT user_id, $track track, $year year, $tanya_start_date tanya_start_date, $length_days length_days, $length_days_offset length_days_offset, $lines_done lines_done, $lines_offset lines_offset, $pledges pledges, $collected collected, $tanya_lines tanya_lines, $mishna_lines mishna_lines FROM users WHERE user_id = $user_id AND school_id = $school_id" . ($class_id != -1 ? " AND class_id = $class_id" : '') . " ON DUPLICATE KEY UPDATE track = $track, year = $year, tanya_start_date = $tanya_start_date, length_days = $length_days, length_days_offset = $length_days_offset, lines_done = $lines_done, lines_offset = $lines_offset, tanya_lines = $tanya_lines, mishna_lines = $mishna_lines, pledges = $pledges, collected = $collected");
        mq("INSERT INTO tanya_users (user_id, tanya_lines, mishna_lines) SELECT user_id, $tanya_lines tanya_lines, $mishna_lines mishna_lines FROM users WHERE user_id = $user_id AND school_id = $school_id" . ($class_id != -1 ? " AND class_id = $class_id" : '') . " ON DUPLICATE KEY UPDATE track = $track, year = $year, tanya_start_date = $tanya_start_date, length_days = $length_days, length_days_offset = $length_days_offset, lines_done = $lines_done, lines_offset = $lines_offset, tanya_lines = $tanya_lines, mishna_lines = $mishna_lines, pledges = $pledges, collected = $collected");
      /*
      } else {
