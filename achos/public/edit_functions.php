...
function remove_user_photo($parameters) {
	...
	$sql = "SELECT u.user_photo_id, s.school_name FROM users AS u JOIN schools AS s USING (school_id) WHERE user_id=" . $user_id;
	$query = mysql_query($sql);
	...
	if ($row) {
		...
		$sql2 = "UPDATE users SET user_photo_id=NULL WHERE user_id=" . $user_id;
...
function update_user($parameters) {
	...
	$user_address1 = $parameters[7];
	$user_address2 = $parameters[8];
	$user_city = $parameters[9];
	$user_state = $parameters[10];
	$user_postal = $parameters[11];
	$user_country = $parameters[12];
	$user_phone = $parameters[13];
	
	$class_id = $parameters[14];
