...
if (isset($_POST['submit'])) {
	...
	$dates = array();
	$sql = "SELECT * FROM reports WHERE report_type='mission_cover_sheet' AND visibility != 'none' and start_date > 2456169 ORDER BY start_date";	
	$result = mysql_query($sql);
	while ($row = mysql_fetch_assoc($result)) {
		$dates[] = $row;
	}
	...
	if ($admin->auth == 'super') {
		...
		$result = mysql_query($sql);
		echo "Choose school: <select name='school'>";
		while ($row = mysql_fetch_assoc($result)) {
