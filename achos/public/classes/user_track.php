...
class user_track 
{
	...
	function get_date_tasks_missions($school_type_id, $start_date, $end_date) 
	{
		...
		$sql = "SELECT * FROM date_tasks_missions WHERE school_type_id=" . $school_type_id . " AND subject_id=" . $this->subject_id . " AND level=" . $this->level . " AND track_id=" . $this->track_id . " AND start_date >= " . $start_date . " AND end_date <= " . $end_date . " ORDER BY mission_number, start_date, mission_name";
		
		//////////if ($this->subject_id == 91)
		echo "<input type='hidden' name='SQL ONE' value='" . $sql . "'>\n";
        
        $dir = getcwd();
		if (strpos($dir, 'mobile'))        
		...
		while ($row = mysql_fetch_assoc($query)) {
			...
			$date_tasks_mission = new date_tasks_mission($row);
            $this->date_tasks_missions[] = $date_tasks_mission;
	...
	function check_task($task_id, $output = false) {
		...
		if ($num > 0) { 
			...
			if ($num > 0) {
				if ($num >= $needed || $row_mark['done_qty'] >= $needed) {
					$completed = true;
				} else if ($qty) {
					if ($row_mark['mark_quantity'] >= $qty) {
						$completed = true;
					}
				}
			}
	...
	function check_missions($output = false) {
		$deleted = 0;
		$inserted = 0;
		$today = unixtojd();
