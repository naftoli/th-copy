...
class date_tasks_mission {
	...
	function get_daily_tasks($start_date, $end_date, $user_id, $subject_id, $subject_name, $track_id, $level, $subject_image_id) {	
		...
		$sql = $sql . "AND dt.deleted = 0 "; 	
		$sql .= "and (dt.created_by is null or dt.created_by = $user_id) ";	
		//$sql = $sql . "ORDER BY f.frequency_id, fp.frequency_period_id, dt.sequence_number";
		$sql = $sql . "ORDER BY dt.ord, dt.label_ord";
        echo "<input type='hidden' name='daily tasks' value=\"" . $sql . "\">\n";
		
		$query = mysql_query($sql);
        $d = new Defaults($user_id);
		while ($row = mysql_fetch_assoc($query)) {
		    //if (!$row['default_on'] && !$row['mandatory_qty'] && !$d->isOn($row['date_task_id'], 'task')) continue;
	...
	function get_no_label_tasks($start_date, $end_date, $mission_name, $mission_number, $user_id, $subject_name, $subject_image_id) {	
		$no_label_tasks = array();
		 
		$sql = "SELECT * ";
		$sql = $sql . "FROM date_tasks AS dt ";
		$sql = $sql . "WHERE dt.date_tasks_mission_id=" . $this->date_tasks_mission_id . " ";
		$sql = $sql . "AND dt.label_id IS NULL ";
		$sql = $sql . "ORDER BY dt.ord, dt.label_ord";
        //echo "<input type='hidden' name='No Label Sql' value='" . $sql . "' />";
