...
$auction = mysql_fetch_assoc(mq("SELECT auction_name, auction_points_start_date, auction_date, auction_points_trigger_date, auction_run_date FROM auctions WHERE auction_id = $auction_id AND auctions.auction_ran = 0 AND (auction_run_date IS NULL OR auction_run_date >= " . unixtojd() . ") AND (school_id IS NULL OR school_id = {$user['school_id']})"));
...
<?else:?>
      <div id="page_title"><?=es(sprintf(T_('Chinese Auction %s'), $auction['auction_name']))?></div>
      <div id="auction_container">
        ...
        <div class="bottom">
			<span style="white-space: nowrap;">
				<?=T_('Miles available')?>: <?=floatval($auction_points['cur'])?>
			</span> 
			&bull;
			<span style="white-space: nowrap;">
			...
			&bull;
			<span style="white-space: nowrap;">
				<?=T_('Miles left')?>: <?=floatval($auction_points['cur'] - $user_prizes_total)?>
			</span>
        </div>
