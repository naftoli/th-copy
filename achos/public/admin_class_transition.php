    ...
    if(gr('to_class', false)) {
      ...
      if(mysql_num_rows(mq("SELECT * FROM classes WHERE school_id = $school_id AND class_id = $new_class_id"))) 
          foreach(gra('users') as $user_id) {
            ...
            mq("UPDATE users SET class_id = $new_class_id, heb_year = $hebYear, user_registered = now() WHERE school_id = $school_id AND user_id = $user_id");
            /*
			...
			*/
			...
			//delete all medals except white
            $sql3 = "delete from medal_marks where user_id = " . $user_id . " and medal_ord != 1";
			mq($sql3);
...
<script type="text/javascript">
    $(function() {
        $("#move").click(function() {
            ...
            var cell = $("#cell").val();
            if (email == "") {
                 alert("You must enter an email address for this teacher.");
                 return false;
            } else {
                //alert(classID);
...
<? else: ?>
	...
	<BR>
<div class="infobox">
<p><?=T_('Below you will find your platoons (classes) from last year. Please update your platoons (classes) for the coming school year.')?></p>
</div>
...
- Teachers email address (mandatory) preferably gmail so they can be shared on Google Docs<br />
- Teachers cell phone numbers (optional) in order to receive updates and reminders through text<br /></span>
