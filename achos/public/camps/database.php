...
function mysql_fetch_column_tuple($result) {
  if(mysql_num_fields($result) < 2) return mysql_fetch_column($result);
  $ret = array();
  while($row = mysql_fetch_assoc($result)) {
...
function totalMarks($extra='', $group='') { //$extra can be used for joins or WHERE
  ...
  return "
  SELECT $group IFNULL(SUM(mark_points), 0) mark_points FROM
  (
...
function auctionPoints($user_id, $auction_row = NULL) {
  ...
  if($cur_points >= 1200) $cur_points = floatval(mysql_result(mq(totalMarks("WHERE user_id = $user_id AND mark_date <= {$auction_row['auction_date']}")), 0));
...
function createInvite($email, $id, $auth, $role_id, $name) {
  global $admin_user;
  if(is_null($role_id)) $role_id = 'NULL';

  if(mysql_result(mq("SELECT GET_LOCK('invitations', 30)"),0) != 1) trigger_error_server('could not get lock', E_USER_ERROR);
  $count = 0;
  do {
