...
mysql_select_db('achos') or trigger_error_server('Failed to select db', E_USER_ERROR);
...
function auctionPoints($user_id, $auction_row = NULL, $checkIcorpa = true)
{
	if ($checkIcorpa) { 
	...
	if ($checkIcorpa) 
		$cur_points += $arrPoints["hebrew_points"];
	...
	*/
	...
	if (!is_null($auction_row['auction_points_trigger_date']))
	{
		...
		if ($checkIcorpa) 
			$trigger_points += $arrPoints["jd_points"];
