...
//make sure to show current week from Sunday
$d = unixtojd();
$day = date("N");
$end = $d;
...
if (isset($_POST['action']) || count($admin->children) == 1) {
	...
	if ($action == "produce_report" || count($admin->children) == 1) {
		...
		if (isset($_POST['date_list'])) {
			...
			$sql = "SELECT * FROM reports WHERE report_type='mission_cover_sheet' AND visibility != 'none' AND start_date > " . ($end - 8) . " AND end_date <= " . $end . " ORDER BY start_date";	
			$query = mysql_query($sql);
			$row = mysql_fetch_assoc($query);
...
<? if (count($admin->children == 1)) { ?>
	<p align='center'>Please Note: You may have to actively click on the weekly drop-down and select a Parsha 
	(even if it is already highlighted) in order to mark or print missions.</p>
<? } ?>

<? if ($child_id < 1 && count($admin->children) > 1) : ?>
			<form name="parents_date_tasks_report" id="parents_date_tasks_report" action="parents_date_tasks_report.php" method="post" accept-charset="UTF-8">
						...
						<div class="date_list select_box">					
							...
							<select name="date_list" class="sSelect">
								<? for ($rno = 0; $rno < count($reports); $rno++) : ?>
									<? $report = $reports[$rno]; ?>									
									<?// echo "<input type='hidden' name='DATES' value='" .$report->start_date . ":" . $report->end_date . "-" . $selected_dates . "'>"; ?>
									
									<? if (($rno == count($reports) - 1 && $last_week == true) || ($selected_dates == $report->start_date . ":" . $report->end_date) ) : //$report->start_date == $start_date?>
