							...
							?>
								...
								for ($dtno = 0; $dtno < $numTasks; $dtno++) {
									if ($user->daily_tasks[$dtno]->label_name == $label) {
							            ...
							            <div class="row">
											<div class="rowImg">
												<?php $logo = $this->campaignLogos[$daily_task->subject_id] ?? ''; if ($logo): ?>
												<img src="/mission_report/campaignLogos/<?= htmlspecialchars($logo) ?>" width="50" height="52" alt=""/>
												<?php endif; ?>
											</div>
						 		...
						 		for ($wtno = 0; $wtno < $numTasks; $wtno++) {
									...
									if ($label_name == $user->weekly_labels[$lno]) {
										...
										<div class="row">
											...
											<div class="rowImg">
												<?php $logo = $this->campaignLogos[$weekly_task->subject_id] ?? ''; if ($logo): ?>
												<img src="/mission_report/campaignLogos/<?= htmlspecialchars($logo) ?>" width="50" height="52" alt=""/>
												<?php endif; ?>
											</div>
						 		...
						 		for ($stno = 0; $stno < $numTasks; $stno++) {
									...
									if ($label_name == $user->shabbos_labels[$lno]) {
										...
										<div class="row">
											...
											<div class="rowImg">
												<?php $logo = $this->campaignLogos[$shabbos_task->subject_id] ?? ''; if ($logo): ?>
												<img src="/mission_report/campaignLogos/<?= htmlspecialchars($logo) ?>" width="50" height="52" alt=""/>
												<?php endif; ?>
											</div>
						...
						for ($nltno = 0; $nltno < $numTasks; $nltno++) {
							...
							if ($key1 == $key2) {
								...
								<div class="row">
									...
									<div class="rowImg">
										<?php $logo = $this->campaignLogos[$no_label_task->subject_id] ?? ''; if ($logo): ?>
										<img src="/mission_report/campaignLogos/<?= htmlspecialchars($logo) ?>" width="50" height="52" alt=""/>
										<?php endif; ?>
									</div>
		...
		<div class='container'>
			...
			if ($report_mission) {
				...
				<div class='track'>
					...
					var sheet = document.getElementById('user-<?= (int) $this->user_id ?>');
					var elem = sheet ? $(sheet).find('.track').eq(0) : $('.track').eq(0);
		...
		<div class="extra-info" dir="ltr">
				...
				<?php foreach ($user->medals as $medal) : ?>
					...
					<div class='medal'>
						<div class='medal-icon'>
							<?php $logo = $this->campaignLogos[$medal['subject_id']] ?? ''; if ($logo): ?>
							<img src='/mission_report/campaignLogos/<?= htmlspecialchars($logo) ?>' alt='<?= htmlspecialchars($medal['medal_name']) ?>' />
							<?php endif; ?>
						</div>
			...
			<div class='streaks-container'>
				<?php foreach ($activeStreaks as $streak) : ?>
					<div class='streak'>
						<div class='campaign-icon'>
							<?php $logo = $this->campaignLogos[$streak['subject_id']] ?? ''; if ($logo): ?>
							<img src='/mission_report/campaignLogos/<?= htmlspecialchars($logo) ?>' alt='<?= htmlspecialchars($streak['duch_cat']) ?>' />
							<?php endif; ?>
						</div>
						<div class='task'>
