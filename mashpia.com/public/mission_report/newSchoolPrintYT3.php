...
// yk / succos / breishis 5778
$dates['start'] = array( 2458026, 2458033, 2458040 );
$dates['end'] = array( 2458032, 2458039, 2458046 );
?>
<!DOCTYPE html>
<html>
	<head>
		...
		$missions = array();	
			if ( $user_id != -1 ) {
				for ($i = 0; $i < $numMissions; $i++) {
					...
					$m = new Missions( $startReport, $endReport, $user_id, 0, 0, false );
				...
				if ( $class_id != -1 ) {
					...
					foreach ($users as $user) {
						for ($i = 0; $i < $numMissions; $i++) {
							$startReport = $dates['start'][$i];
							$endReport = $dates['end'][$i];	
							$m = new Missions( $startReport, $endReport, $user, 0, 0, false );
							$missions[] = $m->getMissions();
						}
					}
				} else {
	...
	<script>
		$( function() {
			$(".arrow-left").click( function() {
				...
				window.location.href = "newSchoolPrintYT3.php?school=" + school + "&grade=" + grade + "&user=" + student + "&dblSided=" + dblSided + "&showDate=" + showDate;
			});
			...
			$(".arrow-right").click( function() {
				...
				var dblSided = <?=$dblSided?>;
				var showDate = <?=$showDate?>;
				window.location.href = "newSchoolPrintYT3.php?school=" + school + "&grade=" + grade + "&user=" + student + "&start=" + "&dblSided=" + dblSided + "&showDate=" + showDate;
			});
