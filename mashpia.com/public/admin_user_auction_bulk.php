...
$auction = mysql_fetch_assoc(mq("select * from auctions where auction_id = " . $auction_id));
...
//} else {
 if ($auction) {
  ...
  $prizes = mq("SELECT prize_id, prize_name, prize_number, prize_points FROM prizes_auction JOIN auction_prizes USING (prize_id) WHERE auction_id = $auction_id" . (!is_null($auction['max_prize_points']) ? " AND prize_points <= {$auction['max_prize_points']}" : '') . ' ORDER BY prize_points, prize_number');
	...
	$qry = "SELECT FLOOR(IFNULL(used_points,0)) avail_points, COUNT(*) num
					FROM users
					JOIN classes USING (class_id)
					JOIN $auction_points_query USING (user_id)
					LEFT JOIN (
						SELECT SUM(prize_points*quantity) used_points, user_id
						FROM auction_user_prizes
						JOIN prizes_auction USING (prize_id)
						JOIN auction_prizes USING (auction_id, prize_id)
						WHERE auction_id = $auction_id GROUP BY user_id) used USING (user_id)
					WHERE (used_points IS NULL OR $auction_points_query_exp > used_points)
					AND user_start_date IS NOT NULL
					GROUP BY avail_points";
	//echo "<br />" . $qry;
  //$histogram = mq($qry) or die($qry . "<br />" . mysql_error());
}

//$auction_result = mq("SELECT auction_id, auction_name, auction_date, school_id FROM auctions WHERE auction_ran = 0 AND" . ($admin_user['auth'] == 'super' && $school_id == -1 ? ' auctions.school_id IS NULL' : " auctions.school_id = $school_id") . ' ORDER BY auction_date');
$auction_result = mq("select auction_id, auction_name, auction_date from auctions where auction_ran = 0 and auction_date > " . unixtojd());
?>
