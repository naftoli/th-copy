...
<script>
  ...
  document.addEventListener('DOMContentLoaded', async function () {
    ...
    for (let school in data) {
      ...
      html += `
        <table><tr>${school > 0 ? '' : "<th>School</th>"}<th>Family ID</th><th>Grade</th><th>Student</th><th>Serial #</th><th>TH Started Date</th>
      ...
      for (let user of data[school]) {
        ...
        html += `
          <tr>${school > 0 ? '' : "<td>" + schools[user.school_id] + "</td>"}<td>${user.admin_id}</td><td>${grade}</td><td>${user.first + ' ' + user.last}</td>
