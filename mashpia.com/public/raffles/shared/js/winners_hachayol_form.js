...
function updateRaffle() {
    ...
    $.post("/raffles/shared/ajax/get_raffle_winners.php", {school_id: school_id, raffle_id: raffle_id, single_list: true, sorting: sorting}, function(data){
        ...
        html += '<thead>';
        html += '<th>Prize Won</th><th>Name</th>';
        html += '</thead>';
        html += '<tbody>';
...
winner_renderer.prototype.render = function(){
    ...
    this.html += '<td style="text-transform: capitalize;"><a href="https://mashpia.com/admin_user.php?action=edit&user_id=' + this.winner.user_id + '&school_id=' + this.winner.school_id + '">';
    this.html += (this.winner.first_name ? this.winner.first_name.toLowerCase() : "") + " ";
    this.html += (this.winner.last_name ? this.winner.last_name.toLowerCase() : "") + '</a><br/>';
    this.html += (this.winner.hachayol_name ? this.winner.hachayol_name.toLowerCase() : this.winner.school_name) + '</td>';
    this.html += '</tr>';
    return this.html;
};
...
function exportToCSV() {
    ...
    var csvContent = "Prize Won,Name,School\n"; // the baisc csv file
    var universalBOM = "\uFEFF";
    // TODO add headers
    $.each($("tr.winner"), function(index, item) {
        ...
        $.each(item.find("td"), function(index, item) {
            if($(item).find('a').length > 0){
                row.push($(item).find('a').text() + "," + $(item).html().split("<br>")[1]);
            } else {
                row.push('"' + $(item).text()+'"');
            }
        });
