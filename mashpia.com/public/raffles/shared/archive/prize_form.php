...
if($action == "create" || $action == "update"){ // for both create and update, run the validations
    ...
    // type of prize only required on create
    if($action == "create" && $_POST['type_of_prize'] != ""){
        $type_of_prize = $_POST['type_of_prize'];
    }
...
if ($action == "update"){
    if ($valid){
        ...
        $prize->name = $name;
        //$prize->type_of_prize = $type_of_prize; // cannot update type of prize
...
<HTML>
    <HEAD>
        ...
        <link href="/raffles/shared/styles/form_style.css" rel="stylesheet" type="text/css">
    </HEAD>
    <BODY>
        ...
        <?} else if ($action == "edit"){ // *************************** EDIT ACTION PAGE **********************//?>
            ...
            <form method="POST" enctype="multipart/form-data" action="<? echo htmlspecialchars($_SERVER["PHP_SELF"]); echo $debug ? "?debug=true" : "";?>">
                ...
                <div class='input_box'>
                    ...
                    <div class="input_group input_full">
                        Type*:
                        <select id="type_of_prize" name='type_of_prize' disabled><? // change id to name and remove disabled to make this "work" again
                            $options = ["weekly", "monthly"]; // list of options in the select box "yearly"
                            foreach($options as $option){
                                $selected = $option == $prize->type_of_prize ? "selected" : "" ; // generate if it is selected
                                echo "<option value='$option' $selected >".ucfirst($option)."</option>"; // render the option with the first letter in caps
                            }?>
                        </select>
                    </div>
                    <div class="input_group input_full">
            ...
            <table id="raffles">
                <thead><tr><th>Raffle</th><th>Runs On</th>
                <? echo $prize->type_of_prize == "weekly" ? "<th>Included</th>" : ""; // only show the included tab for weekly raffles ?>
                ...
                <?foreach($raffles as $raffle){ // render each prize option
                    ...
                    <tr>
                        ...
                        <? if ($prize->type_of_prize == "weekly"){ // only show the checkbox for weekly raffles ?>
                        <td>
                            <input type="checkbox" id="raffle_<?=$raffle->raffle_id?>" <?= $prize->raffles[$raffle->raffle_id] ? "checked ": ""; ?>/>
                        </td> 
                        <?}?>
                        <td>
                            <input type="number" <?=$raffle->type == "weekly" ? "" : "disabled";?>
        ...
        <div class="action-links">
            ...
            <select class="grey" id="type" name="type" disabled>
...
<!--                <option value="monthly">Monthly</option>-->
        ...
        <script>
            ...
            $("select#type").change(function(event){
                ...
                $.post("/raffles/shared/ajax/table_prize.php" + (debug_mode ? "?debug=true" : ""), {type: type}, function(data){
                    $("#prizes").html(data);
                });
            });
