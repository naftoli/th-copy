<? 
	...
	{
		if($photo_id["user_photo_id"] >0)
		{
			$row = mysql_fetch_assoc(mqu('SELECT file_name, file_content_type, UNIX_TIMESTAMP(file_last_mod) file_last_mod, ' . ($_SERVER['REQUEST_METHOD'] !== 'HEAD' ? 'file_data, ' : '') . 'LENGTH(file_data) file_length FROM files WHERE file_id = '.$photo_id['user_photo_id']));
