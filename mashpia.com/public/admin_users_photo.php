...
$edit_result = mq("
SELECT users.user_id, users.first, users.last, users.username, class_grade, class_sub, class_teacher, users.user_photo_id, users.mobile_pic   
FROM users LEFT JOIN classes USING (school_id, class_id) 
WHERE school_id = $school_id" . ($class_id != -1 ? " AND class_id = $class_id" : '') . 
($missing ? " and (user_photo_id is null or user_photo_id = 0) and mobile_pic is null " : '') . '  
ORDER BY users.last, users.first, users.username');
?>
...
<p>Please note that while rotating images will update them on this page you will need to clear your browsers cache to see the updated image across the system.</p>
		...
		<DIV>
			...
			<TABLE class="list list_<?=$align_start?>">
				...
				<TR class="<?=($toggle ^= 1) ? 'odd' : 'even'?>">
					...
					<TD>
						<a href="admin_user_photo.php?school_id=<?=$school_id;?>&user_id=<?=$row['user_id'];?>
					...
					<TD name="td_<?=$row['user_id'];?>" id="td_<?=$row['user_id'];?>">
						<? if(!is_null($row['user_photo_id']) || !empty($row['mobile_pic'])) : ?>
							<LABEL>
							...
							<? else : ?>
								<img src="file_view.php?id=<?=$row['user_photo_id']?>" height="80" />
