...
SWFUpload.prototype.callFlash = function (functionName, argumentArray) {
	...
	try {
		if (movieElement != undefined) {
			returnString = movieElement.CallFunction('<invoke name="' + functionName + '" returntype="javascript">' + __flash__argumentsToXML(argumentArray, 0) + '</invoke>'); // this line throws an unidentifed error (literally just "Error") with a vaughe undocumented error number 
			returnValue = eval(returnString);
		} else {
			this.debug("Can't call flash because the movie wasn't found.");
		}
	} catch (ex) {
		// this is where our issue lies
