...
class SchedulerProc
{
	...
	public function process_mission_book3($arrParams)
	{
		...
		if (!isset($arrParams["task_offset"]) || (!$arrParams["task_offset"] && $arrParams["task_offset"] !== 0))
		{
			...
			{
				$arrParams["task_offset"] = intval($objLatestUserCampaignStart->task_increment)+($intTaskVelocity > 1 ? 1 : 0);
				...
				{
					$arrParams["task_offset"] += intval($objLatestUserCampaignStart->ladder_velocity);
			...
			else
			{
				...
				$arrParams["task_offset"] = intval(@$objUserCampaignEnrollment->line_offset)+1;
			}
		}
		$intIteration = 1;
		...
		while ($intIteration < $intMaxIterations)
		{
			...
			//print $intIteration.'<br />';
			//print date("F j, Y, g:i a", round($this->_POSITION["time"])) . '<br />';
			...
			if (count($arrPauseResume))
			{
				...
				{
					// check if there was a resume after the last resume
					// this means that the resume should be ignored
					if (
						$arrParams["capture_end_date"] < $this->_POSITION["time"]
					) {
						if ($objPauseResume->status == 'Paused')
							break;
						if ($boolPaused)
							break;
						if (!$boolResumed)
							break;
						if ($objPauseResume->status == 'Resumed')
						{
							$boolPaused = 1;
							break;
						}
			...
			//dumper($boolPaused);
			//dumper(date("F j, Y, g:i a", round($this->_POSITION["time"])));
			if (
				!$boolPaused
				&& (
					!isset($arrParams["capture_start_date"])
