		...
		if ($strReportType == "grade")
		{
			...
			foreach ($arrCampaignEnrollment as $arrInstitution)
			{
				foreach ($arrInstitution as $objEnrollment)
				{
					...
					//array_pop($arrPauses);
					//array_pop($arrPauses);
					//array_shift($arrPauses);
					//array_shift($arrPauses);
					//array_shift($arrPauses);
					//array_shift($arrPauses);
					//dumper($arrPauses,0,1);
		...
		if ($this->_request->isPost())
		{
			...
			if (isset($arrPostParams["total_points"]))
			{
				foreach ($arrUsers as $objUser)
				{
					...
					{
						$arrPointsParams["_GREATER"]["created"] = $intStartDate;
						$arrPointsParams["_GREATER"]["_TIMESTAMP"] = true;
					}
					if (isset($intEndDate))
					{
						$arrPointsParams["_LESSER"]["created"] = $intEndDate;
						$arrPointsParams["_LESSER"]["_TIMESTAMP"] = true;
					}
