...
function build_items() {
    ...
    foreach ($categories as $cat) {
        ...
        foreach ($items[$cat] as $item) {
            ...
            $html .= "<input type='checkbox' name='items[" . $cat . "][" . htmlspecialchars($name, ENT_QUOTES) . "]' class='item' /> ";
            if ($item == 'Auction') $html .= "Auction (End of Last Yr)<br />";
            else $html .= ucwords($item) . "<br />";
        }
        $html .= "</div>";
    }
    return $html;
            ...
            foreach ($items as $item) {
                ...
                if ($show_item) {
                    if ($output) {
                        ...
                        $originalValue = 0;
                        if (! empty($status)) $originalValue = $status['status'];
                        ...
                        for ($i = 1; $i <= $total; $i++) {
                            echo "<option value='$i'";
                            if (
                                (!empty($status) && $status['qty'] == $i) || (empty($status) && $i == $total)
                            ) {
                                echo " selected ";
                            }
                            echo ">$i</option>";
