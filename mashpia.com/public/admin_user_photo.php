...
if (isset($_POST['action'])) {
	...
	if (isset($_FILES['user_photo']))  {
		...
		$str = "Location: http://mashpia.com/admin_users_photo.php?school_id=" . $school_id;
		...
		<DIV class="ui_<?=$ui_type?> <?=$align_start?>">
			...
			<DIV class="body">
				...
				<FORM action="admin_user_photo.php" method="post" accept-charset="UTF-8" enctype="multipart/form-data">
					...
					<input type="hidden" name="user_photo_id" value="<?=$user->mobile_pic?$user->mobile_pic:$user->user_photo_id;?>">
					...
					<TABLE class="list list_<?=$align_start?>">
						...
						<TR class="odd">
							...
							<TD>
								<? if ($user->user_photo_id > 0 || !empty($user->mobile_pic)) : ?>
									<!--
									<LABEL>
										<?=T_('Delete current photo')?> 
										<INPUT type="checkbox" name="photo_delete_<?=$user->user_id?>" class="checkbox" value="1">
									</LABEL>
									<BR>
									-->
									<? if (!empty($user->mobile_pic)) : ?>
										<img src="/mobile/reg/<?=$user->mobile_pic?>" height="80" />
									<? else : ?>
										<img src="file_view.php?id=<?=$user->user_photo_id?>" height="80" />
