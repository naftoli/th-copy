...
// get list of missing medals from db
$missing = array();
$sql = "select * from missing_medals";
$result = mysql_query( $sql );
while ($row = mysql_fetch_assoc( $result )) {
    $missing[$row['user']][$row['subject']][] = $row['medal'];
}
...
<HTML DIR="<?=$dir?>">
    ...
    <BODY>
        ...
        $toRemove = array(269,61,55,66,110,112,180,82);
        ...
        <div class='no-print'>
            ...
            <?
            if ( $previous ) {  
                echo "Click <a href='medal_rank_ceremony2.php'>here</a> to show next report dates.<br /><br />";
            } else {  
                echo "Click <a href='medal_rank_ceremony2.php?go=back'>here</a> to show previous report dates.<br /><br />";
            }
            ?>
							...
							foreach ( $class as $grade => $info ) {
								...
								foreach ( $info as $user_id => $medals ) {
									foreach ( $medals as $subject => $more ) {
                                        if (isset($missing[$user_id][$subject])) {
                                            echo "<tr><td>" . getRank($user_id) . " " . $userInfo[$user_id] . "</td><td>";
                                            foreach ($more as $medal) {
                                                if (array_search($medal, $missing[$user_id][$subject]) !== false) {
                                                    if ($subject == 'יומי דפגרא') echo "<input type='checkbox' class='missingMedal' checked /> " . $subject . "-" . $medal . " (for completing " . $special[$medal] . " special days of missions)<br />";
                                                    else if (in_array($subject, array('WWTC','מבצעים'))) echo "<input type='checkbox' class='missingMedal' checked /> " . $subject . "-" . $medal . " (for completing " . $months[$medal] . " months of missions)<br />";
                                                    else echo "<input type='checkbox' class='missingMedal' checked /> " . $subject . "-" . $medal . " (for completing " . $weeks[$medal] . " weeks of missions)<br />";
                                                }
                                            }
										}
									}
