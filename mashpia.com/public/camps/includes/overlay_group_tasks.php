                ...
                $(".checklist .checkbox-select").die().live('click',
                    function(event) {
						...
						if (groups == 0) {
							...
							var function_name = "assign_group_tasks";				
							var parameters = [camp_id, camp_task_id, groups, period, group_task];							
											...
											<select onchange="check_checklist(this);" id="groups" class="select group">
												...
												<? for ($gtno = 0; $gtno < count($group_types); $gtno++) : ?>
													<optgroup label="<?=$group_types[$gtno]->group_type_name;?>">
														...
														<? $gt_selected = $group_types[$gtno]->get_group_task($camp_task_id, 1); ?>													
														...
														<? for ($dno = 0; $dno < count($group_types[$gtno]->divisions); $dno++) : ?>	
															<? $division = $group_types[$gtno]->divisions[$dno]; ?>
															<? $div_selected = ""; ?>
															<? if ($gt_selected == "") $div_selected = $group_types[$gtno]->divisions[$dno]->get_division_task($camp_task_id, 1); else $div_selected = ""; ?>
