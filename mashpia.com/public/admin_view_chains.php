...
<HTML DIR="<?=$dir?>">
<HEAD>
<TITLE><?=T_('View Chains'), ' - ', T_('Tzivos Hashem Management System')?></TITLE>
...
<FORM action="admin_view_chains.php" method="get" accept-charset="UTF-8">
<P>
...
<? while($track_row = mysql_fetch_assoc($tracks_result)): ?>
  <TR>
    ...
    <? foreach(range(3, 14) as $each_level): ?>
      ...
      <TD style="text-align: <?=$align_start?>;" <?= $level==$each_level && $track_id==$track_row['track_id'] ? 'id="selected"' : '' ?>><A HREF="admin_view_chains.php?school_type_id=<?=$school_type_id?>&amp;subject_id=<?=$subject_id?>&amp;level=<?=$each_level?>&amp;track_id=<?=$track_row['track_id']?>"><?=$row['missions']?> <?=T_('Missions')?><BR><?=$row['tasks']?> <?=T_('Tasks')?></A></TD>
    <? endforeach; ?>
  </TR>
<? endwhile; ?>
</TABLE>
...
<? $result = mq("SELECT mission_name, mission_description, chain_item_id, floor, room, name, description, mandatory_qty, optional_qty, label_name, quantity, points FROM chain_items LEFT JOIN chain_missions USING (school_type_id, subject_id, level, track_id, floor) LEFT JOIN labels USING (label_id) WHERE school_type_id = $school_type_id AND subject_id = $subject_id AND level = $level AND track_id = $track_id ORDER BY school_type_id, subject_id, level, track_id, floor, room"); ?>
