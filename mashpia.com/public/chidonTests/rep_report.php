...
foreach ($students as $child) {
    ...
    $final = $totalAvg / 2;
    $child_marks[$child['gender']][$grade][$id] = $final;
...
foreach ($child_marks as $gender => $other) {
    foreach ($other as $grade => $more) {
        arsort($child_marks[$gender][$grade]);
    }
}
?>
<!DOCTYPE html>
...
foreach ($child_marks as $gender => $more) {
    ...
    foreach ($more as $grade => $other) {
        ...
        foreach ($other as $id => $avg) {
            echo "<tr><td>" . $i++ . "</td><td>" . $child_info[$id]['school'] . "</td><td>" . $id . "</td><td>" .
                $child_info['first'] . "</td><td>" . $child_info['last'] . "</td><td>" . $avg . "</td><td>";
