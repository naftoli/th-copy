...
if ($action != '') {
	...
	switch($action) {
		case 'add':
			$sql = "INSERT INTO camp_divisions SET camp_group_id=" . $camp_group_id .  ", division_name=" . ms(gr('division_name'));
			mq($sql);
			$action = "";
			$group_id = -1;
		break;
		...
		case 'save':
			$sql = "UPDATE camp_groups SET division_name=" . ms(gr('division_name')) . " WHERE camp_division_id=" . $camp_division_id;
			mq($sql);
			$action = "";
			$group_id = -1;
		break;
		...
		case 'delete':
			$sql = "DELETE FROM camp_divisions WHERE camp_division_id=" . $camp_division_id;
			mq($sql);
			$action = "";
			$group_id = -1;
		break;		
...
<HTML DIR="<?=$dir?>">
	...
	<HEAD>
		<TITLE><?=T_('Groups'), ' - ', T_('Tzivos Hashem Management System')?></TITLE>
...
<? if ($action == "") : ?>	
	<body onload="get_levels('123', 'divisions_form');">
<? else : ?>
	<body>
<? endif; ?>		
...
<? if ($action == 'add_new' || $action == "edit") : ?>
	...
	<form action="admin_camp_divisions.php" method="post" accept-charset="UTF-8">
		...
		<? $row = mysql_fetch_assoc(mq("SELECT group_name FROM camp_groups WHERE camp_group_id=" . $camp_group_id)); ?>
		<h2><?=T_('Group Type');?>: <label style="color:blue;"><?=$row['group_name'];?></label></h2>
		
		<? 
			if ($action == "edit") {
				$sql = "SELECT division_name FROM camp_divisions WHERE camp_division_id=" . $camp_division_id;
				$query = mq($sql);
		...
		<table>
			<tr><td><?=T_('Group Name');?>:<input type="text" name="division_name" id="division_name" value="<?=$division_name;?>"></td></tr>
		</table>
...
<? if ($action == "") : ?>
	...
	<form name="divisions_form" id="divisions_form" action="admin_camp_divisions.php" method="post" accept-charset="UTF-8">
		<input type="hidden" name="action" id="action" value="">
		<? if ($camp_id > 0) : ?>
		<input type="hidden" name="camp_id" id="camp_id" value="<?=$camp_id;?>">
		<? endif; ?>
		...
		<a href="#" onclick="document.getElementById('action').value='add_new'; document.forms['divisions_form'].submit();"><?=T_('Add New Group')?></a>
