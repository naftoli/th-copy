...
if (($handle = fopen($_FILES['file']['tmp_name'], "r")) !== FALSE) {
  ...
  while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
    if ( $row++ < 1 ) continue;
    $qry = "update th_chidon set sunday_pm_bus = '" . $data[1] . "' where th_chidon_id = " . $data[0];
    $qrys[] = $qry;
  }
  fclose($handle);
}
...
<html>
  ...
  <body>
    <form action="sunday_buses.php" method="post" accept-charset="UTF-8" enctype="multipart/form-data">
