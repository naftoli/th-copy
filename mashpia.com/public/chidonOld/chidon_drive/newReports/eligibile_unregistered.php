...
function getRegInfo() {
    ...
    $sqlReg = "select u.user_id, u.user_serial, u.first, u.last, u.school_id, u.class_id, c.class_grade, tc.th_chidon_id, 
            ...
            where (tc.date_paid is null or tc.date_paid = '' or tc.date_paid = 0) 
            and tc.year = " . $year;
...
<table>
    ...
    foreach ($reg as $row) {
        ...
        $phone = '';
        $phone .= '<a target="_blank" href="https://api.whatsapp.com/send/?phone=' . (strlen($mobile) == 10 ? '1' . $mobile : $mobile) . '">' . $mobile . '</a>';
