...
// sort by avg
foreach ( $userInfo as $school => $more ) {
    foreach ( $more as $gender => $other ) {
        foreach ( $other as $grade => $avgs ) {
            krsort( $userInfo[$school][$gender][$grade] );
        }
    }
}
...
<HTML>
    ...
    <script>        
        ...
        $(".trophy").click( function() {
            var id = $(this).parent().parent().attr('id');
            var val = $(this).is(":checked") ? 1 : 0;
            $.post('ajax/updateChidon.php', { id : id, field : 'trophy_contestant', val : val }, function( error ) {
                if (parseInt(error)) {
                    alert("Error Updating.");
                } else {
                    //alert("Updated.");
