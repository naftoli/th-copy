...
// Birthday cache: user_id => [mission_id => true]. Set by printDuchAll before the mission loop.
...
class user_track_for_duch 
{
	...
	/** @return array mission_id => true for this user (from global $birthday_cache) */
	private function _birthday_mission_ids() {
		$uid = (int) $this->user_id;
		return $GLOBALS['birthday_cache'][ $uid ] ?? [];
	}
                                     ...
                                     $print_parent_tasks = true, $chidonLimmud = false)
	{
		...
		foreach ( $all_date_tasks_missions[ $this->subject_id ][ $school_type_id ][ $lang ][ $this->level ][ $this->track_id ] as $row ) {
			...
			if ( strpos( $row['mission_name'], 'Birthday!' ) !== false || strpos( $row['mission_description'], 'יום הולדת' ) !== false ) {
				if ( ! $allowPersonalization || ! isset( $birthday_ids[ (int) $row['date_tasks_mission_id'] ] ) ) {
					continue;
				}
			}
