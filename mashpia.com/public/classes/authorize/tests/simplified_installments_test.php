<?php
/**
 * Simplified test script for the Installments class
 */
...
echo "<h2>Simplified Installments Test</h2>";
...
try {
    ...
    $installments->setupInstallmentPlan($total_amount, $num_installments, $start_date);
    
    // Step 4: Create the subscription
    debug_print("Creating subscription");
    $result = $installments->createSubscription();
    
    debug_print("Subscription creation result", $result);
    
    if (strpos($result, "Success") !== false) {
        debug_print("Subscription created successfully with ID", $installments->subscription_id);
    } else {
