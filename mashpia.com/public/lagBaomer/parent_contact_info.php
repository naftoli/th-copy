...
$db_fields = ['admin_id', ['first', 'last'], 'mother', 'father', 'admin_phone_home', 'admin_phone_mobile2', 'admin_phone_mobile', 'admin_email', 'user_serial', 'child_first', 'child_last', 'school_name', 'rank_name'];


$users = [];
// create sql statement
$sql = "select a.*, u.user_serial, u.first as child_first, u.last as child_last, s.school_name, r.rank_name, MAX( rm.rank_ord ) as rank_ord 
        from users u
            ...
            <?php foreach ( $users as $user ) : ?>
                <tr>
                    <?php foreach ( $db_fields as $field ) : ?>
                        <td><?= is_array( $field ) ? $user[$field[0]] . ' ' . $user[$field[1]] : $user[$field] ?></td>
                    <?php endforeach; ?>
                </tr>
            <?php endforeach; ?>
        </table>
