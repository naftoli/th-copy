...
class Soldier extends \ActiveRecord\Model implements \JsonSerializable {
    ...
    public function parentAccount() {
        global $MASHPIA_DB;
        $query = $MASHPIA_DB->prepare(
            'SELECT admin_id, first, father, mother, last, admin_phone_mobile AS phone, admin_phone_mobile2 AS phone2, username, 
...
//        }
    ...
    //    if (!isset($_COOKIE['naftoli']) && self::turnOffChidon() && !in_array($this->school_id, [66, 110, 112, 180])) {
...
//            echo $reg_query->debugDumpParams();
            ...
            if ($ckids) $type = 'ckids';
            if (!$this->registrationCharge($type, $amount, $trans_id, $year, $discount)) {
...
//        $wpb = new WpBirthday( $this->user_id );  @$wpb->syncToWp();
    ...
    public function getTransactions($from, $to, $types) {
        $p = new Points($this->user_id);
        return $p->getHistory($from, $to, $types);
    }
}
