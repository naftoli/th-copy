...
<div class="ui_<?= $ui_type ?> <?= $align_start ?>">
  <div class="body">
            ...
            <label><?= T_('Select Institution') ?>:
              <select name="school_id">
                  <? while ($school_row = mysql_fetch_assoc($school_result)) { ?>
                    <option
                      value="<?= $school_row['school_id'] ?>" <?= $school_row['school_id'] == $school_id ? 'selected' : '' ?>><?= es($school_row['school_name']) ?></option>
                  <? } ?>
              </select>
            </label>
    ...
    <div class="infobox" style="line-height: 1.2">
      <i>This feature (to change ladders) is only available the week before Shabbos Mevorchim, starting the Sunday of
        that week.</i><br/>
      Click <a href="https://docs.google.com/document/d/1fGI4Dmd3hoD03dJhrSXOoThzEcb_IjIi22c-vLQorWg/edit?usp=sharing">here</a>
      for the SMT Manual.
    </div>
    <br/>
                ...
                <p>
                  ...
                  <label><?= T_('Show only Platoon') ?>:
                    <select name="class_id">
                      ...
                      <option
                        value="<?= $class_row['class_id'] ?>" <?= $class_row['class_id'] == $class_id ? 'SELECTED' : '' ?>><?= es($class_row['class_grade']) ?>
                        -<?= es($class_row['class_sub']) ?></option>
                        <? } ?>
                    </select>
                  </label><BR>
                      ...
                      <!--
                        ...
                        <? while ($row = mysql_fetch_assoc($edit_result)) { ?>
                          ...
                          <tr>
                            ...
                            <td>
                              <select
                                name="tracks[<?= $row['user_id'] ?>][<?= $row['subject_id'] ?>][track]" <?php if (!$showTehillimQuota) echo "disabled" ?>>
                                <option value="-1">&lt;<?= T_('Subject Disabled') ?>&gt;
                                    <? while ($track_row = mysql_fetch_assoc($tracks_result)) { ?>
                                    <? if ($track_row['track_name'] == 6) break; ?>
                                    <? if ($track_row['track_name'] == 10) continue; ?>
                                <option
                                  value="<?= $track_row['track_id'] ?>" <?= $track_row['track_id'] == ($rowTrack['track_id'] - 2) ? 'SELECTED' : '' ?>><?= es($track_row['track_name']) ?></option>
                                  <? } // end while loop?>
                                  <? mysql_data_seek($tracks_result, 0); ?>
                              </select>
                              <br/>
                            ...
                            <input type="hidden" name="tracks[<?= $row['user_id'] ?>][<?= $row['subject_id'] ?>][level]"
                                   value="<?= es($level) ?>"/>
                          </tr>
                        <? } ?>
                    </table>
