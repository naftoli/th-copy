...
<div align='center' class='hide'>
  ...
  <input type='button' value='Download CSV' onclick='window.location.href="?download=csv";'/>
</div>
<?php
...
if (isset($_GET['download']) && $_GET['download'] === 'csv') {
    ...
    // Add headers
    fputcsv($output, array('School Name', 'Shipping Name', 'Address', 'Type', 'Principal', 'Total Teachers', 'Total Children', 'Total Hachayol', 'Chidon Registered', 'Chayolei Boys Posters', 'Chayolei Girls Posters', 'Chayolei Both Posters', 'Chidon Boys Posters', 'Chidon Girls Posters', 'Chidon Both Posters'));
        ...
        foreach ($more as $type => $other) {
            foreach ($other as $school_id => $school) {
                $row = array(
                    ...
                    $total,
                    $school['chidonReg'],
                    $posters[$school_id]['chayolei_b'] ?? 0,
                    $posters[$school_id]['chayolei_g'] ?? 0,
                    $posters[$school_id]['chayolei_bg'] ?? 0,
                    $posters[$school_id]['chidon_b'] ?? 0,
                    $posters[$school_id]['chidon_g'] ?? 0,
                    $posters[$school_id]['chidon_bg'] ?? 0
                );
                fputcsv($output, $row);
            }
        }
