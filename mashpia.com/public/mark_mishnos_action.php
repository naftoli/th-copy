...
$users = isset( $_POST['student'] ) ? $_POST['student'] : array();
...
//echo "<pre>"; print_r($perek); print_r($mishna); print_r($lines); echo "</pre>"; exit;
/*
$users = array();
if ($user > 0) {
	$users[] = $user;
	...
	for ($i = 0; $i < $numRows; $i++) {
		...
		try {
			...
			if (isset($_POST['parent']) && $_POST['parent'] == 1) {
				header("Location: parent_mark_mishnos.php?msg=" . urlencode($msg));
				exit;
			} else {
				header("Location: mark_mishnos.php?msg=" . urlencode($msg));
				exit;
			}
		}
		*/
...
if ($success) {
	...
	if (isset($_POST['parent']) && $_POST['parent'] == 1) {
		header("Location: parent_mark_mishnos.php?msg=y");
		exit;
	} else {
		header("Location: mark_mishnos.php?msg=y&school=$school&grade=$grade");
		exit;
	}
} else {
