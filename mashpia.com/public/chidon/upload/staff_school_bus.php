...
if (($handle = fopen($_FILES['file']['tmp_name'], "r")) !== FALSE) {
  ...
  while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
    if ( $row++ < 1 ) continue;
    if ( !empty( $data[1] ) ) $qry = "update th_chidon_staff set school_bus = '" . $data[1] . "' where staff_id = " . $data[0];
    $qrys[] = $qry;
  }
  fclose($handle);
}
...
<html>
  ...
  <body>
    <form action="staff_school_bus.php" method="post" accept-charset="UTF-8" enctype="multipart/form-data">
