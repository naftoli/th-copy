...
if (($handle = fopen($_FILES['file']['tmp_name'], "r")) !== FALSE) {
  ...
  while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
    if ( $row++ < 1 ) continue;
    $qry = "update th_chidon set seat_number = " . intval( $data[1] ) . " where th_chidon_id = " . intval( $data[0] );
    $qrys[] = $qry;
  }
  fclose($handle);
}
...
<html>
  ...
  <body>
    <form action="stage_seats.php" method="post" accept-charset="UTF-8" enctype="multipart/form-data">
