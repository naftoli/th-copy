...
/**
 * Handles WPDesk tracker actions.
 */
class Flexible_Shipping_UPS_Tracker {
	...
	private function prepare_default_plugin_data( Flexible_Shipping_UPS_Shipping_Method $flexible_shipping_ups ) {
		if ( $flexible_shipping_ups->custom_origin() ) {
			$ups_origin_country = explode( ':', $flexible_shipping_ups->get_option( 'origin_country', '' ) );
		} else {
		...
		if ( isset( $shipping_methods['flexible_shipping_ups'] ) ) {
			...
			foreach ( $shipping_zones as $zone_data ) {
				...
				foreach ( $shipping_methods as $shipping_method ) {
					if ( 'flexible_shipping_ups' === $shipping_method->id && $shipping_method instanceof Flexible_Shipping_UPS_Shipping_Method ) {
	...
	public function wpdesk_tracker_notice_screens( $screens ) {
		...
		if ( $current_screen instanceof WP_Screen ) {
			if ( 'woocommerce_page_wc-settings' === $current_screen->id ) {
				if ( isset( $_GET['tab'] ) && 'shipping' === $_GET['tab'] && isset( $_GET['section'] ) && 'flexible_shipping_ups' === $_GET['section'] ) { // WPCS: Input var okay. CSRF ok.
...
if ( ! function_exists( 'wpdesk_tracker_enabled' ) ) {
	...
	function wpdesk_tracker_enabled() {
		...
		if ( ! empty( $_SERVER['SERVER_ADDR'] ) && '127.0.0.1' === $_SERVER['SERVER_ADDR'] ) { // WPCS: Input var okay.
