...
if ( ! class_exists( 'Flexible_Shipping_UPS_Access_Points_Helper' ) ) {
	...
	class Flexible_Shipping_UPS_Access_Points_Helper {
		...
		public function get_access_points_for_postcode( $country, $postcode, $max_number_of_points = 0 ) {
			...
			$transient_name = self::TRANSIENT_NAME_FOR_NEAREST_ACCESS_POINTS . '_' . sanitize_title( $country ) . '_' . sanitize_title( $postcode ) . '_' . sanitize_title( (string) $max_number_of_points );
			...
			if ( !$locations ) {
				...
				$acccess_point_search = new \UpsFreeVendor\Ups\Entity\AccessPointSearch;
				$acccess_point_search->setAccessPointStatus( \UpsFreeVendor\Ups\Entity\AccessPointSearch::STATUS_ACTIVE_AVAILABLE );
				...
				$locations = $locator->getLocations( $locator_request, \UpsFreeVendor\Ups\Locator::OPTION_UPS_ACCESS_POINT_LOCATIONS );
		 ...
		 *
		 * @param string $access_point_id Access point ID.
		 * @param string $country_code Country code.
		 *
		 * @return mixed|stdClass
		 * @throws Exception Exception.
		 */
		public function get_access_point_by_id( $access_point_id, $country_code ) {
			...
			if ( false === $locations ) {
				...
				try {
					$locations = $locator->getLocations(
						$locator_request,
						\UpsFreeVendor\Ups\Locator::OPTION_UPS_ACCESS_POINT_LOCATIONS
					);
				} catch ( Exception $e ) {
					return null;
				}
