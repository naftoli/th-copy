 ...
 * Displys rate notice.
 */
class Flexible_Shipping_UPS_Rate_Notice implements \UpsFreeVendor\WPDesk\PluginBuilder\Plugin\Hookable {
	 ...
	 * @param string $source .
	 */
	public function maybe_reset_orders_counter( $notice_name, $source = null ) {
		if ( 'ups_rating' === $notice_name && ( empty( $source ) || self::CLOSE_TEMPORARY_NOTICE === $source ) ) {
			delete_option( Flexible_Shipping_UPS_Order_Counter::FS_UPS_COUNTER );
	...
	public function maybe_show_notice() {
		$order_counter = intval( get_option( Flexible_Shipping_UPS_Order_Counter::FS_UPS_COUNTER, '0' ) );
