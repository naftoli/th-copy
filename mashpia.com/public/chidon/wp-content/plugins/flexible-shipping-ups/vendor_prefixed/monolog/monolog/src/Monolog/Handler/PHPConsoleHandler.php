...
use Exception;
use UpsFreeVendor\Monolog\Formatter\LineFormatter;
use UpsFreeVendor\Monolog\Logger;
use UpsFreeVendor\PhpConsole\Connector;
use UpsFreeVendor\PhpConsole\Handler;
use UpsFreeVendor\PhpConsole\Helper;
/**
 ...
 */
class PHPConsoleHandler extends \UpsFreeVendor\Monolog\Handler\AbstractProcessingHandler
{
     ...
     * @throws Exception
     */
    public function __construct(array $options = array(), \UpsFreeVendor\PhpConsole\Connector $connector = null, $level = \UpsFreeVendor\Monolog\Logger::DEBUG, $bubble = \true)
    {
        if (!\class_exists('UpsFreeVendor\\PhpConsole\\Connector')) {
    ...
    private function initConnector(\UpsFreeVendor\PhpConsole\Connector $connector = null)
    {
        if (!$connector) {
            if ($this->options['dataStorage']) {
                \UpsFreeVendor\PhpConsole\Connector::setPostponeStorage($this->options['dataStorage']);
            }
            $connector = \UpsFreeVendor\PhpConsole\Connector::getInstance();
        }
        if ($this->options['registerHelper'] && !\UpsFreeVendor\PhpConsole\Helper::isRegistered()) {
            \UpsFreeVendor\PhpConsole\Helper::register();
        }
        if ($this->options['enabled'] && $connector->isActiveClient()) {
            if ($this->options['useOwnErrorsHandler'] || $this->options['useOwnExceptionsHandler']) {
                $handler = \UpsFreeVendor\PhpConsole\Handler::getInstance();
    ...
    {
        if ($record['level'] < \UpsFreeVendor\Monolog\Logger::NOTICE) {
    ...
    {
        return new \UpsFreeVendor\Monolog\Formatter\LineFormatter('%message%');
    }
}
