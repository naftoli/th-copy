 ...
 */
namespace UpsFreeVendor\Monolog;

use Psr\Log\LoggerInterface;
use Psr\Log\LogLevel;
use UpsFreeVendor\Monolog\Handler\AbstractHandler;
...
class ErrorHandler
{
    ...
    public static function register(\Psr\Log\LoggerInterface $logger, $errorLevelMap = array(), $exceptionLevel = null, $fatalLevel = null)
    {
        ...
        \class_exists('UpsFreeVendor\\Psr\\Log\\LogLevel', \true);
    ...
    {
        $this->logger->log($this->uncaughtExceptionLevel === null ? \Psr\Log\LogLevel::ERROR : $this->uncaughtExceptionLevel, \sprintf('Uncaught Exception %s: "%s" at %s line %s', \UpsFreeVendor\Monolog\Utils::getClass($e), $e->getMessage(), $e->getFile(), $e->getLine()), array('exception' => $e));
    ...
    public function handleFatalError()
    {
        ...
        if ($lastError && \in_array($lastError['type'], self::$fatalErrors, \true)) {
            $this->logger->log($this->fatalLevel === null ? \Psr\Log\LogLevel::ALERT : $this->fatalLevel, 'Fatal Error (' . self::codeToString($lastError['type']) . '): ' . $lastError['message'], array('code' => $lastError['type'], 'message' => $lastError['message'], 'file' => $lastError['file'], 'line' => $lastError['line'], 'trace' => $this->lastFatalTrace));
            if ($this->logger instanceof \UpsFreeVendor\Monolog\Logger) {
                foreach ($this->logger->getHandlers() as $handler) {
                    if ($handler instanceof \UpsFreeVendor\Monolog\Handler\AbstractHandler) {
                        $handler->close();
                    }
                }
            }
