...
class Shipping extends \UpsFreeVendor\Ups\Ups
{
     ...
     */
    public function __construct($accessKey = null, $userId = null, $password = null, $useIntegration = \false, \UpsFreeVendor\Ups\RequestInterface $request = null, \Psr\Log\LoggerInterface $logger = null)
    {
        if (null !== $request) {
    ...
    private function createConfirmRequest($validation, \UpsFreeVendor\Ups\Entity\Shipment $shipment, \UpsFreeVendor\Ups\Entity\ShipmentRequestLabelSpecification $labelSpec = null, \UpsFreeVendor\Ups\Entity\ShipmentRequestReceiptSpecification $receiptSpec = null)
    {
        ...
        foreach ($shipment->getPackages() as $package) {
            $shipmentNode->appendChild($xml->importNode($package->toNode($xml), \true));
        }
        ...
        if ($labelSpec) {
            $container->appendChild($xml->importNode($this->compileLabelSpecificationNode($labelSpec), \true));
        }
        ...
        if ($receiptSpec) {
            $container->appendChild($xml->importNode($this->compileReceiptSpecificationNode($receiptSpec), \true));
        }
        return $xml->saveXML();
    }
    /**
    ...
    private function compileReceiptSpecificationNode(\UpsFreeVendor\Ups\Entity\ShipmentRequestReceiptSpecification $receiptSpec)
    {
        ...
        return $receiptSpecNode->cloneNode(\true);
    }
    /**
    ...
    private function compileLabelSpecificationNode(\UpsFreeVendor\Ups\Entity\ShipmentRequestLabelSpecification $labelSpec)
    {
        ...
        return $labelSpecNode->cloneNode(\true);
    }
}
