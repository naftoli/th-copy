 ...
 */
class EachPromise implements \UpsFreeVendor\GuzzleHttp\Promise\PromisorInterface
{
    private $pending = [];
    /** @var \Iterator */
    private $iterable;
    ...
    {
        $this->mutex = \false;
        $this->aggregate = new \UpsFreeVendor\GuzzleHttp\Promise\Promise(function () {
            \reset($this->pending);
            ...
            while ($promise = \current($this->pending)) {
                \next($this->pending);
                $promise->wait();
                if ($this->aggregate->getState() !== \UpsFreeVendor\GuzzleHttp\Promise\PromiseInterface::PENDING) {
                    return;
                }
            }
        });
