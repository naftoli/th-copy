...
use UpsFreeVendor\GuzzleHttp\HandlerStack;
...
class MockHandler implements \Countable
{
    ...
    public function append()
    {
        foreach (\func_get_args() as $value) {
            if ($value instanceof \Psr\Http\Message\ResponseInterface || $value instanceof \Exception || $value instanceof \UpsFreeVendor\GuzzleHttp\Promise\PromiseInterface || \is_callable($value)) {
                $this->queue[] = $value;
            } else {
    ...
    private function invokeStats(\Psr\Http\Message\RequestInterface $request, array $options, \Psr\Http\Message\ResponseInterface $response = null, $reason = null)
    {
        if (isset($options['on_stats'])) {
            $stats = new \UpsFreeVendor\GuzzleHttp\TransferStats($request, $response, 0, $reason);
