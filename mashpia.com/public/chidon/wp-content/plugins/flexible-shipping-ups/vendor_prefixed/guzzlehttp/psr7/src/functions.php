...
function uri_for($uri)
{
    if ($uri instanceof \Psr\Http\Message\UriInterface) {
        ...
        return new \UpsFreeVendor\GuzzleHttp\Psr7\Uri($uri);
    }
...
function stream_for($resource = '', array $options = [])
{
    if (\is_scalar($resource)) {
        ...
        return new \UpsFreeVendor\GuzzleHttp\Psr7\Stream($stream, $options);
    }
    switch (\gettype($resource)) {
        case 'resource':
            return new \UpsFreeVendor\GuzzleHttp\Psr7\Stream($resource, $options);
        case 'object':
            if ($resource instanceof \Psr\Http\Message\StreamInterface) {
                return $resource;
            } elseif ($resource instanceof \Iterator) {
                return new \UpsFreeVendor\GuzzleHttp\Psr7\PumpStream(function () use($resource) {
                    if (!$resource->valid()) {
                        return \false;
                    }
            ...
            break;
        case 'NULL':
            return new \UpsFreeVendor\GuzzleHttp\Psr7\Stream(\fopen('php://temp', 'r+'), $options);
    }
    if (\is_callable($resource)) {
        return new \UpsFreeVendor\GuzzleHttp\Psr7\PumpStream($resource, $options);
    }
...
function modify_request(\Psr\Http\Message\RequestInterface $request, array $changes)
{
    ...
    if ($request instanceof \Psr\Http\Message\ServerRequestInterface) {
        return (new \UpsFreeVendor\GuzzleHttp\Psr7\ServerRequest(isset($changes['method']) ? $changes['method'] : $request->getMethod(), $uri, $headers, isset($changes['body']) ? $changes['body'] : $request->getBody(), isset($changes['version']) ? $changes['version'] : $request->getProtocolVersion(), $request->getServerParams()))->withParsedBody($request->getParsedBody())->withQueryParams($request->getQueryParams())->withCookieParams($request->getCookieParams())->withUploadedFiles($request->getUploadedFiles());
    }
    return new \UpsFreeVendor\GuzzleHttp\Psr7\Request(isset($changes['method']) ? $changes['method'] : $request->getMethod(), $uri, $headers, isset($changes['body']) ? $changes['body'] : $request->getBody(), isset($changes['version']) ? $changes['version'] : $request->getProtocolVersion());
}
/**
...
function parse_request($message)
{
    ...
    $request = new \UpsFreeVendor\GuzzleHttp\Psr7\Request($parts[0], $matches[1] === '/' ? _parse_request_uri($parts[1], $data['headers']) : $parts[1], $data['headers'], $data['body'], $version);
...
function parse_response($message)
{
    ...
    $parts = \explode(' ', $data['start-line'], 3);
    return new \UpsFreeVendor\GuzzleHttp\Psr7\Response($parts[1], $data['headers'], $data['body'], \explode('/', $parts[0])[1], isset($parts[2]) ? $parts[2] : null);
}
/**
...
function _parse_message($message)
{
    ...
    if (\preg_match("/(?:^HTTP\\/|^[A-Z]+ \\S+ HTTP\\/)(\\d+(?:\\.\\d+)?)/i", $startLine, $matches) && $matches[1] === '1.0') {
        ...
        $rawHeaders = \preg_replace(\UpsFreeVendor\GuzzleHttp\Psr7\Rfc7230::HEADER_FOLD_REGEX, ' ', $rawHeaders);
    }
    /** @var array[] $headerLines */
    $count = \preg_match_all(\UpsFreeVendor\GuzzleHttp\Psr7\Rfc7230::HEADER_REGEX, $rawHeaders, $headerLines, \PREG_SET_ORDER);
    // If these aren't the same, then one line didn't match and there's an invalid header.
    if ($count !== \substr_count($rawHeaders, "\n")) {
        // Folding is deprecated, see https://tools.ietf.org/html/rfc7230#section-3.2.4
        if (\preg_match(\UpsFreeVendor\GuzzleHttp\Psr7\Rfc7230::HEADER_FOLD_REGEX, $rawHeaders)) {
