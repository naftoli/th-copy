...
class S214_Settings
{
    ...
    public function __construct($slug = \false, $default_tab = 'general')
    {
        ...
        if (!\class_exists('UpsFreeVendor\\S214_Sysinfo')) {
            require_once 'modules/sysinfo/class.s214-sysinfo.php';
        }
        $this->sysinfo = new \UpsFreeVendor\S214_Sysinfo($this->slug, $this->func, $this->version);
    }
    /**
     * Run action and filter hooks
     *
    ...
    public function add_settings_page()
    {
        ...
        $menu = \apply_filters($this->func . '_menu', array('type' => 'menu', 'parent' => 'options-general.php', 'page_title' => \__('Section214 Settings', 's214'), 'show_title' => \false, 'menu_title' => \__('Section214 Settings', 's214'), 'capability' => 'manage_options', 'icon' => '', 'position' => null));
    ...
    function register_settings()
    {
        ...
        foreach ($this->get_registered_settings() as $tab => $sections) {
            foreach ($sections as $section => $settings) {
                ...
                \add_settings_section($this->func . '_settings_' . $tab . '_' . $section, \__return_null(), '__return_false', $this->func . '_settings_' . $tab . '_' . $section);
    ...
    public function settings_sanitize($input = array())
    {
        ...
        foreach ($setting_types as $key => $type) {
            ...
            $non_setting_types = \apply_filters($this->func . '_non_setting_types', array('header', 'descriptive_text', 'hook'));
    ...
    {
        return \trim(\wp_strip_all_tags($input, \true));
    }
    /**
     * Header callback
     *
    ...
    public function editor_callback($args)
    {
        ...
        $wpautop = isset($args['wpautop']) ? $args['wpautop'] : \true;
        $buttons = isset($args['buttons']) ? $args['buttons'] : \true;
        $teeny = isset($args['teeny']) ? $args['teeny'] : \false;
        \wp_editor($value, $this->func . '_settings_' . $args['id'], array('wpautop' => $wpautop, 'media_buttons' => $buttons, 'textarea_name' => $this->func . '_settings[' . $args['id'] . ']', 'textarea_rows' => $rows, 'teeny' => $teeny));
        ...
        if (!empty($args['options'])) {
            ...
            foreach ($args['options'] as $key => $option) {
                if (isset(${$this->func . '_options'}[$args['id']][$key])) {
                    ...
                    $enabled = isset($args['std'][$key]) ? $args['std'][$key] : \UpsFreeVendor\NULL;
                }
    ...
    public function radio_callback($args)
    {
        ...
        if (!empty($args['options'])) {
            ...
            foreach ($args['options'] as $key => $option) {
                ...
                $html .= '<input name="' . $this->func . '_settings[' . $args['id'] . ']" id="' . $this->func . '_settings[' . $args['id'] . '][' . $key . ']" type="radio" value="' . $key . '" ' . \checked(\true, $checked, \false) . '/>&nbsp;';
                $html .= '<label for="' . $this->func . '_settings[' . $args['id'] . '][' . $key . ']">' . $option . '</label><br />';
            }
            $html .= '<p class="description">' . $args['desc'] . '</p>';
            echo \apply_filters($this->func . '_after_setting_output', $html, $args);
    ...
    public function enqueue_scripts($hook)
    {
        ...
        $url_path = \str_replace(\WP_CONTENT_DIR, \UpsFreeVendor\WP_CONTENT_URL, \dirname(__FILE__));
        ...
        \wp_localize_script($this->slug . '-s214-settings', 's214_settings_vars', \apply_filters($this->func . 'localize_script', array('func' => $this->func, 'image_media_button' => \__('Insert Image', 's214-settings'), 'image_media_title' => \__('Select Image', 's214-settings'))));
    }
    /**
     * Add tooltips
     *
