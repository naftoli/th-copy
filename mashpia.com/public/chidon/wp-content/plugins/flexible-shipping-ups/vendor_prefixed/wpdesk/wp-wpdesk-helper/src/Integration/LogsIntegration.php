 ...
 */
class LogsIntegration implements \UpsFreeVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
    ...
    /** @var Logger */
    private $logger;
    public function __construct(\UpsFreeVendor\WPDesk\Helper\Page\SettingsPage $settings_page)
    {
    ...
    {
        $logger_factory = new \UpsFreeVendor\WPDesk\Logger\WPDeskLoggerFactory();
        ...
        if (\property_exists($logger_factory, 'shouldLoggerBeActivated')) {
            ...
            $this->logger = new \UpsFreeVendor\Monolog\Logger('fallback');
        }
    }
    private function maybe_show_logging_notice()
    {
        ...
        if ($debug_log_enabled) {
            if (\apply_filters('wpdesk_helper_show_log_notices_library', \true)) {
                new \UpsFreeVendor\WPDesk\Notice\Notice(\sprintf(
                    // Translators: link.
