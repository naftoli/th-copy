...
if (!\class_exists('UpsFreeVendor\\WPDesk_Logger')) {
    /**
    ...
    class WPDesk_Logger
    {
        ...
        {
            if (!self::$logger) {
                $loggerFactroy = new \UpsFreeVendor\WPDesk\Logger\WPDeskLoggerFactory();
        ...
        {
            if (\defined('UpsFreeVendor\\WC_VERSION')) {
                if (\version_compare(\UpsFreeVendor\WC_VERSION, '3.0', '<')) {
