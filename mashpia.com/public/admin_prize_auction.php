		...
		case 'edit2':
			...
			if ($admin_user['auth'] == 'super' || mysql_num_rows(mq("SELECT prize_id FROM prizes_auction WHERE prize_id = $prize_id AND school_id IN (" . implode(',', $admin_user['auths']['school']) . ')'))) {
				...
				if(mysql_num_rows(mq('SELECT 1 FROM prizes_auction WHERE prize_name = ' . ms($prize_name) . " AND school_id <=> $school_id AND prize_id != $prize_id"))) {
					...
					$result = mq("SELECT $prize_id prize_id, $school_id school_id, " . ms($prize_name) . ' prize_name, ' . nullif_ms(gr('min_grade'), '') . ' min_grade, ' . nullif_ms(gr('max_grade'), '') . ' max_grade, ' . ms(gr('prize_description')) . " prize_description, $prize_points prize_points, $prize_ratio prize_ratio, prize_image_id, in_stock FROM prizes_auction WHERE prize_id = $prize_id");
					$edit_row = mysql_fetch_assoc($result);
					$action = 'edit';
				} 
				else {
...
<!--						--><?php //=T_('Ratio')?>
<!--						<BR>-->
<!--						<INPUT type="text" name="prize_ratio" maxlength=5 value="--><?php //=$edit_row['prize_ratio']?><!--">-->
<!--					</LABEL>-->
<!--					-->
<!--					<BR>-->
            <input type="hidden" name="prize_ratio" value="1" />
...
<!--							 <OPTION --><?php //=$grade == $edit_row['min_grade'] ? 'SELECTED' : ''?><!--<?php //=es($grade)?></OPTION> -->
<!--							--><?// //endforeach; ?>
<!--						</SELECT>-->
<!--					</LABEL>-->
<!--					<BR>-->
...
<!--							 <OPTION --><?php //=$grade == $edit_row['max_grade'] ? 'SELECTED' : ''?><!--<?php //=es($grade)?></OPTION> -->
<!--							--><?// //endforeach; ?>
<!--						</SELECT>-->
<!--					</LABEL>-->
<!--					-->
...
<!--							<OPTION value="-1">&lt;--><?php //=T_('Multi institution prize')?><!--&gt;</OPTION>-->
<!--							--><?// while($school_row = mysql_fetch_assoc($school_result)) : ?>
<!--							<OPTION value="--><?php //=$school_row['school_id']?><!--" --><?php //=$school_row['school_id'] == $edit_row['school_id'] ? 'SELECTED' : ''?><!--<?php //=es($school_row['inst_name'])?> --><?php //=es($school_row['school_name'])?><!--</OPTION>-->
<!--							--><?// endwhile; ?>
<!--						</SELECT>-->
<!--					</LABEL>-->
<!--					-->
...
<?else:?>
			...
			<?$result = mq('SELECT prize_id, shipping_code, prize_name, prize_points, prize_ratio, prize_image_id, min_grade, max_grade, in_stock, archived FROM prizes_auction ORDER BY archived, prize_id');?>
