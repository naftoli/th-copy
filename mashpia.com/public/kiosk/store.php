...
$user_store_miles = $camper->total_points;
...
$prize_price = 0;
if (isset($_POST['prize_price'])) {
	$prize_price = $_POST['prize_price'];
}
// ***** TOTAL PRICE ***** //
...
if ($prize_id > 0) {
	...
	if ($row['store_purchase_id'] > 0) {	
		...
		$sql = $sql . $user_id . ", " . $prize_id . ", 0, " . $prize_price . ", " . $quantity . ")";
		mq($sql);
	}
...
// ********** CART ITEMS ********** //
$cart_items = array();
$sql = "SELECT cp.prize_id, ps.prize_name, ps.prize_image_id, cp.prize_quantity FROM store_purchases AS cp JOIN prizes_camp AS ps USING (prize_id) WHERE user_id=" . $user_id . " AND prize_shipped=0";
//echo $sql . "<br />";
$query = mysql_query($sql);
...
FROM users
     ...
     LEFT JOIN (SELECT user_id, MAX(rank_ord) rank_ord FROM rank_marks WHERE user_id = " . $user_id . " GROUP BY user_id) rank USING (user_id)
...
function get_store_prize_points($camp_id) {
	global $prize_points;
	
	$echo_string = "";
	
	$sql = "SELECT prize_points FROM prizes_camp ";
	$sql = $sql . "WHERE camp_id = " . $camp_id . " AND installed=1 GROUP BY prize_points ORDER BY prize_points";
...
function get_prizes_store($camp_id, $prize_points) {	
	global $user_id;
	
	$sql = "SELECT ps.prize_id, ps.prize_points, ps.prize_name, ps.prize_image_id, sp.prize_quantity FROM prizes_camp AS ps LEFT JOIN store_purchases AS sp ON user_id=" . $user_id ." AND ps.prize_id=sp.prize_id AND sp.prize_shipped=0 WHERE camp_id=" . $camp_id . " AND installed = 1 ORDER BY prize_points";
	...
	<HEAD>
		...
		<SCRIPT type="text/javascript">
			...
			function update_points(quantity, price, hidden_field, id, prize_available) {
				...
				alert("user_store_miles:" + user_store_miles);
...
<?php
$row_num = 0;
$sql = "SELECT ps.*, cp.prize_quantity AS quantity FROM prizes_camp AS ps LEFT JOIN store_purchases AS cp ON user_id=" . $user_id . " AND ps.prize_id=cp.prize_id AND cp.prize_shipped=0";
$query = mysql_query($sql);
						...
						<div class="tabs_pane">
							...
							<div class="tabs">
								...
								<ul class="prices tab">
									...
									<? get_store_prize_points($camp_id); ?>							
							...
							<div class="pane prizes">
								...
								<div class="pane_items scroll-pane">
									...
									<? get_prizes_store($camp_id, $prize_points); ?>
