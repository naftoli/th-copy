...
if ($mission>0) {
	...
	$tasks_sql = "SELECT * "; 
	$tasks_sql = $tasks_sql . "FROM date_tasks ";
	$tasks_sql = $tasks_sql . "LEFT JOIN labels ON date_tasks.label_id = labels.label_id ";
	$tasks_sql = $tasks_sql . "LEFT JOIN date_tasks_dates USING (date_task_id) ";
	$tasks_sql = $tasks_sql . "LEFT JOIN date_tasks_marks AS dtm ON (dtm.user_id=" . $user['user_id'] . " AND dtm.date_task_id=date_tasks.date_task_id) ";
	$tasks_sql = $tasks_sql . "WHERE date_tasks.date_tasks_mission_id=" . $mission . " ";
	$tasks_sql = $tasks_sql . "GROUP BY date_tasks.ord ";

	echo "<input type='hidden' name='TASKS SQL' value='" . $tasks_sql . "'>\n";
