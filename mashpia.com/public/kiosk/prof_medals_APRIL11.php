...
{
    global $current_ladder,$current_miss_num,$missions;
...
function dbMissionsToArray($result, $total)
{
    ...
    {     
        ...
        if($i+1 == $z && $i+1<$total)
        {
            ...
            //echo 'i = '.$i.' => y = '.($y).', medal_ord = '.$aresults[$y]["medal_ord"].' '; 
        }
        //echo 'ord = '.$missions[$i].' ';
    }
    return $missions; 
...
function getMedalSum($subj_id)
{
    ...
    $current_date = unixtojd();
    $current_miss_date = 0;
    $total_data = mysql_fetch_assoc(mq("SELECT SUM(missions_required) as total
                                            FROM medals_subjects_totals
                                            WHERE
                                                subject_id = {$subj_id}
                                            ")); 
    ...
    {  
        ...
        else
        {
            ...
            if($mission['end_date'] >= $birth_date || $num_rendered_missions == $total)
            {
...
$sqlWhere = " WHERE medals_subjects.subject_id = subjects.subject_id ";
$sqlWhere = $sqlWhere . " AND medals_subjects.medal_ord = medals.medal_ord ";
//if ($subject_id > -1)
//	$sqlWhere = $sqlWhere . " AND medals_subjects.subject_id = " . $subject_id . " ";
$sql = $sqlSelect . $sqlFrom . $sqlWhere . " ORDER BY subject_id, medal_ord ";
$all_medals_result = mq($sql);
...
<script type="text/javascript">
	...
	function onLoad() {
		//var ol = document.getElementById("controls");
		//var liArray = ol.getElementsByTagName("li");

		//alert("LENGTH:" + liArray.length);
		
		//for (var i = 0; i < liArray.length; i++) {
		//	if (i == 3)
		//		liArray[i].selected = true;
		//}
                        ...
                        { 
                            ...
                            elseif((isset($user_medals[$medal['subject_id']][$medal['medal_ord']]) && $user_medals[$medal['subject_id']][$medal['medal_ord']]))
                            {
                                ...
                                <div class="active">
									...
									<a href="#" onclick="submitForm(<?=$medal['subject_id'];?>, <?=$medal['medal_ord'];?>);"><?=linkImgFile($medal['profile_photo_id'], 96, 100);?></a>
								</div>
                                <?}
                            }
                            else
