...
function get_missions($track_level_school_type) {
	...
	$rows = mysql_query($sql);
	while ($row = mysql_fetch_assoc($rows)) {
		$mission = new mission($row, $current_date);
...
function createCompletedMissionDiv($date_tasks_mission_id, $mission_name, $start_date) {
	...
	//echo "\n\t\t<div class='date'>" . $greg_start_date . "</div>";
...
//echo "MISSIONS REQUIRED:" . $missions_required[$medal_ord]->missions_required . "<br />";
//echo "STARTING MISSION:" . $starting_mission . "<br />";
// ********** MISSIONS REQUIRED ********** //

// ********** TOTAL NUMBER OF TASKS COMPLETED ********** //
//$total_tasks_completed = count_completed_tasks($user_id, $subject_id);
//echo "TASKS COMPLETED:" . $total_tasks_completed . "<br />";
// ********** TOTAL NUMBER OF TASKS COMPLETED ********** //
...
<body class="blue">
	...
	<div id="wrapper">
		...
		<?php
			...
			echo "<input type='hidden' name='INFO' value='" . $medal_ord . ":" . $last_medal . "'>\n";
