	...
	public function award_big_prizes()
	{
		...
		foreach ($this->big_auction_prizes as $big_auction_prize)
		{
			...
			do {				
				...
				$sql = $sql . "AND u.school_id not in (12,13,14) ";		
...
//				}			
		...
		echo "Total big prizes awarded: " . $big_prizes_awarded . "<br /><br />";
	}
	...
	public function award_small_prizes()
	{
		...
		{	
			...
			$sql = $sql . "AND u.small_prizes_won < 6 ";
			$sql = $sql . "AND u.school_id not in (12,13,14) ";
		...
		foreach ($this->small_auction_prizes as $small_auction_prize)
		{
			...
			do {				
				...
				echo "Prize ID: " . $small_auction_prize->prize_id . "<br />";
				echo "User ID: " . $small_auction_prize->small_auction_user_prizes[$winner]->user_id . "<br />";
				...
				$sql1 = $sql1 . "AND aw.auction_id = " . $this->auction_id;
				//$sql1 = $sql1 . "AND a.auction_date > " . $beginning_of_hebrew_year;

				$query1 = mysql_query($sql1);
				$num_rows1 = mysql_num_rows($query1); 
				echo "Student won this prize this year: " . $num_rows1 . "<br />";
				// ***** Make sure that this student has not already won this prize this year ***** //
				...
				echo "Number of small prizes won this year: " . $small_prizes_won . "<br />";
				// ***** Make sure that this student has not already won 3 small prizes this year ***** //
				
				//find out if user won already a prize this auction
				$won = "select * from auction_winners where user_id = " . $small_auction_prize->small_auction_user_prizes[$winner]->user_id . " and auction_id = " . $this->auction_id;
				$resWon = mysql_query($won);
				$numWon = mysql_num_rows($resWon);
				echo "Number of prizes won this auction: " . $numWon . "<br />";
								
				if (($num_rows1 == 0 || $num_rows1 > 0) && ($small_prizes_won < 10) && (!$this->ratio_reached($school_id)) && ($numWon < 3))
				{
					$insert_sql = "INSERT INTO auction_winners VALUES (" . $this->auction_id . "," . $small_auction_prize->small_auction_user_prizes[$winner]->user_id . "," . $small_auction_prize->prize_id . ",1 ) on duplicate key update quantity = quantity+1";
					//$insert_sql = "INSERT INTO auction_winners VALUES (" . $this->auction_id . "," . $small_auction_prize->small_auction_user_prizes[$winner]->user_id . "," . $small_auction_prize->prize_id . ",1 )";
					echo $insert_sql . "<br />";
					$insert_query = mysql_query($insert_sql);
					$prizes_awarded++;
					...
					echo "Small prizes awarded: " . $small_prizes_awarded . "<br /><br />";
				}
				
			} while ($prizes_awarded < $available);			
		}
		echo "Total small prizes awarded: " . $small_prizes_awarded . "<br />";
	}
	...
	public function ratio_reached($school_id)
	{
		...
		echo "Number of Winners: " . $num_prizes . "<br />";
		echo "Number of Children: " . $num_users . "<br />";
		
		if ($num_prizes > 0) {
			if (round($num_users / $num_prizes) < $this->ratio) {
				echo "Ratio reached" . "<br />";
				return true;
			}
			else {
				echo "Ratio not reached" . "<br />";
				return false;
			}
		} else {
			echo "No Prizes won yet" . "<br />";
			return false;
		}
	}
	*/
	
	public function set_ratio($id) {
		//modified by nr
		//get total users for this school
		$sql = "select * from users where school_id = $id and user_registered > 0";
		$result = mysql_query($sql);
		$school_users = mysql_num_rows($result);
		
		//get total users from all schools		
		$sql = "select * from users u 
				join schools s using (school_id) 
				where u.user_registered > 0 
				and s.school_era is null  
				and s.school_id not in (12,13,14)";
		$result = mysql_query($sql);
		$total_users = mysql_num_rows($result);
		
		//ratio is school users divided by total users
		$this->ratio = round(100 * ($school_users / $total_users))+2;
	}
	
	public function ratio_reached($school_id)
	{	
		$this->set_ratio($school_id);

		//get total number of prizes from this school
		$sql = "select aw.* from auction_winners as aw, users as u 
				where u.user_id = aw.user_id 
				and aw.auction_id = " . $this->auction_id . "
				and u.school_id = $school_id";
		echo $sql . "<br />";
		$result = mysql_query($sql);
		$school_prizes = mysql_num_rows($result);
		
		//get total number of prizes for auction
		$sql = "SELECT SUM(available) FROM  `auction_prizes` WHERE auction_id = " . $this->auction_id;
		$result = mysql_query($sql);
		$row = mysql_fetch_row($result);
		$total_prizes = $row[0];
		...
		echo "School: " . $school_id . "<br />";
		echo "Number of Winners: " . $school_prizes . "<br />";
		echo "Number of Prizes: " . $total_prizes . "<br />";
		
		if ($school_prizes > 0) {
			if (round(100 * ($school_prizes / $total_prizes)) > $this->ratio) {
				echo "Ratio reached" . "<br />";
				return true;
			}
			else {
