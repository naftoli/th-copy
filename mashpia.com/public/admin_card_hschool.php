...
if (!empty($admin_user) && $admin_user['auth'] == 'super' && ($rank_marks = gra('rank_marks'))) {
    ...
    foreach($rank_marks as $user_id => $rank_ord) {
        ...
        if (!mysql_query("UPDATE rank_marks SET date_printed = NOW() WHERE user_id = $user_id AND rank_ord = $rank_ord AND date_printed IS NULL"))
             die(mysql_error());
    }
}
*/
...
$sql1 = "SELECT user_id, first, last, first_he, last_he, username, user_code, user_serial, school_name, school_city, school_state, school_number, school_logo_id, class_grade, class_sub, user_photo_id, dob, dob_he_offset, user_start_date  
        FROM users " . ' 
...
<HTML DIR="<?=$dir?>">
    ...
    <BODY>
        ...
        <script>
            $(document).ready(function() {
                ...
                $('input[name="print"]').click( function() { 
                    var users = $('input[name="userIDs"]').val();
                    var admin = <?=$admin_id?>;
                    var type = '<?=$card_type?>';
                    $.post('ajax/setCardsPrinted.php', {users : [users], admin_id : admin, type : type});
                    window.print();
                });                
            });
        </script>
            ...
            <DIV class="noprint">           
                ...
                <? if (!empty($admin_user)) : ?>
                    ...
                    <? if ($admin_user['auth'] == 'super' || count($admin_user['auths']['school']) != 1) : ?>
                        <? $school_result = mq('SELECT school_id, school_name FROM schools' . ($admin_user['auth'] != 'super' ? ' WHERE school_id IN (' . implode(',', $admin_user['auths']['school']) . ')' : '') . ' where inst_id = 4 ORDER BY school_name'); ?>

                        <FORM action="admin_card_hschool.php" method="get" accept-charset="UTF-8">
                            <P>
        ...
        $user_result = mq("SELECT class_grade, class_sub, user_id, username, first, last FROM users LEFT JOIN classes USING (school_id, class_id) WHERE school_id = $school_id" .  ($class_id != -1 ? " AND class_id = $class_id" : '') . " AND user_start_date IS NOT NULL ORDER BY class_grade, class_sub, last, first");
        ?>
        ...
        <FORM action="admin_card_hschool.php" method="get" accept-charset="UTF-8">
            ...
            <P>                             
                ...
                <input type="checkbox" name="pic"> Include soldier(s) without pictures
                <br />
                <br />
        ...
        <?endif;?>
        <P class="noprint" style="text-align: center;"><INPUT type="button" name="print" value="<?=T_('Print')?>"></P>
        
        <FORM action="admin_card_hschool.php" method="post" accept-charset="UTF-8">
            <!--
            ...
            <? if ($action != "") : ?>
                ...
                <? if (!isset($_GET['pic']) && !$row['user_photo_id'] > 0) continue; ?>
    ...
    <?
                    ...
                    if (!isset($row['rank_color']) || is_null($row['rank_color'])) 
    ...
    ?>
                    ...
                    <TD class="card">
                        ...
                        <DIV class="sig" style="border-color: <?=es($row['rank_color'])?>;">
                            ...
                            <H1><EM style="color: <?=es($row['rank_color'])?>;"></EM> <?=es($row['first_he'] ? $row['first_he'] : $row['first']), ' ', es($row['last_he'] ? $row['last_he'] : $row['last'])?></H1>
                            ...
                            <TABLE cellspacing="0" cellpadding="0">
                                <TR>
                                    ...
                                    <TD class="photo" width="1">
                                        ...
                                        <? else : ?>
                                        <img src="" width="1" class="noBorder"></img>
                                        <? endif; ?>
