...
foreach ($schools as $id => $school) { 
	...
	$sql = "select teacher_hname, class_grade, class_sub from classes 
			where school_id = $id 
			and class_era = 0";
	$result = mysql_query($sql);
	while ($row = mysql_fetch_assoc($result)) {
		if (!empty($row['teacher_hname'])) {
			$names[$row['class_grade'] . '-' . $row['class_sub']][] = $row['teacher_hname'];
		}
	}
	
	$sql = "select u.he_name, c.class_grade, c.class_sub from users u 
	...
	while ($row = mysql_fetch_assoc($result)) {
		if (!empty($row['he_name'])) {
			$names[$row['class_grade'] . '-' . $row['class_sub']][] = $row['he_name'];
		}
	}
...
<html>
	...
	<body>
		...
		foreach ($info as $school => $names) {
			...
			echo "<div style='clear: both; page-break-after: always'></div><br />";
			
			foreach ($names['names'] as $grade => $rest) {
				echo "Grade: " . $grade . "<br />";
