...
if ( isset( $_POST['school'] ) && isset( $_POST['grade'] ) ) {
  ...
  $userIDs = [];
  $sql = "SELECT user_id FROM users WHERE school_id = 269 AND (user_registered > 0 OR yan = 1)";
  $result = mysql_query( $sql );
  while ( $row = mysql_fetch_assoc( $result ) ) {
    $userIDs[] = $row['user_id'];
  }

  $diff = array_diff( $user_ids, $userIDs );
  echo "<pre>"; print_r( $diff ); echo "</pre>"; 

  $sql = "select sum(lines_learned) as total from lines_learned where campaign_id = 13 and user_id in (" . implode(',', $diff) . ")";
  $result = mysql_query( $sql );
...
<html>
  ...
  <p><?= $totalKids ?></p>
	...
	<? else : ?>
		<form action='editSoldierLines3.php<?=$debug ? "?debug=true" : ""?>' method="post">				
