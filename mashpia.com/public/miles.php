...
if ($admin_auth[0] == 'school') { 
    ...
    if ( $numSchools > 1 && !isset( $_POST['submit'] ) ) {
        ...
        if ( ($numSchools == 1 && !isset( $_POST['submit'] )) || (isset( $_POST['school'] ) && !isset( $_POST['class'] ) ) ) {
            ...
            //$points = header_store_points( $codes );
            ...
            foreach ( $user_classes as $grade => $arr ) {
            	...
            	foreach ( $arr as $user ) {
            		...
            		//$cur_points = floatval(mysql_result(mq(totalMarks("WHERE user_id = $user_id and mark_date > 2457629")), 0));
					//echo "<input type='hidden' name='" . $user_id . "M' value='" . $cur_points . "' />";
					//echo "<input type='hidden' name='" . $user_id . "V2' value='" . $points[$user['user_code']] . "' />";
            		//$cur_points += isset( $points[$user['user_code']] ) ? $points[$user['user_code']] : 0 ;
					$p = new Points( $user_id );
					$cur_points = $p->getStorePoints();
            		echo "<tr><td>" . $grade . "</td><td>" . $user['last'] . "</td><td>" . $user['first'] . "</td><td>" . floor($cur_points) . "</td></tr>";
            	} 
