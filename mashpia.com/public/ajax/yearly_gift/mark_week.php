...
if (mysql_num_rows($query) > 0){ // if we do, just update the row
    $sql = "UPDATE user_yearly_gift SET marked=$marked WHERE user_id=$user_id AND start_date = $start_date AND end_date = $end_date";
...
if (!$result) { // check if the query failed
    echo json_encode([success => false, error => "Could not save mark. Please contact support and provide details on what you where attempting to mark"]);
    die(); // the result was returned. kill the script now
} else {
    echo json_encode([success => true, message => ["sql" => $sql, "request" => $_POST, "params" => [$user_id, $start_date, $end_date, $marked]]]);
    die(); // all is well, kill the script now
}
