var lang; // make sure there is no refrence error
// when the page loads
$(document).ready(function(){
    localStorage.removeItem("mashpia_bug_href_source");
    if (lang && lang == 2) { // if it is set to hebrew
        $(".bug-report").addClass("he");
    }
    // when a user clicks on the bug-report button
    $(".bug-report").click(function(event){
        var data = {};
        
        if ($(event.target).data().user_id) {
            data.user_id = $(event.target).data().user_id;
        }
        
        $.post("/mobile/ajax/get_user_bug_info.php", data, function(response){
            ...
            if (response.success) {
                ...
                localStorage.setItem("mashpia_bug_account_type", "Parent");
                // user info
                ...
                } else if (data.admin && data.admin.school_name) {
                    localStorage.setItem("mashpia_bug_school_name", data.admin.school_name);
                }
                // front page info
