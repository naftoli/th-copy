...
<head>
    ...
    <title>Purchase Game Sets</title>
    <style type="text/css">
...
<header class="navbar" id="top" role="banner">
    <div class="span12" style="max-width:1024px;width:100%;margin:0 auto;float:none;text-align: center;">
        <span id="page-title">Purchase Game Set(s)</span>
    </div>
</header>
...
<section style="width: 300px; margin: 10px auto;">
    <div class="container page5">
        <div class="row" id="purchase_info" style="display: none;">
            <div class="col-12" style="font-size: 16px;">
                HUGE SALE<br />
                Great card game for<br />
                YUD DALED KISLEV
                <br /><br />
                Regularly $12 Per Game<br />
                Sale Price 50% off<br />
                ONLY $6 (includes delivery to school)
                <br /><br />
                Game Includes:<br />
                    - Game Instructions<br />
                    - 11 sets of 4 cards.<br />
                Each card has a unique fact about its category.
                <br /><br />
                Categories:
                <ol>
                    <li>The Shidduch - השידוך</li>
                    <li>The invitation - ההזמנות</li>
                    <li>The invitation - ההזמנות</li>
                    <li>Preparations - הכנות לחתונה</li>
                    <li>Days Of Joy - ימי שמחה</li>
                    <li>Kabbolas Ponim - קבלת פנים</li>
                    <li>The Escorts - השושבינים</li>
                    <li>The Chuppah - החופה</li>
                    <li>The Wedding Feast - הסעודה</li>
                    <li>The Simcha In Dnipropetrovsk - השמחה בדנייפרופטרובתק</li>
                    <li>Wedding Presents - מתנות</li>
                    <li>The Great Day - היום הבהיר</li>
                </ol>
            </div>
<!--            <div class="col-12" id="sold-out" style="font-size: 16px; display:none;">-->
<!--                <hr />-->
<!--                Wow! <span class="sold"></span> Chayolim bought their own lulov and esrog. <br />-->
<!--                Let's see who will do mivtzoim for the most amount of time!<br />-->
<!--                Let's see who will get the most shakes!<br />-->
<!--            </div>-->
            <hr />
            <h2>Number of sets: 500</h2>
            <h2>Already Sold: <span class="sold"></span></h2>
            <h2>Currently Available: <span id="leftToSell"></span></h2>
            <hr />
        </div>

        <div style="width: 250px; margin: auto; display: none;" id='no-children'>
            <div class="row">
                <div class="col-12">
                    <div>
                        <h3>
                            We're sorry, but you don't have any children that are from schools that we can send the sets to.
                        </h3>
                    </div>
                </div>
            </div>
        ...
        <div style="width: 250px; margin: auto; display: none;" id="purchases">
            <div class="row">
                <h2>Purchase For:</h2>
                <div class="col-12">
                    <div class="chilrens" id="children">
                        <script id="childrenTpl" type="text/x-handlebars-template">
                            {{#each children}}
                            <div class="child">
                                <div style="padding: 10px; font-size: 14px;">
                                            <span>
                                                {{#if gamePurchased}}
                                                {{else}}
                                                <input type="checkbox" class="game" style="width: 30px;" id="{{@key}}" />
                                                {{/if}}
                                                <b>{{rank}}</b> {{first}} {{last}}
                                            </span>
<!--                                    <div>Shipping Charge: <strong>$<span class="shipping-charge">{{lulav_shipping}}</span></strong></div>-->
                                    {{#if gamePurchased}}
                                    <div style='color:red'><strong><i>Child has already purchased a game set.</i></strong></div>
                                    {{/if}}
                                </div>
                            </div>
                            {{/each}}
            ...
            <form action="" method="post">
                <div class="row">
                    ...
                    <div class="form-group">
                        <label for="cc-first" class="control-label mb-1">First Name on Card</label>
                        <input id="cc-first" name="cc-first" type="text" class="form-control cc-first identified" required placeholder="First Name" maxlength="18">
                        <!-- <span class="invalid-feedback">Enter a valid credit card number</span> -->
                    </div>
                    <div class="form-group">
                        <label for="cc-last" class="control-label mb-1">Last Name on Card</label>
                        <input id="cc-last" name="cc-last" type="text" class="form-control cc-last identified" required placeholder="Last Name" maxlength="18">
                        <!-- <span class="invalid-feedback">Enter a valid credit card number</span> -->
                    </div>
                    <div class="form-group">
                        <label for="cc-number" class="control-label mb-1">Card number (no spaces please)</label>
                        <input id="cc-number" name="cc-number" type="number" class="form-control cc-number identified" required placeholder="XXXXXXXXXXXXXXXX" maxlength="16">
                        <!-- <span class="invalid-feedback">Enter a valid credit card number</span> -->
                    </div>
                    <div class="form-group">
                        <label for="cc-exp" class="control-label mb-1">Expiration (no dashes or slashes please)</label>
                        <input id="cc-exp" name="cc-exp" type="number" class="form-control cc-exp" required placeholder="MMYY" autocomplete="cc-exp" maxlength="4">
                        <!-- <span class="invalid-feedback">Enter the expiration date</span> -->
                    </div>
                    <label for="x_card_code" class="control-label mb-1">Security code</label>
                    <div class="form-group">
                        <input id="x_card_code" name="x_card_code" type="number" class="form-control cc-cvc" required autocomplete="off" placeholder="XXX" pattern="[0-9]{3,4}" maxlength="4">
                        <!-- <span class="invalid-feedback order-last">Enter the 3-digit code on back</span> -->
                    </div>
                    <div class="form-group">
                        <label for="zip" class="control-label mb-1">Zip / Postal Code</label>
                        <input id="zip" name="zip" type="text" class="form-control" placeholder="XXXXX" maxlength="12" required />
                        <!-- <span class="invalid-feedback">Enter a valid zip / postal code</span> -->
                    </div>
                </div>
                <div class="row" style="padding: 20px !important; text-align: center;">
                    <button id="purchase" class="btn btn-primray">Purchase Set(s)</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    document.addEventListener("DOMContentLoaded", function(event) {
        ...
        console.log(admin)
        if (!admin) { window.close() }

        // find out how many sets have been sold
        var soldOut = false;
        ...
        $.post('ajax/gamePurchases.php', function( sold ) {
            var startWith = 500;
            var alreadySold = 212; // sets set aside for hq/staff/schools
            if (sold) alreadySold += parseInt( sold );
            var leftToSell = startWith - alreadySold;
            $(".sold").text( alreadySold );
            $("#leftToSell").text( leftToSell );
            if ( soldOut || leftToSell < 1 ) {
                soldOut = true;
                $("#sold-out").show();
                $("#purchases").hide();
            }
        ...
        var year;
        $.ajax({ type: "POST", url: "/mobile/chidon/ajax/getYear.php", async: false,
            success: function( info ) { year = info; }
        });

        // load children
        $.post('ajax/getChildren.php', { admin : admin, year : year }, function( success ) {
            if ( success == -1 ) {
                ...
                // sepearate registered and un-registered
                var registred_children = {};
                // var unregistered_children = {};

                for (var c in children) {
                    var child = children[c];
                    registred_children[c] = child;
                    // if (child.mivtzaLulav) {
                    //     registred_children[c] = child;
                    // } else {
                    //     unregistered_children[c] = child;
                    // }
                }

                // Grab the template script
                var tplScript = $("#childrenTpl").html();

                // Compile the template
                var tpl = Handlebars.compile( tplScript );

                // Pass our data to the template
                var html = tpl( { children : registred_children } );
                ...
                if ( $.trim( html ) === '' ) {
                    ...
                    $(".chilrens").append( html );
                    $(".year").text(year);
                    if ( soldOut ) $("#sold-out").show();
                    else $("#purchases").show();
                }
            }
        });

        $(document).on('click', '.game', calcAmount);
        $(document).on("click", "#purchase", purchaseSets);

        function calcAmount() {
            var total = 0;
            var cost  = 6;
            var numChildren = $(".game:checked").length;

            if ( numChildren ) {
                $(".game:checked").each( function() {
                    // var id = $(this).attr('id');
                    // var shipping = parseInt( $(this).parent().parent().find('.shipping-charge').text() );
                    // if (shipping > 0) {
                    //     total += cost + shipping;
                    // } else {
                        total += cost;
        ...
        function purchaseSets( e ) {
            e.preventDefault();
            var total = $("#total").val();
            if ( total > 0 ) {

                var first = $("#cc-first").val().trim();
                var last = $("#cc-last").val().trim();
                var num = $("#cc-number").val().trim();
                var exp = $("#cc-exp").val().trim();
                var zip = $("#zip").val().trim();
                var cvv = $("#x_card_code").val().trim();

                if ( !first || !last || !num || !exp || !zip || !cvv ) {
                    alert("You must fill in all fields.");
                    return false;
                }

                if ( isNaN( num ) || isNaN( exp ) || isNaN( cvv ) ) {
                    alert("Credit Card Number, Expiry Date, and Security Code may only contain DIGITS.");
                    return false;
                }
                ...
                info.zip = zip;
                info.users = [];
                $.each($(".game:checked"), function() {
                    var id = $(this).attr('id');
                    info.users.push( id );
                });

                var num_sets = info.users.length;
                $.post('ajax/gamePayment.php', { info: info }, function( info ) {
                    var result = JSON.parse( info );
                    if ( result.success ) {
                        alert("Thank you! You have successfully purchased " + num_sets + " set(s). Your child(ren)'s school will receive them for your child(ren).");
                        // location.reload();
                    } else {
                        alert( result.error );
                    }
                ...
                alert("You have not indicated how many sets you need.");
            }
        }
    });
</script>
