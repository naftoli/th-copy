...
Medal.prototype.render = function () {
    ...
    html += '<div  class="progress-bar ' + this.getColor(true).toLowerCase() + '" role="progressbar" style="width: ' + status_width + '%;float:right;"></div>';

    if (this.next - 1 < this.colors.length) {
        html += '<span style="direction:rtl;">' + (this.needed - this.total) + " עד " + this.getColor(false) + '</span>';
    } else {
        html += '<span>הסתיים!</span>';
    }
