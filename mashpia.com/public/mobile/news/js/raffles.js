...
async function getLatestRaffle() {
  const res = await fetch('api/getLatestRaffle.php')
...
document.addEventListener("DOMContentLoaded", async function (event) {
  ...
  raffle_id = await getLatestRaffle()
  raffles.loadRaffle(raffle_id);
...
winner_renderer.prototype.render = function () {
  ...
  if (this.show_image) {
    html += '<div class="image" style="background-image: url(' + pic_src + ')" alt="picture of prize"></div>';
  }
  ...
  if (this.show_image) {
    html += 'Prize: ' + this.winner.prize_name;
  }
