...
if ($school_id[0] > 0) {
    foreach ($school_id as $id) {
        $discounts = $d->getAllStudentDiscountsForSchoolYear($year, $id);
        $info = array_merge($info, $discounts);
    }
} else {
    $discounts = $d->getAllStudentDiscountsForYear($year);
    $info = array_merge($info, $discounts);
}

$data = [];
$headers = ['Year', 'Soldier', 'Amount', 'Reason', 'Created By', 'Created', 'Used'];
foreach ($info as $discount) {
    $row = [];
    $row['Year'] = $discount['year'];
    $row['Soldier'] = $discount['first'] . ' ' . $discount['last'];
