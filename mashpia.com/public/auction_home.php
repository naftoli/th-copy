...
//change kiosk_auction in table to show auction in page
$auctions = mq("SELECT auction_id, auction_name, auction_message, auction_date, auction_run_date FROM auctions WHERE kiosk_auction = 1 and auction_ran = 0 AND (auction_run_date IS NULL OR auction_run_date >= " . unixtojd() . ")  AND (school_id IS NULL OR school_id = {$user['school_id']})");
?>
