...
class ChidonShipping
{
    ...
    public function getGuides($gender, $school, $guides = []) {
        ...
        foreach ($guides as $guide) {
            if (in_array($guide, ['study guides', 'limud trackers', 'chidon bookmarks'])) {
                $sql = "select * from th_chidon tc 
                        join users u using (user_id) 
                        where tc.year = :year";
                if ($gender == 'm') $sql .= " and u.gender = 'M'";
                ...
                foreach ($rows as $row) {
                    $cat = 'guides';
                    $item = substr($guide, 0, strlen($guide) - 1);
                    $book = $row['book'];
                    $id = $this->getItemID($cat, $guide, $item . ' ' . $book);
                    $info[$row['user_id']][] = [
                        'item'  => $guide,
                        'size'  => $book,
                        'color' => '',
    ...
    public function getRecruitmentPrizes($gender, $school, $limitTo = []) {
        ...
        if (! isset($children[$this->year])) return $info;
...
//        echo "highest track3: " . $highest_track3 . ' key3: ' . $key3 . "<br />";
    ...
    public function getPrizes($gender, $school, $limitTo = []) {
        ...
        $sql = "SELECT 
                    ...
                    u.school_id, u.class_id, tc.th_chidon_id, tc.reward_type 
                FROM
...
//        echo $sql; exit;
        ...
        foreach ($rows as $row) {
            if (in_array($row['user_id'], $this->toExclude)) continue;
            if (!empty($this->only) && !in_array($row['user_id'], $this->only)) continue;
            if ($row['reward_type'] == 'maven') continue; // don't show prizes if registered for yesod

            // check that child's track is higher than yesod
            if (in_array($row['reward_type'], ['', 'highest track passed']) || is_null($row['reward_type'])) {
                $ct = new ChidonTests();
                $child = [
                    'user_id' => $row['user_id'],
                ...
                if (in_array($track, ['', 'Yesod', 'yesod', 'maven'])) continue;
            }
...
//        echo "<pre>"; print_r($info); echo "</pre>"; exit;
    ...
    public function getItems() {
        ...
        $items = [
            'brochures'             => ['brochure'],
            'guides'                => ['study guides', 'khk guides', 'limud trackers', 'chidon bookmarks'],
    ...
    public function getItemID($cat, $item, $deep = '', $deeper = '') {
        ...
        $item_ids = [
            ...
            'guides'    => [
                ...
                'limud trackers'  => [
                    'limud tracker 1' => 'CHI02A',
                    'limud tracker 2' => 'CHI02B',
                    'limud tracker 3' => 'CHI02C',
                    'limud tracker 4' => 'CHI02D',
                    'limud tracker 5' => 'CHI02E'
                ], 
                'chidon bookmarks' => [
                    'chidon bookmark 1' => 'CHI03A',
                    'chidon bookmark 2' => 'CHI03B',
                    'chidon bookmark 3' => 'CHI03C',
                    'chidon bookmark 4' => 'CHI03D',
                    'chidon bookmark 5' => 'CHI03E'
                ],
                'khk guides'     => 'CHI012',
            ],
            'recruitment prizes'    => [
