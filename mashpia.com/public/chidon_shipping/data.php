...
function addToSummary($item, $school, $status) {
    ...
    $key = $item['id'];
    if (! empty(trim($item['name'])) && strpos($item['item'], 'Name Bracelet') === false) $key .= 'P';
        ...
        foreach ($more as $user_id => $list) {
            foreach ($list as $item) {
                ...
                $csv[$i++] = [$admin['admin_id'], ($first . ' ' . $admin['last']), $admin['first'], $admin['last'],
                    ...
                    $qty, ($user['u_first'] . ' ' . $user['u_last'] . ' - ' . $user['user_serial']), $admin['admin_email'], ($child_count[$admin['admin_id']] ?? 0), '',
