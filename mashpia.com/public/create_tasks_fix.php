                                ...
                                foreach( $info as $start => $arr ) {
                                    foreach( $arr as $end => $info ) {
                                        foreach ($info as $lang => $other) {
                                            // find out if mission exists
                                            $id = 0;
                                            $sql = "select date_tasks_mission_id from date_tasks_missions
                                                    where school_type_id = " . mysql_real_escape_string($type) . "
                                                    and subject_id = " . mysql_real_escape_string($subject_id) . " 
                                                    and level = " . mysql_real_escape_string($level) . "  
                                                    and track_id = " . mysql_real_escape_string($track) . "  
                                                    and mission_name = \"" . mysql_real_escape_string( $mName ) . "\"   
                                                    and mission_value = " . mysql_real_escape_string($mVal) . "  
                                                    and start_date = " . mysql_real_escape_string($start) . "  
                                                    and end_date = " . mysql_real_escape_string($end) . " 
                                                    and lang_id = " . mysql_real_escape_string($lang);
                                            $result = mysql_query($sql);
                                            ...
                                            if ($id) {
                                                ...
                                                foreach ( $other as $task ) {
                                                    // find out if task exists
                                                    $sql = "select date_task_id from date_tasks
                                                            where date_tasks_mission_id = $id  
                                                            and name = \"" . mysql_real_escape_string( $task['task'] ) . "\" 
                                                            and cat_ord_new = " . mysql_real_escape_string( $task['catOrd'] ) . " 
                                                            and cat = \"" . mysql_real_escape_string( $task['cat'] ) . "\" 
                                                            and points = " . $task['points'] . " 
                                                            and short_name = \"" . mysql_real_escape_string($task['short_name']) . "\"
                                                            and grid_marking = " . mysql_real_escape_string($task['grid_marking']) . "
                                                            and mission_marking = " . mysql_real_escape_string($task['mission_marking']) . "
                                                            and daily_task = " . mysql_real_escape_string($task['daily']) . "
                                                            and label_id = " . mysql_real_escape_string($task['label']) . "
                                                            and grid_id = " . mysql_real_escape_string($task['grid_id']); 
                                                    if ( $task['mand'] ) { 
                                                        $sql .= " and mandatory_qty = 1 and optional_qty = 0";
                                                    } else {
                                                        $sql .= " and mandatory_qty = 0 and optional_qty = 1";
                                                    }
                                                    if ( $task['labelOrd'] ) {
                                                        $sql .= " and label_ord = " . mysql_real_escape_string($task['labelOrd']);
                                                    }
                                                    if ( $task['qty'] ) {
                                                        $sql .= " and quantity = " . mysql_real_escape_string($task['qty']);
                                                    }
                                                    if ( $task['needed'] ) {
                                                        $sql .= " and needed = " . mysql_real_escape_string($task['needed']);
                                                    }
                                                    if ( $task['focus'] ) {
                                                        $sql .= " and focus_task = " . mysql_real_escape_string($task['focus']);
                                                    }
                                                    if ( !$task['def'] ) {
                                                        $sql .= " and default_on = 0";
                                                    }
                                                    if ( $task['pic'] ) {
                                                        $sql .= " and medium_pic = \"" . mysql_real_escape_string($task['pic']) . "\"";
                                                    }
                                                    $result = mysql_query($sql);
                                                    if (mysql_num_rows($result) == 0) {
...
?>        
        ...
        <div id="task_form">
            <h2>Upload File</h2>
            <form enctype="multipart/form-data" action="create_tasks_fix.php" method="post">
                Choose Campaign: <br />
