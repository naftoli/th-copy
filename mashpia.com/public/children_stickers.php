...
//echo "<pre>"; print_r($childrenSelected); echo "</pre>"; exit;
    ...
    <BODY>
		...
		if (!isset($_POST['submit'])) {
			foreach ($missions as $id => $mission) {
				if (in_array($id, array(21,92,93,94,100))) continue;
				$str .= $id . ',';
				$missionsSelected[] = $id;
			}
		} else {
		...
		if (empty($childrenSelected)) {
			echo "You have no registered children. Please click on the 'Register Children' link under 'Child Management'.";
			exit;
		}
		...
		if (!$modify) {
			echo "Click <a href='children_stickers.php?modify=1'>here</a> to modify report.<br /><br />";
		}
