...
<HTML>
    ...
    <BODY>
        ...
        $userMedals = array();      
        require_once 'class.medalsEarned.php';
        $subjects = MedalsEarned::getAllSubjects();
        $medals = MedalsEarned::getAllMedals();
        ...
        if ( isset( $_POST['submit'] ) ) {
            ...
            $showDateEarned = empty( $_POST['showDateEarned'] ) ? false : true; 
            ...
            foreach ( $schools as $id => $school ) {
                $m = new MedalsEarned( $id );
                if ( isset( $start_date ) ) 
                    $m->setDates( $start_date, $end_date );
                if ( isset( $_POST['classes'] ) ) 
                    $m->setClasses( $_POST['classes'] );
                $m->setMedalsEarned( $subjectsPosted );
                $userMedals[$id] = $m->getMedalsEarned();
            }
            ...
            */
            ...
            foreach ( $userMedals as $school => $users ) {
                echo "<h2>" . $schools[$school] . "</h2>";
                if ( isset( $start_he ) )
                    echo "<p>Medals earned between " . $start_he . " and " . $end_he . "</p>";
                echo "<table>";
                echo "<tr><th>Grade</th><th>Student</th>";
                foreach ( $subjectsPosted as $sp ) {
                    echo "<th>" . $subjects[$sp] . "</th>";
                }
                echo "</tr>";
                foreach ( $users as $class => $user ) {
                    foreach ( $user as $name => $campaigns ) {
                        echo "<tr><td>" . $class . "</td><td>" . $name . "</td>"; 
                        foreach ( $subjectsPosted as $sp ) {
                            if ( !isset( $campaigns[$subjects[$sp]] ) ) {
                                echo "<td>&nbsp;</td>";
                            } else {
                                echo "<td>"; 
                                foreach ( $campaigns as $campaignName => $campaign ) {
                                    $i = 1; 
                                    if ( $campaignName == $subjects[$sp] ) {
                                        foreach ( $campaign as $medal ) { 
                                            $medalInfo = explode( ':', $medal ); 
                                            $date = jdtogregorian( $medalInfo[1] );
                                            if ( $showDateEarned ) {
                                                echo $medals[$medalInfo[0]] . "<br />(" . $date . ")<br />";
                                            } else {
                                                echo $medals[$medalInfo[0]] . "<br />";
                                            }
                                        }
                                    }
                                }
            ...
            ?>
            <form action="medals_report.php" method="post">
                <fieldset>
                    <legend>
                        Select Campaigns
                    </legend>
                ...
                if ( $numSubjects > 1 ) {
                    $middle = ceil( $numSubjects / 3.0 );
                    echo "<div class='subjectSelection'>";
                    $i = 0;
                    foreach ( $subjects as $sid => $subject ) {
                        if ( $i++ == $middle ) {
                            $middle *= 2;
                            echo "</div><div class='subjectSelection'>";
                        }
                        echo "<input type='checkbox' class='mission' name='subjects[]' value='" . $sid . "' />" . $subject . "<br />";
                    }
                    echo "</div>";
                }
                ?>
                ...
                <fieldset>
                    ...
                    <input type="checkbox" name="showDateEarned" />
                    Show date medals earned
                </fieldset>
                <div align='center'>
