<?php
$admin_auth = array('camp'); 

require('header.php');
require_once('calendar.php');

$admin_id = gri("admin_id");

// ***** Determine if the user is a camp director or a super user ***** //
if ($admin_user['auth'] == "super")
	$user_type = "super";
else
	$user_type = "camp";
// ***** Determine if the user is a camp director or a super user ***** //

// ***** Camp Information ***** //
if ($user_type == "camp") {
	$camp_id = $admin_user['auths']['camp'][0]; 
	$authorization = mysql_fetch_assoc(mq("SELECT r.role_auth FROM admin_auths AS aa JOIN roles AS r USING (role_id) WHERE aa.admin_id=" . $admin_user['admin_id']));
	$role_auth = $authorization['role_auth'];
}
else {
	$camp_id = gri('camp_id', -1);
}

if ($user_type == "camp" || $camp_id > -1) {
	$sql = "SELECT * FROM camps WHERE camp_id=" . $camp_id;
	$query = mysql_query($sql);
	$camp = mysql_fetch_assoc($query);
	$camp_name = $camp['camp_name'];
}
// ***** Camp Information ***** //


$group_type_id = gri('group_type_id', -1);
$division_id = gri('division_id', -1);
$group_id = gri('group_id', -1);

$action = gr('action', '');
if ($action != '') {

	switch($action) {
		case 'add':
			$group_name = ms(gr('group_name'));

			$num_rows = mysql_num_rows(mq("SELECT * FROM groups WHERE group_id=" . $group_id . " AND group_name=" . $group_name));
			if ($num_rows == 0) {
				$sql = "INSERT INTO groups SET division_id=" . $division_id .  ", group_name=" . ms(gr('group_name'));
				mq($sql);
				$action = "";
			}
			else {
				$message = T_('Group name already exists for this division');
				$action = "";
			}
		break;
		
		case 'save':
			$sql = "UPDATE groups SET group_name=" . ms(gr('group_name')) . " WHERE group_id=" . $group_id;
			mq($sql);
			$action = "";
		break;
		
		case 'delete':
			$sql = "DELETE FROM groups WHERE group_id=" . $group_id;
			mq($sql);
			$action = "";
		break;	

		//case "add":
		//	$row = mysql_fetch_assoc(mq("SELECT divisions FROM groups WHERE group_id=" . $group_id));
		//	if ($row['divisions'] == 0) {
		//		$message = "Divisions cannot be added";
		//		$action = "";
		//	}
		//break;
		
	}

}
?>

<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01//EN" "http://www.w3.org/TR/html4/strict.dtd">

<HTML DIR="<?=$dir?>">

	<HEAD>
		<TITLE><?=T_('Divisions'), ' - ', T_('Tzivos Hashem Management System')?></TITLE>
		<LINK href="admin_styles.css" rel="stylesheet" type="text/css">
		<SCRIPT type="text/javascript" src="icalendar.js"></SCRIPT>
		<SCRIPT type="text/javascript" src="ajax_get_groups.js"></SCRIPT>
		<script type="text/javascript">	
			var admin_id = "<?=$admin_id;?>";
			var user_type = "<?=$user_type;?>";
			var camp_id = "<?=$camp_id;?>";
			var group_type_id = "<?=$group_type_id;?>";
			var division_id = "<?=$division_id;?>";
			var group_id = "<?=$group_id;?>";
						
			var divs_array = ["div_1", "div_2", "div_3", "div_4"];
			var divisions = "";
		</script>		
		
	</HEAD>
	
<? if ($action == "") : ?>	
	<body onload="get_groups('1234', 'groups_form');">
<? else : ?>
	<body>
<? endif; ?>		

		<? include('admin_header.php'); ?>
		
		<DIV CLASS="body">
		
			<H1>
				<?=T_('Divisions')?>
			</H1>
			
			<? if (!empty($message)) : ?>
				<H2>
					<label style="color:red;"><?=$message?></label>
				</H2>
			<? endif; ?>
			
<!-- **************************************** ADD NEW or EDIT **************************************** -->			
<? if ($action == 'add_new' || $action == "edit") : ?>

	<form name="groups_form" id="groups_form" action="admin_groups.php" method="post" accept-charset="UTF-8">
		<input type="hidden" name="action" id="action" value="">		
		<input type="hidden" name="camp_id" id="camp_id" value="<?=$camp_id;?>">
		<input type="hidden" name="group_type_id" id="group_type_id" value="<?=$group_type_id;?>">
		<input type="hidden" name="division_id" id="division_id" value="<?=$division_id;?>">
		<? if ($action == "edit") : ?>
		<input type="hidden" name="group_id" id="group_id" value="<?=$group_id;?>">		
		<? endif; ?>
		
		<h2><?=T_('Camp');?>: <label style="color:blue;"><?=$camp_name;?></label></h2>
		
		<? $row = mysql_fetch_assoc(mq("SELECT division_name, group_type_name FROM divisions JOIN group_types USING (group_type_id) WHERE division_id=" . $division_id)); ?>
		<h2><?=T_('Group Type');?>: <label style="color:blue;"><?=$row['group_type_name'];?></label></h2>
		
		<h2><?=T_('Group');?>: <label style="color:blue;"><?=$row['division_name'];?></label></h2>
		
		<? 
			if ($action == "edit") {
				$row = mysql_fetch_assoc(mq("SELECT group_name FROM groups WHERE group_id=" . $group_id));
				$group_name = $row['group_name'];
			}
			else {
				$group_name = "";
			}
		?>
		
		<br />
				
		<table>
			<tr><td><?=T_('Group Name');?>:<input type="text" size="100" maxlength="100" name="group_name" id="group_name" value="<?=$group_name;?>"></td></tr>
		</table>

		<br />
		
		<? if ($action == "add_new") : ?>
			<input type="submit" value="<?=T_('ADD');?>" onclick="document.getElementById('action').value='add';">
			<input type="submit" value="<?=T_('CANCEL');?>" onclick="document.getElementById('action').value='';">
		<? else : ?>
			<input type="submit" value="<?=T_('SAVE');?>" onclick="document.getElementById('action').value='save';">
			<input type="submit" value="<?=T_('CANCEL');?>" onclick="document.getElementById('action').value='';">		
		<? endif; ?>
	</form>
	
<? endif; ?> <!-- if ($action == 'add_new') : -->
<!-- **************************************** ADD NEW or EDIT **************************************** -->			

<!-- **************************************** NO ACTION **************************************** -->			
<? if ($action == "") : ?>

	<form name="groups_form" id="groups_form" action="admin_groups.php" method="post" accept-charset="UTF-8">
		<input type="hidden" name="action" id="action" value="">
		<? if ($camp_id > 0) : ?>
		<input type="hidden" name="camp_id" id="camp_id" value="<?=$camp_id;?>">
		<? endif; ?>	
		<input type="hidden" name="group_id" id="group_id" value="<?=$group_id;?>">
		
		<a href="#" onclick="document.getElementById('action').value='add_new'; document.forms['groups_form'].submit();"><?=T_('Add New Group')?></a>
			
		<br />
		<br />
		
		<div id="div_1">
		</div>		
		
		<br />
		<br />
		
		<div id="div_2">
		</div>		
		
		<br />
		<br />
		
		<div id="div_3">
		</div>		
		
		<br />
		<br />
		
		<div id="div_4">
		</div>		
		
		<br />
				
	</form>
	
<? endif; ?> <!-- if ($action == "") : -->
<!-- **************************************** NO ACTION **************************************** -->			

		</DIV>
	
		<? include('admin_footer.php'); ?>
		
	</BODY>
	
</HTML>
