...
<html>
	...
	<h1>Hachayol Report (Paid for Shipping)</h1>
		...
		Shipment Number: <select id="shipment_number" class="form-control">
            <option value="0">Choose Shipment Number</option>
	...
	<script>
		const shipmentNumber = document.getElementById('shipment_number');
		shipmentNumber.addEventListener('change', () => {
            if (this.value > 0) {
                fetch('api/getShipmentInfo.php', {
                    method: 'POST',
                    body: JSON.stringify({ shipment_number: this.value })
                })
