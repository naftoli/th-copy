...
CREATE TABLE `users` (
  ...
  `lite_edition` tinyint(3) unsigned NOT NULL DEFAULT 0,
  `chidon_pic_5781` varchar(100) DEFAULT NULL,
  `ckids` tinyint(4) DEFAULT 0,
  `hachayols` tinyint(1) NOT NULL DEFAULT 1,
  `medals_ranks` tinyint(1) NOT NULL DEFAULT 1,
  `first_known_en` varchar(45) DEFAULT NULL,
  `last_known_en` varchar(45) DEFAULT NULL,
  `first_known_he` varchar(45) DEFAULT NULL,
  `last_known_he` varchar(45) DEFAULT NULL,
  `chayolei_eligible` tinyint(3) unsigned NOT NULL DEFAULT 0,
  `chidon_eligible` tinyint(3) unsigned NOT NULL DEFAULT 0,
  `khk_eligible` tinyint(3) unsigned NOT NULL DEFAULT 0,
  `non_th_school` varchar(65) DEFAULT NULL,
  `non_th_school_id` int(10) unsigned NOT NULL DEFAULT 0,
  `chidon_pic_5782` varchar(95) DEFAULT NULL,
  `hachayol` tinyint(3) unsigned NOT NULL DEFAULT 0,
  `mothers_name` varchar(65) DEFAULT NULL,
  `comments` varchar(255) DEFAULT NULL,
  `cached_medal_count` int(10) unsigned DEFAULT 0,
  `cached_rank` int(10) unsigned DEFAULT 0,
  `cached_mission_count` int(10) unsigned DEFAULT 0,
  PRIMARY KEY (`user_id`),
  UNIQUE KEY `user_code` (`user_code`),
  UNIQUE KEY `user_serial` (`user_serial`),
  KEY `registration` (`user_registered`),
  KEY `medals` (`medals_ranks`),
  KEY `hachayol` (`hachayol`),
  KEY `gender` (`gender`),
  KEY `school_id` (`school_id`),
  KEY `class_id` (`class_id`),
  KEY `idx_cached_leaderboard` (`cached_rank`,`cached_medal_count`,`cached_mission_count`),
  KEY `idx_users_class` (`class_id`,`user_registered`),
  KEY `idx_users_school` (`school_id`,`user_registered`)
) ENGINE=InnoDB AUTO_INCREMENT=81746 DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;
...
-- Dump completed on 2026-04-30 16:35:35
